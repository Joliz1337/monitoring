...
export default function CpuCoresChart({ perCpuPercent, className = '' }: CpuCoresChartProps) {
  ...
  const coresPerRow = Math.min(8, cores.length)
  const barHeight = cores.length > 48 ? 'h-5' : cores.length > 16 ? 'h-6' : 'h-8'
  const fontSize = cores.length > 32 ? '9px' : cores.length > 16 ? '10px' : '12px'
  ...
  return (
    <div className={className}>
      ...
      <div
        ...
        style={{
          gridTemplateColumns: `repeat(${coresPerRow}, 1fr)`
        }}
      >
        {cores.map((core) => (
          <div
