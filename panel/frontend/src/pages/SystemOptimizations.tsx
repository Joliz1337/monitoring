...
interface NodeState {
  ...
  // Host RAM/CPU changed since the tuning was rendered. Every size-dependent
  // sysctl was computed from the OLD facts, so a re-apply is needed even though
  // configs/VERSION has not moved.
  drift: boolean
...
export default function SystemOptimizations() {
    ...
    try {
      ...
      const worker = async () => {
        while (queue.length > 0) {
          ...
          if (!node) return
          await fetchNodeData(node.id)
        }
      }
      const workers = Array.from({ length: Math.min(POOL_SIZE, queue.length) }, worker)
      Promise.all(workers).catch(() => { /* per-node ошибки уже обработаны внутри fetchNodeData */ })
    } catch {
  ...
  const needsUpdate = (node: NodeState): boolean => {
    if (!node.installed) return Boolean(baseInfo?.optimizations?.latest_version)
    // Drift alone is enough: the config on disk may match the current VERSION
    // and still be wrong for this host, because every size-dependent value was
    // derived from the RAM/CPU it had at render time.
    if (node.drift) return true
