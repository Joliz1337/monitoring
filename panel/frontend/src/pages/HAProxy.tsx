...
export default function HAProxy() {
  ...
  useEffect(() => {
    if (!serverId) return
    haproxyProfilesApi.getAvailableServers().then(res => {
      const srv = res.data.find((s: { id: number }) => s.id === Number(serverId))
      if (srv?.active_profile_id) {
        haproxyProfilesApi.getProfile(srv.active_profile_id).then(pRes => {
          setProfileInfo({ name: pRes.data.name, id: pRes.data.id })
        }).catch(() => {})
      }
    }).catch(() => {})
  }, [serverId])
  ...
  // Each request has its own catch — partial success updates what it can
  ...
  const liveRefreshRef = useRef<() => Promise<void>>()
  liveRefreshRef.current = refreshData

  useEffect(() => {
    fetchServers()
    fetchData().then(() => {
      // After showing cached data, fetch live data to ensure freshness
      const timer = setTimeout(() => liveRefreshRef.current?.(), 300)
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex gap-2">
                              ...
                              />
                              <Tooltip label={t('haproxy.generate_random_email')}>
                                <motion.button
                                  type="button"
                                  onClick={() => {
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-hidden p-5">
                      {configLoading ? (
                        ...
                        <pre className="w-full h-[50vh] bg-dark-950 border border-dark-700 rounded-xl p-4
                                     font-mono text-sm text-dark-200 overflow-auto
                                     scrollbar-thin scrollbar-thumb-dark-700 scrollbar-track-transparent whitespace-pre"
                        >{configContent}</pre>
                      )}
                  ...
                  {filteredFirewallRules.map((rule, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Tooltip label={t('firewall.close_port')}>
                          <motion.button
