...
export default function HAProxy() {
  ...
  // Auto-refresh removed - data is cached on panel (every 30s)
  ...
  const handleSubmitRule = async (e: FormEvent) => {
    ...
    try {
      ...
      setEditingRule(null)
      setRuleForm({ name: '', rule_type: 'tcp', listen_port: '', target_ip: '', target_port: '', cert_domain: '', target_ssl: true })
    } catch (err: unknown) {
  ...
  const handleRenewCerts = async () => {
    ...
    try {
      await proxyApi.renewCerts(Number(serverId))
      await refreshData()
    } catch {}
    setActionLoading(null)
  }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="text-dark-400">{t('haproxy.loading')}</p>
      </div>
    )
  }
