...
import { SSHOverviewTable } from '../components/ssh/SSHOverviewTable'
import { useSSHBulkStream, BulkProgressState } from '../components/ssh/useSSHBulkStream'
...
function PresetCard({ type, icon, iconBg, title, desc, preset, applyingPreset, bulkApplying, bulkDisabled, selectedCount, onApply, onBulk, onDelete, t }: PresetCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 mt-auto pt-1">
        ...
        <motion.button
          onClick={onBulk}
          disabled={bulkApplying || bulkDisabled}
          className="btn btn-secondary text-xs px-3 py-1.5"
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
        >
          {t('ssh_security.apply_to_selected', { count: selectedCount })}
        </motion.button>
      </div>
    </motion.div>
  )
...
export default function SSHSecurity() {
  ...
  const showBulkSummaryToast = (s: BulkProgressState) => {
    if (s.error) {
      toast.error(s.error)
      return
    }
    if (!s.finished) return // отменено пользователем
    if (s.failed === 0) {
      toast.success(t('ssh_security.bulk_all_ok', { ok: s.ok, total: s.total }))
    } else if (s.ok === 0) {
      toast.error(t('ssh_security.bulk_all_failed', { total: s.total }))
    } else {
      toast.warning(t('ssh_security.bulk_partial', { ok: s.ok, total: s.total, failed: s.failed }))
    }
  }
  ...
  const handleApply = async () => {
    if (!activeServerId || !hasChanges) return
    setApplying(true)
    try {
      if (hasSSHChanges) {
        const res = await sshSecurityApi.updateConfig(activeServerId, editedConfig)
        if (res.data.warnings?.length) {
          res.data.warnings.forEach(w => toast.warning(w))
        }
      }
      if (hasFail2banChanges) {
        await sshSecurityApi.updateFail2ban(activeServerId, editedFail2ban)
      }
  ...
  const handleBulkApply = async () => {
    if (!hasChanges || bulkDisabled) return
    const body: { server_ids: number[]; ssh?: Partial<SSHConfig>; fail2ban?: Partial<Fail2banConfig> } = {
      server_ids: selectedServerIds,
    }
    if (hasSSHChanges) body.ssh = editedConfig
    if (hasFail2banChanges) body.fail2ban = editedFail2ban
    const result = await bulk.run(sshBulkStreamUrls.apply, body)
    showBulkSummaryToast(result)
    await refreshActive()
  }

  const handleApplyPreset = async (type: string) => {
    if (!presets || !activeServerId) return
    const preset = type === 'recommended' || type === 'maximum'
      ? presets[type]
      : presets.custom?.find(p => p.name === type)
    if (!preset) return
    setApplyingPreset(type)
    try {
      if (preset.ssh && Object.keys(preset.ssh).length > 0) {
        await sshSecurityApi.updateConfig(activeServerId, preset.ssh)
      }
      if (preset.fail2ban && Object.keys(preset.fail2ban).length > 0) {
        await sshSecurityApi.updateFail2ban(activeServerId, preset.fail2ban)
      }
  ...
  const handleBulkPreset = async (type: string) => {
    if (!presets || bulkDisabled) return
    const preset = type === 'recommended' || type === 'maximum'
      ? presets[type]
      : presets.custom?.find(p => p.name === type)
    if (!preset) return
    const body: { server_ids: number[]; ssh?: Record<string, unknown>; fail2ban?: Record<string, unknown> } = {
      server_ids: selectedServerIds,
    }
    if (preset.ssh && Object.keys(preset.ssh).length > 0) body.ssh = preset.ssh
    if (preset.fail2ban && Object.keys(preset.fail2ban).length > 0) body.fail2ban = preset.fail2ban
    if (!body.ssh && !body.fail2ban) return
    const result = await bulk.run(sshBulkStreamUrls.apply, body)
    showBulkSummaryToast(result)
    await refreshActive()
  }
  ...
  const handleUnban = async (ip: string) => {
    ...
    setUnbanningIp(ip)
    try {
      await sshSecurityApi.unbanIp(activeServerId, ip)
  ...
  const handleUnbanAll = async () => {
    ...
    try {
      await sshSecurityApi.unbanAll(activeServerId)
  ...
  const handleAddKey = async () => {
    if (!activeServerId || !newKeyText.trim()) return
    setAddingKey(true)
    try {
      await sshSecurityApi.addKey(activeServerId, newKeyText.trim())
      toast.success(t('ssh_security.key_added'))
      setNewKeyText('')
      const res = await sshSecurityApi.getKeys(activeServerId)
  ...
  const handleBulkAddKey = async () => {
    if (!newKeyText.trim() || bulkDisabled) return
    const result = await bulk.run(sshBulkStreamUrls.keys, {
      server_ids: selectedServerIds,
      public_key: newKeyText.trim(),
      user: 'root',
    })
    showBulkSummaryToast(result)
    if (result.finished && !result.error) setNewKeyText('')
    await refreshActive()
  }
  ...
  const handleRemoveKey = async (fingerprint: string) => {
    ...
    try {
      await sshSecurityApi.removeKey(activeServerId, fingerprint)
  ...
  const handleChangePassword = async () => {
    if (!activeServerId || !passwordValue) return
    setChangingPassword(true)
    try {
      await sshSecurityApi.changePassword(activeServerId, passwordValue, passwordUser)
  ...
  const handleBulkPassword = async () => {
    if (!passwordValue || passwordValue.length < 8 || bulkDisabled) return
    const result = await bulk.run(sshBulkStreamUrls.password, {
      server_ids: selectedServerIds,
      user: passwordUser,
      password: passwordValue,
    })
    showBulkSummaryToast(result)
    if (result.finished && !result.error) setPasswordValue('')
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      >
        ...
        {/* Mode switch */}
        <div className="flex gap-1 bg-dark-800/60 p-1 rounded-xl">
          <button
            onClick={() => setMode('overview')}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
              mode === 'overview' ? 'bg-accent-500/20 text-accent-400' : 'text-dark-400 hover:text-dark-200'
            }`}
          >
            <LayoutGrid className="w-4 h-4" />
            {t('ssh_security.mode_overview')}
          </button>
          <button
            onClick={() => setMode('manage')}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
              mode === 'manage' ? 'bg-accent-500/20 text-accent-400' : 'text-dark-400 hover:text-dark-200'
            }`}
          >
            <SlidersHorizontal className="w-4 h-4" />
            {t('ssh_security.mode_manage')}
          </button>
        </div>
      </motion.div>
      ...
      {servers.length === 0 ? (
        ...
        <SSHOverviewTable servers={servers} onOpenServer={openServer} />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
          {/* Left: server multi-select */}
          <ServerSelector
            servers={servers}
            selectedIds={selectedServerIds}
            onChange={setSelectedServerIds}
            activeId={activeServerId}
            onOpenServer={setActiveServerId}
          />

          {/* Right: settings of the active server */}
          <div className="space-y-6 min-w-0">
            {/* Active server status */}
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="text-sm text-dark-400">
                {t('ssh_security.active_server')}:{' '}
                <span className="text-dark-100 font-medium">{activeServer?.name || '—'}</span>
              </div>
              {status && (
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    status.sshd_running ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'
                  }`}>
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    status.fail2ban_running ? 'bg-emerald-500/15 text-emerald-400' : 'bg-dark-700 text-dark-400'
                  }`}>
            ...
            {presets && activeServerId && !nodeUnsupported && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <PresetCard
                  ...
                  bulkApplying={bulk.progress.active}
                  bulkDisabled={bulkDisabled}
                  selectedCount={selectedServerIds.length}
                  onApply={() => handleApplyPreset('recommended')}
                  onBulk={() => handleBulkPreset('recommended')}
                  t={t}
                />
                <PresetCard
                  type="maximum"
                  ...
                  onBulk={() => handleBulkPreset('maximum')}
                  t={t}
                />
                {presets.custom?.map(cp => (
                  <PresetCard
                    ...
                    onBulk={() => handleBulkPreset(cp.name)}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2 pt-1">
                            ...
                            <motion.button
                              ...
                              disabled={bulkDisabled || !passwordValue || passwordValue.length < 8}
                              className="btn btn-secondary text-xs px-3 py-1.5"
                              whileHover={{ scale: 1.02 }}
                              whileTap={{ scale: 0.98 }}
                            >
                              {bulk.progress.active ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null}
                              {t('ssh_security.apply_to_selected', { count: selectedServerIds.length })}
                            </motion.button>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-4">
                      ...
                      <div className="card xl:w-80">
                        ...
                        >
                          ...
                          {t('ssh_security.keys_add_active')}
                        </motion.button>
                        <motion.button
                          onClick={handleBulkAddKey}
                          disabled={bulkDisabled || !newKeyText.trim()}
                          className="btn btn-secondary w-full"
                          whileHover={{ scale: 1.02 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          {bulk.progress.active ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                          {t('ssh_security.apply_to_selected', { count: selectedServerIds.length })}
                        </motion.button>
                      </div>
                    </div>
      ...
      {/* Sticky Bottom Bar */}
      <AnimatePresence>
        {hasChanges && activeServerId && mode === 'manage' && (
          <motion.div
            ...
            <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {t('ssh_security.apply_to_active')}
                </motion.button>
                <motion.button
