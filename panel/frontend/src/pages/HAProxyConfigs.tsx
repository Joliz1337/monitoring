...
function RuleForm({
  ...
  const autoWeights = async () => {
    ...
    try {
      ...
      const coresMap = res.data  // { "nd2.example.com": 6, "nd.example.com": 4 }
...
function ProfileDetailPanel({ profileId, onRefreshList }: { profileId: number; onRefreshList: () => void }) {
  ...
  const handleStartHaproxy = async (serverId: number) => {
    setStartingServerId(serverId)
    try {
      await proxyApi.startHAProxy(serverId)
      toast.success(t('haproxy_configs.haproxy_started'))
      await fetchServersStatus()
    } catch (err: any) {
      const msg = err?.response?.data?.detail || t('haproxy_configs.haproxy_start_error')
      toast.error(msg)
    } finally { setStartingServerId(null) }
  }
  ...
  const handleStartAllStopped = async () => {
    ...
    if (fail === 0) toast.success(t('haproxy_configs.haproxy_start_bulk_success', { count: ok }))
    else toast.warning(t('haproxy_configs.haproxy_start_bulk_partial', { ok, fail }))
    await fetchServersStatus()
