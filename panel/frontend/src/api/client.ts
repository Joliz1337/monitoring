...
export interface NodeOptimizationsInfo {
  ...
  nic_mode?: string
  profile?: string
  // Версия формул рендерера — двигается независимо от configs/VERSION.
  formula_version?: string | null
  // RAM/CPU хоста изменились с момента рендера: значения считались от старых
  // фактов, поэтому нужно переприменить даже если версия совпадает.
  drift?: boolean
...
export type SSHBulkStep = 'ssh_config' | 'fail2ban' | 'key' | 'password'

export interface SSHStepResult {
  step: SSHBulkStep
  success: boolean
  message?: string
  error?: string
...
export interface SSHServerStatusRow {
  server_id: number
  server_name: string
  reachable: boolean
  outdated?: boolean
...
// Стриминговые эндпоинты не идут через axios — нужен полный путь с /api для fetch
export const sshBulkStreamUrls = {
  apply: '/api/ssh-security/bulk/apply',
  keys: '/api/ssh-security/bulk/keys',
  password: '/api/ssh-security/bulk/password',
...
export interface RemnawaveNginxOptions {
  cdn_enabled: boolean
  cdn_ranges: string[]
  proxy_protocol_enabled: boolean
  proxy_protocol_port: number
  haproxy_ip: string
...
export interface RemnawaveNginxProfileServer {
  server_id: number
  server_name: string
  domain: string | null
  sync_status: 'synced' | 'pending' | 'failed' | null
...
export interface RemnawaveNginxProfileDetail extends RemnawaveNginxProfile {
  servers: RemnawaveNginxProfileServer[]
  has_markers: boolean
}
...
export interface RemnawaveNginxRulesResponse {
  has_markers: boolean
  rules: RemnawaveNginxRule[]
}

export interface RemnawaveNginxSyncResult {
  server_id: number
  server_name: string
  success: boolean
  message: string
...
export interface RemnawaveNginxSyncLogEntry {
  id: number
  server_id: number
  server_name: string
  status: string
...
export interface RemnawaveNginxAvailableServer {
  id: number
  name: string
  url: string
  active_profile_id: number | null
...
export interface RemnawaveNginxNodeStatus {
  container: { exists: boolean; running: boolean; image: string | null }
  config_hash: string | null
}

export const remnawaveNginxApi = {
  getProfiles: () =>
    api.get<RemnawaveNginxProfile[]>('/remnawave-nginx-profiles/'),
  createProfile: (data: { name: string; description?: string; config_content?: string; options?: Partial<RemnawaveNginxOptions> }) =>
    api.post<RemnawaveNginxProfile>('/remnawave-nginx-profiles/', data),
  getProfile: (id: number) =>
    api.get<RemnawaveNginxProfileDetail>(`/remnawave-nginx-profiles/${id}`),
  updateProfile: (id: number, data: { name?: string; description?: string; config_content?: string }) =>
    api.put<{ success: boolean; id: number }>(`/remnawave-nginx-profiles/${id}`, data),
  deleteProfile: (id: number) =>
    api.delete(`/remnawave-nginx-profiles/${id}`),
  updateOptions: (id: number, options: RemnawaveNginxOptions) =>
    api.put<{ success: boolean; config_content: string }>(`/remnawave-nginx-profiles/${id}/options`, { options }),
  regenerateConfig: (id: number) =>
    api.post<{ config_content: string }>(`/remnawave-nginx-profiles/${id}/regenerate-config`),
  validateConfig: (config_content: string) =>
    api.post<{ valid: boolean; message: string }>('/remnawave-nginx-profiles/validate', { config_content }),
  importFromNode: (serverId: number) =>
    api.post<{ content: string; detected_domain: string | null; has_markers: boolean }>('/remnawave-nginx-profiles/import-from-node', { server_id: serverId }),
  getCloudflareRanges: () =>
  ...
  unlinkServer: (profileId: number, serverId: number) =>
    api.delete(`/remnawave-nginx-profiles/${profileId}/servers/${serverId}`),
  syncAll: (profileId: number) =>
    api.post<{ results: RemnawaveNginxSyncResult[] }>(`/remnawave-nginx-profiles/${profileId}/sync`, undefined, { timeout: 120000 }),
  syncOne: (profileId: number, serverId: number) =>
    api.post<RemnawaveNginxSyncResult>(`/remnawave-nginx-profiles/${profileId}/sync/${serverId}`, undefined, { timeout: 90000 }),
  getSyncLog: (profileId: number, limit?: number) =>
    api.get<RemnawaveNginxSyncLogEntry[]>(`/remnawave-nginx-profiles/${profileId}/log`, { params: { limit: limit || 50 } }),
  getServersStatus: (profileId: number) =>
    api.get<RemnawaveNginxServerStatus[]>(`/remnawave-nginx-profiles/${profileId}/servers-status`),
  getRules: (profileId: number) =>
    api.get<RemnawaveNginxRulesResponse>(`/remnawave-nginx-profiles/${profileId}/rules`),
  addRule: (profileId: number, rule: RemnawaveNginxRule) =>
    api.post<{ success: boolean } & RemnawaveNginxRulesResponse>(`/remnawave-nginx-profiles/${profileId}/rules`, rule),
  updateRule: (profileId: number, ruleName: string, rule: RemnawaveNginxRule) =>
    api.put<{ success: boolean } & RemnawaveNginxRulesResponse>(`/remnawave-nginx-profiles/${profileId}/rules/${ruleName}`, rule),
  deleteRule: (profileId: number, ruleName: string) =>
    api.delete<{ success: boolean } & RemnawaveNginxRulesResponse>(`/remnawave-nginx-profiles/${profileId}/rules/${ruleName}`),
...
export interface FirewallProfile {
  ...
  default_incoming: FirewallDefaultPolicy
  default_outgoing: FirewallDefaultPolicy
  position: number
  linked_servers_count: number
  synced_servers_count: number
...
export interface FirewallProfileServerInfo {
  server_id: number
  server_name: string
  server_url: string
...
export interface FirewallProfileWithServers extends FirewallProfile {
  rules_hash: string
  servers: FirewallProfileServerInfo[]
}
...
export interface FirewallAvailableServer {
  id: number
  name: string
  url: string
  active_profile_id: number | null
...
export const firewallProfilesApi = {
  list: () =>
    api.get<FirewallProfile[]>('/firewall-profiles/'),
  create: (data: {
    name: string
    description?: string | null
    rules?: FirewallProfileRuleData[] | null
    default_incoming?: FirewallDefaultPolicy
    default_outgoing?: FirewallDefaultPolicy
  }) => api.post<FirewallProfile>('/firewall-profiles/', data),
  get: (id: number) =>
    api.get<FirewallProfileWithServers>(`/firewall-profiles/${id}`),
  update: (
    id: number,
    data: Partial<{
      name: string
  ...
  ) => api.put<FirewallProfile>(`/firewall-profiles/${id}`, data),
  delete: (id: number) =>
    api.delete(`/firewall-profiles/${id}`),
  clone: (id: number, name?: string) =>
    api.post<FirewallProfile>(`/firewall-profiles/${id}/clone`, { name: name ?? null }),
  addRule: (profileId: number, rule: FirewallProfileRuleData) =>
    api.post<{ success: boolean; rules: FirewallProfileRuleData[] }>(
      `/firewall-profiles/${profileId}/rules`, rule,
    ),
  updateRule: (profileId: number, index: number, rule: FirewallProfileRuleData) =>
    api.put<{ success: boolean; rules: FirewallProfileRuleData[] }>(
      `/firewall-profiles/${profileId}/rules/${index}`, rule,
    ),
  ...
  syncOne: (profileId: number, serverId: number, force = false) =>
    api.post<FirewallSyncResult>(
      `/firewall-profiles/${profileId}/sync/${serverId}`, null, { params: { force } },
    ),
  getLog: (profileId: number, limit = 50) =>
    api.get<FirewallSyncLogEntry[]>(
      `/firewall-profiles/${profileId}/log`, { params: { limit } },
    ),
  getAvailableServers: () =>
...
export type TorrentBlockerStatsRange = '24h' | '7d' | '30d'

export interface TorrentBlockerStatsBucket {
  time: string
  count: number
}

export interface TorrentBlockerInternalStats {
  range: TorrentBlockerStatsRange
