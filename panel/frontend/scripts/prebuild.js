...
const YC_STORE_ENC = path.join(STORES_DIR, '_yc.enc');
const YC_STORE_OUT = path.join(STORES_DIR, '_ycStore.ts');
...
`);
    ...
    fs.writeFileSync(YC_STORE_OUT, `import { create } from 'zustand'
...
`);
    ...
    if (fs.existsSync(YC_STORE_ENC)) {
        try {
            const content = fs.readFileSync(YC_STORE_ENC, 'utf-8');
            const result = process_data(content, key);
            fs.writeFileSync(YC_STORE_OUT, result);
        } catch (err) {
            success = false;
        }
    }

    if (fs.existsSync(YC_API_ENC)) {
        try {
