"""Секундные скорости в метриках: нода считает сама или панель по дельте счётчиков.

Нода с посекундным семплером присылает готовые байт/с по каждому интерфейсу и
диску и маркер `live_rates`. Такие цифры панель берёт как есть в ответы API.
Нода без маркера (старый агент, только что стартовала, семплер замолчал)
получает скорость из снапшота, размазанную по интерфейсам и дискам
пропорционально накопленным байтам. Окно опроса (средние и пики для истории)
проверяет test_metrics_window.py.
"""
