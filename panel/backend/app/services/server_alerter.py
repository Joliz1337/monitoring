...
class ServerAlertState:
    ...
    def __init__(self):
        ...
        # Anti-DDoS: when emergency mode was first observed active, and how many
        # off->on transitions we have seen (for flap detection).
        self.antiddos_on_since: float = 0.0
        self.antiddos_transitions: list = []
...
class ServerAlerter:
    ...
    # Anti-DDoS
    # ------------------------------------------------------------------
    def _msg_antiddos(self, srv: Server, settings: AlertSettings, kind: str, detail: str) -> str:
        ru = self._lang(settings) == "ru"
        if kind == "stuck":
            return (
                f"Антиddos на {srv.name} в аварийном режиме больше 30 минут — {detail}"
                if ru else
                f"Anti-DDoS on {srv.name} has been in emergency mode for over 30 minutes — {detail}"
            )
        if kind == "flap":
            return (
                f"Антиddos на {srv.name} срабатывает слишком часто — {detail}. "
                f"Похоже, порог занижен для этого сервера"
                if ru else
                f"Anti-DDoS on {srv.name} is triggering repeatedly — {detail}. "
                f"The threshold is probably too low for this host"
            )
        return (
            f"Conntrack на {srv.name} заполнен на {detail}"
            if ru else
            f"Conntrack table on {srv.name} is {detail} full"
        )

    async def _check_antiddos(
        self,
        srv: Server,
        state: ServerAlertState,
        settings: AlertSettings,
        ...
        """Alert on anti-DDoS state that needs a human, not on every engagement.

        The node's own manager already sends a Telegram message when emergency
        mode engages. What it cannot see is duration and repetition: a chain that
        has been up for half an hour is either a real sustained attack or a
        threshold set too low, and both need someone to look. Routing this
        through the alerter also gives it the shared cooldown/dedupe that the
        direct notification never had.
        """
        ad = metrics.get("antiddos") or {}
        if not ad:
            return

        cooldown = settings.alert_cooldown or 1800
        mode = str(ad.get("mode") or "off")
        source = str(ad.get("source") or "none")

        if mode == "on":
            if not state.antiddos_on_since:
                state.antiddos_on_since = now
                if source == "auto":
                    state.antiddos_transitions.append(now)
            active_for = now - state.antiddos_on_since
            if active_for > 1800 and self._cooldown_ok(state, "antiddos_stuck", now, cooldown):
                state.last_alert["antiddos_stuck"] = now
                detail = f"{int(active_for // 60)} min (source: {source})"
                await self._send_and_save(
                    srv, settings, "antiddos_stuck", "critical",
                    self._msg_antiddos(srv, settings, "stuck", detail),
                    {"active_seconds": int(active_for), "source": source},
                )
        else:
            state.antiddos_on_since = 0.0

        # Flapping: three or more auto engagements within an hour means the
        # thresholds do not fit this host's normal traffic.
        cutoff = now - 3600
        state.antiddos_transitions = [t for t in state.antiddos_transitions if t >= cutoff]
        if len(state.antiddos_transitions) >= 3 and self._cooldown_ok(
            state, "antiddos_flap", now, cooldown
        ):
            state.last_alert["antiddos_flap"] = now
            count = len(state.antiddos_transitions)
            await self._send_and_save(
                srv, settings, "antiddos_flap", "warning",
                self._msg_antiddos(srv, settings, "flap", f"{count}x/hour"),
                {"transitions_per_hour": count},
            )
        ...
        if settings.conntrack_enabled and not conntrack_ignored and isinstance(
            ...
            await self._send_and_save(
                srv, settings, "conntrack_high", "warning",
                self._msg_antiddos(srv, settings, "conntrack", f"{fill:.0f}%"),
                {
                    "fill_pct": fill,
