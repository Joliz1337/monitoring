...
# Пачка на ноде: столько проверок уходит одним заданием. Порты пачки заняты, пока
# не закончится последняя проверка в ней, поэтому большая пачка тормозит сама
# себя: одна упёршаяся в таймаут держит остальные. Восемь — компромисс между
# числом процессов ядра (пул портов / размер пачки) и этой задержкой.
NODE_BATCH_SIZE = 8
