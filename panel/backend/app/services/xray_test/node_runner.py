...
NODE_BATCH_SIZE = 1
# Сколько проверок вешать на одно ядро процессора ноды. Считать по «проверка
# почти всё время ждёт сеть» нельзя: каждая — это ещё и запуск процесса ядра с
# разбором конфига и инициализацией криптографии, а такой оборот раздувает
# очередь планировщика сильнее, чем сама работа.
#
# Замер на боевой ноде уже без лишней уборки внутри исполнителя: 8 ядер,
# haproxy с транзитом занимает ~1.4 ядра, 64 одновременные проверки дают
# load average 10.5 — то есть очередь. Четыре на ядро оставляют этой ноде 32
# проверки и примерно половину машины под её основную работу.
