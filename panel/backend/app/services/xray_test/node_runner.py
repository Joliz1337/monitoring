...
# ждёт сеть — процессор ей нужен на рукопожатие, и то недолго, — поэтому число
# больше числа ядер. Ограничение нужно не ради скорости, а ради транзита: нода
# делит процессор с трафиком пользователей, и слабую нельзя занимать целиком.
# Восьмиядерная и выше упирается в пул портов, ограничение работает для мелких.
NODE_CHECKS_PER_CORE = 8
