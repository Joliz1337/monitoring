...
# Проверок в одном задании. Компромисс между двумя расходами.
#
# Запуск ядра стоит дорого: xray — большая программа на Go, и каждый старт это
# отображение бинарника, инициализация рантайма и потоков, то есть десятки
# миллисекунд времени ядра ОС. При задании из одной проверки таких стартов
# двадцать в секунду, и разбор /proc/stat на боевой ноде показывал `system` 30%
# при фоне 14% — больше ядра процессора уходило на одни только запуски.
#
# Обратная плата: порты задания заняты, пока не закончится последняя проверка в
# нём, поэтому застрявшая на таймауте задерживает соседей по заданию. Четыре —
# вчетверо меньше стартов при задержке максимум от трёх соседей.
#
# Прежний вывод «пачки медленнее» сделан по замерам до исправления зависания в
# исполнителе: тогда любое задание висело до таймаута в 90 секунд независимо от
# размера пачки, и сравнивать было нечего.
NODE_BATCH_SIZE = 4
# Сколько проверок вешать на одно ядро процессора ноды. Ограничение бережёт
# транзит: нода делит процессор с трафиком пользователей.
#
# Четыре — единственное значение, подтверждённое наблюдением на боевой ноде:
# при нём восьмиядерная получает 32 проверки, прогон из 200 укладывается в
# 46 секунд и нагрузка остаётся рабочей. При 16 (то есть 64 проверках) все
# восемь ядер уходили в полку. Синтетические замеры здесь бесполезны: они
# упираются в защиту сервера от всплеска подключений раньше, чем в процессор,
# и показывают спокойный load при нулевом успехе.
...
def _node_capacity(server: Server) -> int:
    ...
    # Потолок — сам пул: каждая одновременная проверка занимает один порт
    # независимо от того, по сколько их сложено в задание.
    ports = len(PORT_POOL)
    if cores <= 0:
