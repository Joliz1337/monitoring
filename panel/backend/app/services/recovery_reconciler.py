...
from app.services.remnawave_nginx_sync import (
    ...
    sync_profile_to_servers as sync_remnawave_nginx_profile,
)
...
async def _reconcile_remnawave_nginx(server_id: int) -> str:
    async with async_session() as db:
        server = await db.get(Server, server_id)
        ...
        rendered, render_error = render_profile_for_server(profile.config_content, server)
        if render_error:
            return "no_domain"

        install_path = await get_remnawave_nginx_path(db)
        data = await _node_get(server, f"/api/remnawave/nginx/config?path={install_path}")
        if data is None or data.get("content") is None:
            return "node_unreachable"

        # Эталон — хэш файла, записанного самой нодой (он включает подставленные
        # ею лимиты). Пока его нет (нода ни разу не синхронизировалась после
        # обновления панели), сравниваем с отправленным контентом: расхождение
        # даст один лишний ресинк, который и проставит эталон.
        if server.remnawave_nginx_node_hash:
            if data.get("hash") == server.remnawave_nginx_node_hash:
                return "in_sync"
        elif compute_config_hash(_normalize_config(data["content"])) == \
                compute_config_hash(_normalize_config(rendered)):
            return "in_sync"

        results = await sync_remnawave_nginx_profile(profile, db, server_ids=[server_id])
        ok = bool(results) and results[0].success
        return "reapplied" if ok else "reapply_failed"


async def _reconcile_dnat(server_id: int) -> str:
    """Нода сама возвращает DNAT-правила из своего файла состояния; здесь ловится
    случай, когда файл потерян (пересозданный volume) или правила менялись, пока
    нода лежала."""
    async with async_session() as db:
        server = await db.get(Server, server_id)
        ...
        results = await sync_dnat_profile(profile, db, server_ids=[server_id])
        ok = bool(results) and results[0].success
        return "reapplied" if ok else "reapply_failed"
...
async def reconcile_recovered_server(server_id: int, semaphore: asyncio.Semaphore) -> RecoveryReport:
    ...
    async with semaphore:
        ...
        report.dnat = await _reconcile_dnat(server_id)
        ...
        if report.dnat in RECONCILED_STATES:
            settled.append(KIND_DNAT_PROFILE)
        await clear_pending_sync(server_id, settled, started_at)

    logger.info(
        "recovery_reconcile_done server=%s(%s) firewall=%s haproxy_cfg=%s haproxy_run=%s remnawave_nginx=%s blocklist=%s dnat=%s",
        server_id, server_name, report.firewall, report.haproxy_cfg, report.haproxy_run,
        report.remnawave_nginx, report.blocklist, report.dnat,
    )
    return report
