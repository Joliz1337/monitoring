...
class RemnawaveAPI:
    ...
    async def get_all_users(self, start: int = 0, size: int = 50) -> dict:
        """
    ...
    async def get_all_users_paginated(self, size: int = 50) -> list[dict]:
        """
        Get ALL users through pagination.
        ...
        while True:
            result = await self.get_all_users(start=start, size=size)
        ...
        logger.debug(f"Fetched {len(all_users)} users from Remnawave")
        return all_users
