...
async def lookup_ips(ip_list: list[str]) -> dict[str, ASNInfo]:
    ...
    for row in cached:
        result[row.ip] = ASNInfo(asn=row.asn, prefix=row.prefix, holder=row.holder)
        if row.prefix:
            known_prefixes[row.prefix] = ASNInfo(asn=row.asn, prefix=row.prefix, holder=row.holder)
    ...
    if cidr_matched:
        async with async_session() as db:
            for ip, info in cidr_matched:
                db.add(ASNCache(
                    ip=ip, asn=info.asn, prefix=info.prefix, holder=info.holder,
    ...
    if fetched:
        async with async_session() as db:
            ...
            for ip, info in fetched:
                db.add(ASNCache(ip=ip, asn=info.asn, prefix=info.prefix, holder=info.holder, cached_at=now))
            try:
                await db.commit()
            except Exception:
                await db.rollback()
...
def group_ips_by_asn_with_visits(
    ...
    for ip, info in asn_map.items():
        ...
        if info.holder and not groups[key].get("holder"):
            groups[key]["holder"] = info.holder
...
IP_API_FIELDS = "query,as,isp"


async def enrich_with_names(asn_map: dict[str, ASNInfo]) -> None:
    """Enrich ASNInfo entries with ISP holder names via ip-api.com batch.

    Mutates asn_map in-place. Picks one representative IP per unknown-holder ASN,
    does a single batch request, then propagates holder to all IPs sharing that ASN.
    Also updates DB cache with discovered holders.
    """
    # Собираем ASN без holder — по одному IP на ASN для batch-запроса
    need_holder: dict[str, str] = {}  # asn -> representative IP
    for ip, info in asn_map.items():
        if info.asn and not info.holder and info.asn not in need_holder:
            need_holder[info.asn] = ip

    if not need_holder:
        return

    ips_to_query = list(need_holder.values())
    holder_by_asn: dict[str, str] = {}

    try:
        payload = [{"query": ip, "fields": IP_API_FIELDS} for ip in ips_to_query[:100]]
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    logger.debug(f"ip-api.com batch returned {resp.status}")
                    return
                data = await resp.json()
        ...
        for item in data:
            ...
            # as_field выглядит как "AS8359 MTS PJSC" — берём номер
            asn_num = as_field.split()[0].lstrip("AS") if as_field else None
            holder_name = isp or (as_field.split(" ", 1)[1] if " " in as_field else "")

            if asn_num and holder_name:
                holder_by_asn[asn_num] = holder_name

    except Exception as e:
        logger.debug(f"ip-api.com batch error: {e}")
        return

    if not holder_by_asn:
        return

    # Проставляем holder во все записи asn_map
    updated_ips: list[tuple[str, str]] = []  # (ip, holder) для обновления в БД
    for ip, info in asn_map.items():
        if info.asn and not info.holder:
            holder = holder_by_asn.get(info.asn)
            if holder:
                info.holder = holder
    ...
    if updated_ips:
        try:
            async with async_session() as db:
                ...
                for ip, holder in updated_ips:
                    await db.execute(
                        update(ASNCache).where(ASNCache.ip == ip).values(holder=holder)
                    )
                await db.commit()
        except Exception as e:
            logger.debug(f"ASN holder DB update failed: {e}")
