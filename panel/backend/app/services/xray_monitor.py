...
PANEL_SPEED_THRESHOLD_MBPS = 1000
...
async def _run_panel_speedtest() -> dict:
    """Спидтест панели напрямую (без VPN) для проверки канала."""
    ...
    try:
        ...
        try:
            ...
            pass
        return {"ok": False, "error": "panel speedtest timeout"}
    except FileNotFoundError as e:
        return {"ok": False, "error": f"binary not found: {e.filename}"}
    except Exception as e:
        return {"ok": False, "error": str(e)[:200]}
    ...
    return {"ok": True, "download_mbps": download_mbps, "upload_mbps": upload_mbps}
...
class XrayMonitorService:
    ...
    async def _check_all(self, settings: XrayMonitorSettings):
        ...
        panel_result = await _run_panel_speedtest()
        if not panel_result["ok"]:
            logger.warning(f"Panel speedtest failed, skipping cycle: {panel_result.get('error')}")
            return

        panel_dl = panel_result["download_mbps"]
        panel_ul = panel_result["upload_mbps"]
        logger.info(f"Panel speedtest: ↓{panel_dl} Mbit/s  ↑{panel_ul} Mbit/s")

        if panel_dl < PANEL_SPEED_THRESHOLD_MBPS or panel_ul < PANEL_SPEED_THRESHOLD_MBPS:
            self._panel_slow_streak += 1
            threshold = settings.fail_threshold or 2
            logger.warning(
                f"Panel channel insufficient (↓{panel_dl} / ↑{panel_ul} Mbit/s, "
                f"threshold {PANEL_SPEED_THRESHOLD_MBPS}), streak {self._panel_slow_streak}/{threshold}"
            )
            if self._panel_slow_streak == threshold:
                await self._notify_panel_slow(settings, panel_dl, panel_ul)
            return

        self._panel_slow_streak = 0
    ...
    # ------------------------------------------------------------------ telegram
    async def _notify_panel_slow(
        self, settings: XrayMonitorSettings, dl_mbps: float, ul_mbps: float,
    ):
        msg = (
            "\u26a0\ufe0f <b>Xray Monitor — канал панели низкий</b>\n\n"
            f"↓ {dl_mbps} Mbit/s  /  ↑ {ul_mbps} Mbit/s\n"
            f"Порог: {PANEL_SPEED_THRESHOLD_MBPS} Mbit/s\n\n"
            "Тест серверов пропущен."
        )
        await self._send_telegram(settings, msg)
    ...
    async def _send_telegram(self, settings: XrayMonitorSettings, message: str):
        ...
        await get_telegram_bot_service().send_message(bot_token, chat_id, message)
