...
REASON_LABELS = {
    "switched": "у нового выхода больше пройденных проверок",
    "unknown": "у нового выхода больше пройденных проверок, часть проверить не удалось",
...
def alert_text(kind: str, server_name: str, from_value: Optional[str], to_value: Optional[str], reason: str) -> str:
    ...
    if kind == KIND_NO_HEALTHY:
        return (
            ...
            f"Нода <b>{name}</b>: ни один кандидат не прошёл все проверки. "
            f"Трафик идёт через <b>{dst}</b> — у него больше всего пройденных."
        )
