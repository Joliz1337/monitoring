...
class BanTarget:
    ...
    remnawave_block: Optional[dict] = None
    ...
    def _extract_ban_targets(records: list[dict]) -> list[BanTarget]:
        ...
        for record in records:
            ...
            targets.append(BanTarget(
                ...
                detection={
                    "protocol": xray.get("protocol") or None,
                    "network": xray.get("network") or None,
                    "source": xray.get("source") or None,
                    "destination": xray.get("destination") or None,
                    "inbound_tag": xray.get("inboundTag") or None,
                    "inbound_name": xray.get("inboundName") or None,
                    "outbound_tag": xray.get("outboundTag") or None,
                    "detected_at": action.get("processedAt") or None,
                },
                remnawave_block={
                    "blocked": action.get("blocked"),
                    "block_duration_seconds": action.get("blockDuration"),
                    "will_unblock_at": action.get("willUnblockAt") or None,
                },
            ))
        return targets
    ...
    async def _send_webhooks(
        ...
        async def _send_one(target: BanTarget) -> bool:
            payload = {
                ...
                "remnawave_block": target.remnawave_block,
    ...
    async def send_test_webhook(url: str, secret: Optional[str]) -> tuple[bool, str]:
        ...
        payload = {
            ...
            "remnawave_block": {
                ...
                "will_unblock_at": (now + timedelta(seconds=600)).isoformat(),
            },
