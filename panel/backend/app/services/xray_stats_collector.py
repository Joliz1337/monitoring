    ...
    async def _check_anomalies(self, settings: RemnawaveSettings):
        ...
        for email, ip_count in ip_rows:
            ...
            lines = [
                ...
                f"\U0001F464 <code>{cached.username or f'#{email}'}</code> (ID: <code>{email}</code>)",
            ...
            if ua_check_on and user_agent and not ua_pattern.search(user_agent):
                ...
                if not last or (now - last).total_seconds() >= COOLDOWN_SECONDS:
                    ...
                    await get_telegram_bot_service().send_message(
                        ...
                        f"\U0001F464 <code>{cached.username or f'#{cached.email}'}</code> (ID: <code>{cached.email}</code>)\n"
            ...
            if delta >= threshold_bytes:
                ...
                if streak >= confirm_count:
                    ...
                    await get_telegram_bot_service().send_message(
                        ...
                        f"\U0001f464 <code>{uname or f'#{email}'}</code> (ID: <code>{email}</code>)\n"
