...
def _profile_url(profile: str, filename: str) -> str:
    """Build GitHub URL for a profile-specific config file."""
    return f"{GITHUB_CONFIGS_BASE}/{profile}/{filename}"
...
async def get_optimizations_from_github(profile: str = "vpn") -> dict:
    """Fetch optimization configs for a profile and shared NIC scripts from GitHub."""
    ...
    try:
        ...
        responses = await asyncio.gather(
            ...
            client.get(_profile_url(profile, "sysctl.conf"), timeout=15.0),
            client.get(_profile_url(profile, "limits.conf"), timeout=15.0),
            client.get(_profile_url(profile, "systemd-limits.conf"), timeout=15.0),
        ...
        keys = [
            ...
            ("sysctl_content", f"{profile}/sysctl.conf"),
            ("limits_content", f"{profile}/limits.conf"),
            ("systemd_content", f"{profile}/systemd-limits.conf"),
