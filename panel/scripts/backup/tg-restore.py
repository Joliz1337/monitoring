#!/usr/bin/env python3
"""Собрать тома бэкапа панели из Telegram и расшифровать в .dump — без панели.

Обычный путь восстановления — сама панель: в окне «Загрузить бэкап» выбираются
все тома набора (`panel-backup_<дата>.enc.001`, `.002`, …) и вводится пароль
архива. Этот скрипт нужен, только когда `.dump` требуется получить вне панели:
он собирает тома по порядку, расшифровывает и пишет обычный дамп, который
потом загружается в панель как обычный бэкап (без пароля).
...
def main() -> None:
    ...
    print("Загрузите его в панели как обычный бэкап: Настройки → Бэкапы → Загрузить бэкап.")
