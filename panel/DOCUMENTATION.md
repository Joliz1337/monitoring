...
**XHTTP-правило разводит трафик по Content-Type.** Режимы XHTTP ведут себя на проводе по-разному, и одним `pass` их не обслужить: `stream-one` и аплоад `stream-up` приходят с `Content-Type: application/grpc` (Xray ставит его сам, чтобы прокси и CDN пропускали поток) и требуют полного дуплекса — их берёт `grpc_pass`; `packet-up` и даунлоад-стримы ходят обычными GET/POST, и им нужен `proxy_pass` с `proxy_request_buffering off` + `proxy_buffering off` (с буферизацией аплоад копится в nginx, а даунлоад встаёт). Разводка сделана через `error_page {NOT_GRPC_CODE} = @xhttp_{имя};` и однострочный `if` с `return` — второй `if` с другим `pass` в location недопустим. Обе ветки перехватывают ошибки апстрима (`grpc_intercept_errors`/`proxy_intercept_errors`) и уводят их вместе с собственными кодами nginx на `@fallback`: Xray на неверный запрос отдаёт голый 400/404 от Go, по которому угаданный путь отличается от страницы сайта. Без `fallback_url` те же коды уходят в собственную `@xhttp_{имя}_drop` с `return 444` — на серверную `@drop` правило ссылаться не может (её нет в чужом импортированном конфиге), а собственный `error_page` в location отменяет наследование серверного. `large_client_header_buffers 8 32k` в http-контексте — `packet-up` умеет нести данные в заголовке запроса, и на больших `uplinkChunkSize` дефолтных буферов не хватает.
...
- `panel/backend/app/services/remnawave_nginx_config.py` — генератор/парсер: `GrpcRule`, `XhttpRule`, `ProxyRule` (единый список `Rule`), `ProfileOptions`, `generate_full_config`, `parse_rules_from_config`, `splice_rules`, `render_for_server`, `detect_domain`/`replace_domain_with_placeholder`, `validate_rule`/`validate_rules`/`validate_options`, `rule_location_path`, `CLOUDFLARE_RANGES`
- `panel/backend/tests/test_remnawave_nginx_config.py` — 43 unittest-теста: round-trip generate↔parse для всех схем (включая fallback) и сохранение порядка правил, сохранение ручных правок при splice, контент gRPC/XHTTP/CDN/PROXY protocol/fallback, error_page и локации fallback, рендер домена, валидации (в т.ч. `0.0.0.0/0`/`::/0` в CDN-диапазонах допустимы, PP без IP → `0.0.0.0/0`, конфликт fallback с proxy-правилом `path="/"`, некорректный fallback URL, XHTTP-путь обязан быть абсолютным и длиннее `/`, дубли имён и путей), три host-специфичные строки помечены `# auto: node`, `client_max_body_size 0` и `large_client_header_buffers`, HTTP/1.1 + WebSocket-заголовки в proxy/fallback-локациях, ответ без `add_header` и с `proxy_pass_header Server`, обрыв соединения при мёртвой заглушке и при собственных кодах ошибок nginx (включая HTTP-запрос на 443 у `reject_default_server`), ошибки заглушки не перехватываются, а ошибки Xray в XHTTP-локации — наоборот, правила без fallback не ссылаются на `@drop` (у XHTTP для этого своя `@xhttp_{имя}_drop`), не-gRPC запрос по gRPC-пути уходит на fallback (код 418) либо обрывается без него, XHTTP-ветка packet-up идёт без буферизации, `@drop` присутствует всегда, когда на неё есть ссылка
