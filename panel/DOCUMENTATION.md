...
- **Анти-DDoS** — многослойная защита от DDoS-атак на нодах: дежурный режим без лимитов, аварийный режим (iptables-цепочка `ANTIDDOS`: SYNPROXY + connlimit + hashlimit) включаемый вручную или автоматически локальным watchdog по сигналам из `/proc`, whitelist на ipset с ежечасной автосинхронизацией; страница с управлением по нодам, редактированием ручного whitelist (CDN-подсети) и установкой watchdog одной кнопкой
...
1. **Whitelist push** — раз в час (настраивается) собирает whitelist из авто-части (IP всех активных нод + IP панели, резолвится из домена) и ручной части (`user_cidrs` из настроек) и рассылает его на каждую ноду через `POST /api/antiddos/whitelist/sync`.
2. **Status poll** — раз в 60 сек (настраивается) опрашивает `GET /api/antiddos/status` на каждой ноде, пишет результат в `Server.antiddos_*` и при переходе ноды в аварийный режим **автоматически** (`source=auto`, то есть сработал watchdog, а не оператор) отправляет Telegram-алерт (`alert_type="antiddos_emergency"`) и запись в `AlertHistory`. Этот же цикл выполняет **zero-touch раскатку watchdog** (`_maybe_auto_install`) — см. ниже. Оба цикла ограничены `asyncio.Semaphore` (`DB_CONCURRENCY=10`, `HTTP_CONCURRENCY=50`) — масштабируются на сотни нод без роста числа соединений, по образцу `blocklist_manager`. Оба цикла работают, только когда `AntiDdosSettings.enabled=True` — при выключенном мастер-переключателе они простаивают (см. ниже, чем это отличается от полного стенд-дауна парка).

Менеджер регистрируется в `lifespan` панели (`start_antiddos_manager()` / `stop_antiddos_manager()`).

**Мастер-переключатель (`AntiDdosSettings.enabled`):** `PUT /antiddos/settings` при реальном изменении `enabled` запускает в фоне (`asyncio.create_task`, не блокируя ответ клиенту) `manager.apply_master_state(enabled)`:
- `enabled=True` → `set_watchdog_all(True)` — включает автодетект на всех активных нодах.
- `enabled=False` → `set_emergency_all(False)`, затем `set_watchdog_all(False)` — снимает аварийный режим и выключает автодетект на всех активных нодах; весь парк возвращается в дежурный режим.

Раньше выключение `enabled` только останавливало фоновые циклы панели (whitelist push/status poll) — watchdog на нодах продолжал работать самостоятельно и мог сам включить аварийный режим. Теперь мастер-переключатель — это реальный стенд-даун парка, а не только пауза наблюдения панели. Новые методы `AntiDdosManager`: `set_watchdog_all(enabled)` (fan-out `set_node_watchdog` под семафором `HTTP_CONCURRENCY`), `apply_master_state(enabled)`.

**Zero-touch автоустановка/обновление watchdog (`_maybe_auto_install`, вызывается из status poll):**
...
| POST | /antiddos/emergency-all | Включить/выключить аварийный режим на всех активных нодах |
| POST | /antiddos/whitelist/push | Форсировать рассылку whitelist прямо сейчас |
| POST | /antiddos/install-all | Установить/обновить watchdog на всех активных нодах |

**Per-node проксируемые эндпоинты (`panel/backend/app/routers/proxy.py`, prefix `/proxy/{server_id}/antiddos`):** `GET /status` (плюс сохраняет результат в БД), `POST /emergency` (ручной пин — watchdog его не снимет), `POST /watchdog`, `POST /install`.
...
- **Управление** — список нод со статусом (дежурный/аварийный, источник, причина, watchdog), точечные и массовые (все ноды) переключатели аварийного режима и watchdog. Кнопки ручной установки watchdog в интерфейсе больше нет — установка полностью автоматическая (zero-touch фоновым опросом, см. выше); эндпоинты `POST /antiddos/install-all` и `POST /proxy/{id}/antiddos/install` остались в API и используются автоустановкой. Версия watchdog-скрипта нодой отдаётся (`GET /api/antiddos/status` → `version`), но панель использует её только внутри `_maybe_auto_install` для сравнения — в UI не выводится.
- **Белый список** — редактирование ручной части whitelist (`user_cidrs`), кнопка форс-пуша на ноды.
...
**Файлы:**
- `panel/backend/app/services/antiddos_manager.py` — `AntiDdosManager`: два фоновых цикла, whitelist-сборка, on-demand действия, алерты
- `panel/backend/app/routers/antiddos.py` — API роутер (prefix `/antiddos`)
- `panel/backend/app/routers/proxy.py` — per-node роуты `/proxy/{id}/antiddos/*`
- `panel/backend/app/models.py` — модель `AntiDdosSettings`, новые поля `Server`
...
- `panel/frontend/src/pages/AntiDdos.tsx` — страница управления
- `panel/frontend/src/api/client.ts` — `antiDdosApi`, типы `AntiDdosSettings`, `AntiDdosStatus`, `NodeAntiDdosState`, поле `Server.antiddos_emergency_mode`
...
- `panel/frontend/src/locales/ru.json`, `en.json` — i18n пространство имён `anti_ddos`
...
- `panel/backend/app/services/torrent_blocker.py` — `BanTarget` dataclass; `_extract_ban_targets`; `_enrich_targets`; `_send_webhooks` (HMAC-SHA256, `asyncio.Semaphore(WEBHOOK_CONCURRENCY)`); `send_test_webhook`; `_poll_cycle`; `_is_node_live(server, cutoff)`; `_send_to_nodes`
