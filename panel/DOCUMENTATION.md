...
`tls-check` опирается на запасной ход REALITY: «неправильному» клиенту живой сервер отдаёт настоящий сайт-маскировку, поэтому TLS-рукопожатие с его SNI обязано пройти даже без валидного ключа. HAProxy делает рукопожатие, шлёт `GET /` и ждёт ответ, подходящий под `TLS_CHECK_EXPECT` (`^HTTP/1\..\ [234][0-9][0-9]` — 2xx/3xx/4xx годятся, 5xx нет). Так отсеивается задушенный или упавший инбаунд за живым портом, которого `tcp-check` не видит.
