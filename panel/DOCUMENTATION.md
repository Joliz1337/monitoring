...
**Размер пачки — компромисс, а не «чем больше, тем лучше».** Порты пачки заняты, пока не закончится последняя проверка в ней: одна упёршаяся в таймаут держит остальные и свой слот. При `NODE_BATCH_SIZE = 16` это съедало выигрыш от объединения — прогон с ноды стал медленнее, чем был с процессом на ячейку, хотя число процессов упало в шестнадцать раз. `NODE_BATCH_SIZE = 8` при пуле в 64 порта даёт 8 процессов ядра на 64 одновременные проверки и ограничивает задержку от «застрявшей» проверки семью соседями.
