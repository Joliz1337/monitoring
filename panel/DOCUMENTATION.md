...
- `ExitProxySettings` (singleton) — `enabled` (общий выключатель цикла), `port`, `check_interval_minutes`, `blocked_countries` (JSON, по умолчанию `["RU"]`), `builtin_checks` (JSON: `google_country`, `google_captcha`, `gemini`), `custom_checks` (JSON, по умолчанию Claude/ChatGPT/Reddit), `telegram_enabled`, `alert_cooldown_seconds`, `last_cycle_at/error`.
