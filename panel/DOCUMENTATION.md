...
**Потолок проверок на ноде считается по её процессору** (`_node_capacity`): `ядра × NODE_CHECKS_PER_CORE`, ограниченное пулом портов снизу `MIN_NODE_CONCURRENCY`, а без метрик — `NODE_FALLBACK_CONCURRENCY`. Число ядер берётся из `last_metrics` сервера, как в авто-весах HAProxy. Ограничение нужно не ради скорости, а ради транзита: нода делит процессор с трафиком пользователей, и слабую нельзя занимать целиком. Сама проверка почти всё время ждёт сеть, процессор ей нужен на рукопожатие, поэтому коэффициент больше единицы — при восьми ядрах нода упирается уже в пул портов, и ограничение реально работает только для мелких. Коэффициент подбирался дважды: сначала по замеру `load average 14`, который, как выяснилось позже, создавала уборка брошенных ядер внутри самого исполнителя, а не проверки; после её исправления число поднято. Выбранный потолок пишется в журнал прогона — там его и надо сверять с фактической нагрузкой.
