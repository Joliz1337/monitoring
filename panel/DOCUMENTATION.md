...
- **Причина отказа.** Лог ядра теперь общий, поэтому строки делятся по тегам: слот — это номер ячейки, теги выглядят как `mon-test-in-<index>`/`mon-test-out-<index>`, отбор в `_lines_for_slot` (на ноде — `core_failure_line <slot>`). Строки без тега считаются общими для всей пачки. Отбор идёт по границе цифр, иначе слот `1` забирал бы строки слота `11`.
