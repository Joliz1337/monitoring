...
**Прогон с ноды** (`node_runner.py`). Своего эндпоинта у ноды нет: используется общий канал `POST /api/system/execute-stream`, гейт — `require_capability(server, Capability.EXEC, write=True)` в роутере. Исполнитель `configs/xray-test-runner.sh` версионируется (`RUNNER_VERSION`) и доставляется панелью base64-командой при расхождении версии — тот же приём, что у сторожа анти-DDoS. Конфиг ядра генерирует панель: единственный генератор на оба места запуска означает, что «работает на панели» и «работает на ноде» проверяют одно и то же. Ядро панель кладёт на ноду **заранее, в `prepare()`** — отдельной командой до начала проверок: иначе параллельные ячейки начинали тянуть один файл одновременно и мешали друг другу. Ссылка живёт по времени (TTL 5 мин), а не до первого скачивания — одноразовость ломала как раз параллельный случай. Нода сверяет файл по SHA-256 — поэтому `curl --insecure` безопасен и способ работает на серверах без доступа к GitHub. Локальные socks-порты `7501-7532` зарезервированы от эфемерной выдачи в `configs/tune-sysctl.sh` — размер пула и есть потолок пачки на ноде. Задание (`RUNNER_VERSION = 2.0.0`) состоит из строк `CORE`, `OPTS`, `CONF` (общий конфиг ядра) и `CELL` на каждую проверку с её socks-портом; исполнитель поднимает ядро один раз и гонит проверки параллельно по `PARALLEL_CELLS = 8`, поэтому ответы приходят вперемешку и сопоставляются по номеру ячейки (`_parse_results`).
