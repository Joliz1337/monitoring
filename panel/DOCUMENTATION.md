...
- `node_live_rates(metrics)` → `NodeRates(net_rx, net_tx, disk_read, disk_write)` или `None`. `_build_snapshot()` при наличии кладёт скорости ноды в `metrics_snapshots` (история на графиках — секундные замеры раз в интервал опроса, а не средние за интервал); `TrafficIngest.observe()` вызывается в любом случае — он ведёт историю байт по кумулятивным счётчикам, это не скорость.
