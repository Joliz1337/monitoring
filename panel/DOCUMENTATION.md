...
2. **Status poll** — раз в 60 сек (настраивается) опрашивает `GET /api/antiddos/status` на каждой ноде, пишет результат в `Server.antiddos_*` и при переходе ноды в аварийный режим **автоматически** (`source=auto`, то есть сработал watchdog, а не оператор) отправляет Telegram-алерт (`alert_type="antiddos_emergency"`) и запись в `AlertHistory`. Оба цикла ограничены `asyncio.Semaphore` (`DB_CONCURRENCY=10`, `HTTP_CONCURRENCY=50`) — масштабируются на сотни нод без роста числа соединений, по образцу `blocklist_manager`.
...
- **Управление** — список нод со статусом (дежурный/аварийный, источник, причина, watchdog), точечные и массовые (все ноды) переключатели аварийного режима и watchdog, кнопка «Установить watchdog на все».
