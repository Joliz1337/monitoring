...
- `per_cpu` (только при `include_per_cpu=true`, сырые периоды) — `{bucket_sec, timestamps[], cores[][]}`: регулярная сетка бакетов 30 с (1h) / 300 с (24h), вдвое шире при более чем 32 ядрах; `cores[i][j]` — среднее ядра `i` в бакете `j`, пропущенный бакет — `null`. Считается отдельным запросом через `jsonb_array_elements_text(per_cpu_percent::jsonb) WITH ORDINALITY` с гардом `LEFT(per_cpu_percent, 1) = '['`.
