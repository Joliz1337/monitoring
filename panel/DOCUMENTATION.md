...
| GET | /api/exit-proxy/snippet | `{outbound_json, rules_json, text}` — outbound `socks 127.0.0.1:<port>` с тегом `exit-proxy` и правила: блок `udp/443` (QUIC мимо socks не пройдёт) и весь Google одним правилом (`geosite:google`, `geosite:google-gemini`, `googleapis.com`, `gstatic.com`, `googleusercontent.com`) |
