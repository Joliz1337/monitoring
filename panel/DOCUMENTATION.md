...
**Потолок проверок на ноде считается по её процессору** (`_node_capacity`): `ядра × NODE_CHECKS_PER_CORE`, ограниченное пулом портов снизу `MIN_NODE_CONCURRENCY`, а без метрик — `NODE_FALLBACK_CONCURRENCY`. Число ядер берётся из `last_metrics` сервера, как в авто-весах HAProxy. Брать весь пул вслепую нельзя: замер на боевой ноде показал `load average 14` при 64 одновременных проверках и продолжал расти — упор к тому моменту был уже не в порты, а в процессор, который нода делит с пользовательским трафиком. От нехватки CPU проверки идут только медленнее, поэтому наращивать параллельность дальше вредно вдвойне. Выбранный потолок пишется в журнал прогона.
