...
**На ноде задание = одна проверка (`NODE_BATCH_SIZE = 1`), и это измеренное решение, а не экономия.** Порты задания заняты, пока не закончится последняя проверка в нём, а на заблокированном сервере проверка упирается в таймаут секунд на тридцать. При пачке в 16, а затем в 8 такая проверка держала пятнадцать (семь) уже отработавших соседей, и прогон с ноды выходил медленнее, чем со своим ядром на каждую проверку, — при том что процессов стало в разы меньше. Замеры на боевых нодах это подтвердили дважды. Сейчас каждая проверка получает своё ядро, свой порт и освобождает его сразу, как закончилась; одновременных заданий столько же, сколько портов в пуле (`_batch_slots`). Пачки как механизм остались и работают на панели, где порт берётся у системы и очередь не упирается в пул.

Задания уходят на ноду разом, а не по очереди (`asyncio.gather` в `probe_batch`): последовательный цикл сводил бы параллельность к одной проверке на рабочего независимо от размера пула.
