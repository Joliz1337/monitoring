...
- **Dashboard** — карточки серверов с drag-and-drop, статус SSL, Load Average; сводная панель флота (суммарные CPU/RAM/скорость сети по всем нодам) над списком, клик по плитке раскрывает график этой метрики по всему парку за 1ч/24ч/7д/30д/год; IP-адрес ноды копируется в буфер обмена кликом ЛКМ; поиск по имени и адресу (см. ниже)
...
- **Сеть / IP-адреса** — карточка на странице сервера: интерфейсы ноды с адресами, добавление доп. IPv4/IPv6 списком, диапазоном или подсетью целиком и удаление добавленных панелью; нода применяет транзакцией с таймером отката, панель подтверждает, заново подключившись к ноде (см. ниже)
...
- **Exit-прокси** — локальный SOCKS5 в агенте выбранных нод с пулом исходящих IP (основной и добавленные панелью IPv4, WARP): нода сама проверяет, как Google видит каждый выход (страна, капча поиска, Gemini, свои URL), держит трафик на здоровом и переключает только при блокировке, обрывая соединения старого выхода; панель — настройки, статус, ручное переключение, журнал и Telegram-уведомления; готовый кусок конфига Xray для Remnawave (см. «Exit-прокси»)
...
│       ├── routers/exit_proxy.py        # Exit-прокси: настройки, ноды со статусом выходов, проверки, сниппет Remnawave, журнал, установка WARP
│       ├── services/exit_proxy/         # Exit-прокси (см. раздел «Exit-прокси»)
│       │   ├── settings.py              # Singleton-настройки, JSON-поля, SettingsSnapshot, порт для reserved-ports
│       │   ├── render.py                # Конфиг для агента ноды, его хэш, кусок конфига Xray для Remnawave
│       │   ├── node_client.py           # HTTP к /api/system/exit-proxy/* ноды, гейт прав (system:write) и версии
│       │   ├── service.py               # Фоновый цикл: доставка конфига по хэшу, сбор статуса, журнал, исполнитель долга
│       │   ├── views.py                 # Статус ноды → строка раздела, новые события ноды
│       │   ├── alerts.py                # Telegram с cooldown по (нода, вид события), запись в AlertHistory
│       │   └── warp_install.py          # Установка WARP через агента (общий HostInstallJobManager)
...
Выключить нельзя `dashboard`, `servers` и `settings` — ядро панели и единственный путь вернуть скрытое; остальные 17 разделов переключаются. Скрытый nav-item опционального модуля настройкой не управляется.
...
Эндпоинт проверки статуса миграции сертификатов читает проекцию трёх колонок (`uses_shared_cert`, `pki_enabled`, `dedicated_cert`) — только те поля, которые реально использует `classify_server`, без тяжёлых TEXT-колонок `Server` (`last_metrics` и др.).
...
| GET | /api/proxy/{id}/network/state | Интерфейсы и адреса ноды, бэкенд конфига, транзакция доп. IP, снимок задачи панели (`job`); старая нода → `supported: false` + `min_node_version` |
| POST | /api/proxy/{id}/network/preview | Разобрать текст адресов без применения: `{count, ipv4, ipv6, addresses}` |
| POST | /api/proxy/{id}/network/apply | Запустить транзакцию доп. IP (`{interface, add_text, remove}`) → снимок задачи; `409` при незавершённой транзакции |
| POST | /api/proxy/{id}/network/rollback | Откатить pending-транзакцию вручную |
...
**Сеть / IP-адреса (дополнительные адреса на интерфейсе ноды):**

Карточка на странице сервера показывает интерфейсы ноды, способные нести адрес (физические карты, bond, VLAN, bridge; слейвы bond'а и docker-мосты не показываются), с адресами и даёт добавить доп. IPv4/IPv6 или удалить добавленные панелью. Основной адрес и адреса хостера — только чтение (иконка замка). Механизм на стороне ноды — транзакция с таймером отката и boot-guard — описан в [node/DOCUMENTATION.md](../node/DOCUMENTATION.md#дополнительные-ip-адреса-extra_ipspy); панель отвечает за грамматику ввода, подтверждение и отображение.

- Гейт по версии: `MIN_NODE_VERSION_NETWORK = 10.29.0` (`Server.node_version`); старая нода или `404` от неё → `GET state` отдаёт `supported: false` с `min_node_version`, карточка просит обновить ноду. Свой шлюз адреса — `MIN_NODE_VERSION_NETWORK_GATEWAY = 10.31.0`: старая нода молча проигнорировала бы поле `gateway`, поэтому `apply` со шлюзом для неё — `400`, а `GET state` отдаёт `min_node_version_gateway`, по которому карточка гасит поле.
- `services/network_addresses.py` — грамматика одного текстового поля (`expand_entries`): `1.2.3.4` → /32, `2001:db8::2` → /128, `1.2.3.4/24` (host-биты ≠ 0) — один адрес с маской, `1.2.3.0/29` (host-биты = 0, prefix ≤ 30) — все адреса подсети с этой маской (сеть и broadcast пропускаются; `/31` — оба), диапазон `1.2.3.10-1.2.3.15` (и сокращённый `…10-15`) — по /32|/128 на номер; подсети IPv6 не разворачиваются (ошибка с подсказкой); разделители — перевод строки, запятая, точка с запятой, пробел; дубли схлопываются, один адрес с двумя масками — ошибка; ≤256 адресов за применение. `UNASSIGNABLE_NETS` (loopback, link-local, multicast, 0/8, 240/4, `::`, `::1`, `fe80::/10`, `ff00::/8`) отвергаются; RFC1918/CGNAT/ULA разрешены — приватная VLAN хостера это обычный доп. адрес на втором интерфейсе. Ошибки по-русски и начинаются с записи-виновника. `preview()` — для «Будет добавлено N адресов» до применения. `parse_gateway()` — необязательный шлюз: IP, кроме loopback, multicast, `0/8`, `240/4`, `::` и `ff00::/8`; link-local разрешён (`fe80::1` у IPv6 обычное дело). `with_gateway()` раздаёт его всей пачке; смешанные семейства при заданном шлюзе и шлюз, совпадающий с добавляемым адресом, — ошибка. `AddressSpec.payload()` отдаёт `gateway`, только если он задан.
- `services/network_transactions.py` — вся цепочка фоновой задачей (nginx панели режет `/api/` на 60 с, а apply на ноде может идти до минуты): `start_apply()` (`asyncio.Lock` на сервер; активная задача, занятый лок или pending/applying-транзакция на ноде → `PendingTransactionError` → `409`) читает состояние ноды, отказывает при конфликте шлюза (`gateway_conflicts` → `GatewayConflictError` → `400`: уже стоящий наш адрес с другим шлюзом или чужой адрес, которому просят шлюз; шлюз основного адреса из `default_gateway` приравнивается к «без шлюза», как на ноде), вычитает уже стоящие адреса (`missing_on_interface`) и оставляет в удалении только `managed` (`managed_on_interface`), резолвит `protected` — IP host'а `Server.url` (`host_to_ip`), это адрес, по которому панель ходит на ноду, — и запускает `_run_job`: `send_apply` (apply-клиент, `APPLY_TIMEOUT_SECONDS = 180`) → при потере ответа (`NodeUnreachableError`) задача всё равно переходит в подтверждение с дедлайном `now + 120 с` — apply мог пройти, а связь оборваться именно из-за смены адресов → цикл подтверждения раз в 3 с: `fetch_state(fresh_connection=True)` через `new_node_probe_client`; нода отвечает и транзакция `pending` → `send_confirm` → `confirmed`; транзакция уже сменилась — статус ищется в `history`; нода молчит до `deadline_at + 15 с` (запас на расхождение часов) → `rolled_back` (нода откатила сама). HTTP-ответ на apply ждёт задачу не дольше `INLINE_WAIT_SECONDS = 20` и отдаёт снимок (`NetworkJob.snapshot()`: `phase applying|confirming|done`, `status`, `transaction_id`, `deadline_at`, `attempts`, `last_error`, `message`, `error_log`, `rolled_back`, `warnings`, `reachability`); дальше UI поллит `GET state`, куда снимок подмешивается как `job` (хранится в памяти 10 минут после завершения). После `confirmed` — информационная проверка достижимости (`check_reachability`: TCP на порт ноды к каждому новому публичному адресу, 3 с, только без SOCKS-прокси и при ≤16 адресах). `rollback()` гасит задачу и шлёт `POST rollback`; `cancel_all_jobs()` — в shutdown панели (неподтверждённую транзакцию нода откатит сама).
- Роутер `routers/proxy.py`: `GET /proxy/{id}/network/state`, `POST …/preview` (`{add_text, gateway}`), `POST …/apply` (`{interface, add_text, gateway, remove: [{address, prefix}]}`), `POST …/rollback`; все — `require_capability(SYSTEM)`; ошибки ноды маппятся как в `proxy_request` (`403` → `learn_from_denial` + `409`, таймаут → `504`, обрыв → `502`).
- Frontend: `components/servers/NetworkAddressesCard.tsx` на странице сервера под «Лимитом полосы» — список интерфейсов (default первым, down приглушённо; бейджи v4/v6, «основной», DHCP, «через панель», замок с подсказкой для чужих адресов, «Удалить» только для своих), модалка «Добавить IP» (интерфейс, textarea с подсказкой форматов, необязательное поле шлюза с шлюзом основного адреса в плейсхолдере и подсказкой «совпадает с основным — маршруты не понадобятся», дебаунс-preview с пометкой «уже есть — будут пропущены» и «через шлюз X», предупреждение про таймер 120 с); у адреса со своим шлюзом — пометка `via <шлюз>`, модалка удаления, блок прогресса транзакции (применение → проверка связи новым подключением → итог; обратный отсчёт до отката, попытки, кнопка «Отменить (откатить)», при `confirmed` — точки достижимости новых адресов, при ошибке — раскрывающийся `error_log`); поллинг состояния каждые 3 с только пока транзакция не завершена. Ключи `server_details.network_*`, FAQ `SERVER_DETAILS_NETWORK`.
- Тесты: `panel/backend/tests/test_network_addresses.py` (все формы записи, диапазоны, подсети, отказ IPv6-подсетей, unassignable, дубли и лимиты, тексты ошибок, шлюз), `test_network_transactions.py` (гейты версии, host/порт из URL, дедлайн с запасом, вычитание адресов, конфликты шлюза, сериализуемость снимка), `test_network_apply_timeout.py` (location `/api/system/network/` на ноде и его таймауты ≥ `APPLY_TIMEOUT_SECONDS`); `CallSiteCoverageTest.GATED` пополнен `services/network_transactions.py`.

**Доступы хостера (разведка и вырезание на странице сервера):**

Карточка на странице сервера сканирует ноду на средства доступа хостера (qemu-guest-agent, cloud-init, мониторинг-агенты, чужие SSH-ключи, лишних sudo-юзеров, apt-зеркала хостера и `trusted=yes`, ослабленный sshd) и по подтверждению вырезает выбранное. Вся логика детекции и удаления — на ноде ([node/DOCUMENTATION.md](../node/DOCUMENTATION.md#доступы-хостера-hoster_accesspy)); панель проксирует и отвечает за подтверждение и отображение. Удаление необратимо (по решению владельца — без бэкапа и отката).

- Гейт по версии: `MIN_NODE_VERSION_HOSTER = "10.29.0"` (`Server.node_version`) в `routers/proxy.py`; старая нода → `GET scan` отдаёт `supported: false` с `min_node_version`, карточка просит обновить ноду; `POST purge` на старой ноде → `409`.
- Роутер `routers/proxy.py`: `GET /proxy/{id}/hoster-access/scan` (`require_capability(SYSTEM, ro)`, таймаут 60 с) и `POST /proxy/{id}/hoster-access/purge` (`require_capability(SYSTEM, rw)`, тело `{finding_ids[], confirm}`, `proxy_request(..., apply=True, timeout=280)` — долгий apt purge через apply-клиент read=300). `proxy_request` получил параметр `apply` (выбор `get_node_apply_client`). Отдельного сервиса-клиента к ноде нет — всё через `proxy_request`, поэтому реестр `CallSiteCoverageTest.GATED` не трогается.
- nginx панели: `location ~ ^/api/(proxy/\d+/hoster-access/purge)$` с `proxy_read/send_timeout 300s` — общий `/api/` (60 с) оборвал бы purge.
- Frontend: `components/servers/HosterAccessCard.tsx` на странице сервера под «Сетью / IP-адреса» — скан на маунте, список находок с чекбоксами (severity-точка, красный бейдж «аварийный доступ», дефолт по `default_selected`), кнопки «Просканировать заново» и «Вырезать выбранное», модалка подтверждения (перечень, предупреждение про qemu-ga/cloud-init/SSH, галочка «у меня есть независимый доступ»), после purge — повторный скан и результат по каждому пункту. Гейты `nodeAllows(server, 'system', …)`. Ключи `server_details.hoster_*`, FAQ `SERVER_DETAILS_HOSTER`.
- Тесты: `panel/backend/tests/test_hoster_access_timeout.py` — location purge на ноде и на панели не короче `HOSTER_PURGE_TIMEOUT`.
...
**Опрос нод:** `GET /api/system/version/base` отдаёт список нод из БД без походов на них, дальше фронт грузит каждую через `GET /api/system/nodes/{id}/version` (пул по 12). Бэкенд перед запросом к ноде сверяется со статусом коллектора метрик (`resolve_status` + `get_offline_threshold`, `services/server_status.py`): ноду со статусом `offline` он не опрашивает вовсе и сразу отвечает `status: "offline"` с пустыми `optimizations` — иначе каждая лежащая нода занимала бы слот пула на весь таймаут. Ноды `online` и `loading` (ещё ни разу не опрошенные коллектором) запрашиваются как обычно. Тесты: `panel/backend/tests/test_node_version_offline.py`.
...
| GET | /api/servers/fleet/history | Сводная история парка для графика под плитками дашборда (см. «Dashboard: сводная панель флота») |
...
Тот же сертификат мастер автоустановки выпускает сам по чекбоксу «Одноразовый (персональный) ключ» — без записи в `node_install_keys` и без участия оператора, см. [Авторазвёртывание ноды](#авторазвёртывание-ноды).
...
- Мгновенные цифры плиток бэкенд не считает — они уже приходят в `GET /api/servers?include_metrics=true`.

**История сводки.** Плитка — кнопка: клик раскрывает под рядом общий блок с графиком (`MultiLineChart`, полоса пиков, настройки «Графики» через `useChartDisplay`) и `PeriodSelector` на 1h/24h/7d/30d/365d. Клик по другой плитке переключает метрику без запроса — ответ несёт все три сразу; повторный клик по активной сворачивает блок. Приём и отдача — одна метрика на двух плитках: график у них общий (линии ↓ и ↑), поэтому подсвечиваются обе и клик по любой из них сворачивает — подсветка и поведение клика не расходятся. Пока блок свёрнут, история не запрашивается вообще; открытая перечитывается `useAutoRefresh` не чаще появления новой точки (1ч — шаг автообновления дашборда, 24ч — 60 с, дальше — 300 с). У `motion.div` блока постоянный `key`: без него `AnimatePresence` теряет уходящего ребёнка при ререндере от поллинга и оставляет свёрнутый график в DOM.

**`GET /api/servers/fleet/history?period=` (`services/fleet_history.py`)** — отдельной таблицы под сводку нет, точка собирается на лету из per-server истории двумя уровнями агрегации: внутренний CTE усредняет по `(сервер, бакет)`, внешний считает по бакету. Без внутреннего уровня нода с двумя снапшотами в бакете весила бы вдвое. Источник по периодам: 1h и 24h — `metrics_snapshots` с бакетами 60 и 300 с (`date_bin` от epoch), 7d/30d — часовые агрегаты, 365d — суточные. Сырых точек «как есть» у парка не бывает: снапшоты разных нод не выровнены по времени.

Формулы одинаковы для сырого и агрегированного источника, чтобы линия не прыгала при смене периода: `cpu_usage` = Σ(cpu·ядра)/Σядер (та же взвешенная по ядрам логика, что у плитки), `memory_percent` = Σ(mem%·RAM)/ΣRAM, `memory_used` = процент × Σ RAM (у часовых и суточных агрегатов байтов памяти нет, только проценты — поэтому единый путь идёт через процент), скорости — суммы, пики — те же формулы по `max_*`-колонкам. `FILTER` на знаменателе обязателен: нода без значения метрики не должна занижать средневзвешенное своим весом. В точке есть `servers` — сколько нод в неё попало.

Ёмкость (ядра и объём RAM) в истории не хранится, поэтому берётся текущая — из `Server.last_metrics` активных серверов; Python строит три параллельных массива и отдаёт их в SQL, где `unnest` раскладывает их в таблицу весов (`CAST` обязателен: `unnest` полиморфен и без явного типа PostgreSQL не выведет тип параметра). Ресайз VPS переписывает вес всей его истории задним числом — для обзорного графика парка это дешевле, чем колонки ёмкости в каждой строке снапшотов.

Линию рвёт только простой самой панели (`server_downtime.kind = 'panel'`, строка пишется на каждую ноду — `merge_downtime` их дедуплицирует): падение одной ноды просто вычитает её из суммы, это видно по `servers`. Новых индексов не потребовалось — у `metrics_snapshots.timestamp` и `aggregated_metrics.timestamp` уже есть одиночные b-tree индексы под диапазон по времени сразу по всем серверам. Ответ кэшируется в памяти по ключу периода (30/60/300 с) по образцу кэша `GET /servers?include_metrics=true` — иначе каждая открытая вкладка дашборда гоняла бы запрос по снапшотам всего парка заново.

**Файлы:**
- `panel/frontend/src/components/Dashboard/FleetSummary.tsx`
- `panel/frontend/src/pages/Dashboard.tsx`
- `panel/backend/app/services/fleet_history.py`, эндпоинт в `routers/servers.py`, тесты `tests/test_fleet_history.py`
...
**Перекос памяти между процессорами:** нода с 10.30.0 отдаёт `memory.numa_nodes` — память по NUMA-узлам. `components/servers/NumaMemoryWarning.tsx` над карточками метрик на `ServerDetails.tsx` сравнивает узлы с ядрами (узлы без ядер, например CXL-память, пропускает) и показывает жёлтую плашку с объёмом каждого узла, если самый большой узел в `IMBALANCE_RATIO` (1.5) раза и более больше самого маленького. При одном узле, ровной раскладке или старой ноде без поля ничего не выводится. Бэкенд поле не трогает — оно доезжает в `last_metrics` как есть. Ключи `server_details.numa_*`.
...
**Эфемерные порты на странице трафика.** Карточка `components/Traffic/EphemeralPorts.tsx` под блоком TCP/UDP-соединений: диапазон и быстрый запас на направление в подзаголовке, строка на каждый исходящий адрес ноды с полосой заполнения и остатком, разворачивается в список направлений («адрес:порт цели» → занято / запас). Запас — быстрая половина диапазона (`fast_capacity`, занятое — `fast_held`): `connect()` первым проходом перебирает только порты чётности нижней границы, и когда они на направлении кончаются, каждое соединение перебирает их целиком при свободной второй половине — полоса от всего диапазона показывала бы «свободно больше половины» уже в перегрузке. У ноды до 10.30.0 полей `fast_*` нет, и карточка считает от `capacity`/`held`, как раньше. Потолок считается на пару «наш адрес → адрес:порт цели», поэтому у адреса показывается его ближайшее к потолку направление, а не сумма: с тремя направлениями сумма втрое больше потолка и ни о чём не говорит. Занятое и остаток берутся у ноды готовыми — там учтён `tcp_tw_reuse`, при котором TIME_WAIT потолок не подпирает (см. [node/DOCUMENTATION.md](../node/DOCUMENTATION.md#учёт-эфемерных-портов-ephemeral_portspy)); панель их не пересчитывает. Цвет остатка повторяет пороги полосы (60% / 85%), иначе зелёное «свободно» спорило бы с красной полосой на одном адресе. Данные — из живых метрик, поэтому карточка не зависит от сводки трафика и рисуется рядом с графиком скорости; нода без этого поля (старее текущего цикла) карточку просто не показывает, отдельного гейта по версии нет. FAQ `TRAFFIC_EPHEMERAL_PORTS` — значок у заголовка: откуда берётся запас, почему он на направление и почему быстрой бывает только половина, и что делать при заполнении (добавить исходящий адрес, развести по портам цели, для `127.0.0.1 → 127.0.0.1:порт` — шаблон Remnawave nginx с раскладкой по `127.0.0.1–16`, проверить `tcp_tw_reuse`).
...
- `panel/frontend/src/components/Traffic/EphemeralPorts.tsx`, типы `EphemeralPorts`/`EphemeralSource`/`EphemeralDestination` в `api/client.ts`, `data/faq/content/{ru,en}/TRAFFIC_EPHEMERAL_PORTS.md`, ключи `ephemeral.*` в `locales/{ru,en}.json` (`range_fast` — подзаголовок с быстрым запасом, `range` — для нод без него)
...
- **Exit-прокси** (`exit_proxy/service.py`) — `KIND_EXIT_PROXY`, исполнитель `push_exit_proxy_to_servers(server_ids)`: собирает конфиг ноды из настроек и её правил в момент отправки. Долг ставится циклом и `sync_one()` для офлайн-нод и сорвавшихся запросов; отказ по правам/версии в очередь не идёт — нода со старым агентом остаётся `unsupported`, пока цикл не увидит новую версию.
...
|-----|--------|-----------|----------------|
| `ip_exceeds_limit` | IP-адресов > `hwid_device_limit + anomaly_ip_margin` (default 2); перед подсчётом из IP пользователя исключаются IP нод парка (`_get_node_ips()` — hostname из `Server.url` каждой ноды, с DNS-резолвом при необходимости): транзитная нода (DNAT-релей) светится в Xray-логах как source_ip клиента и не является устройством пользователя; подавляется, если уникальных ASN ≤ `hwid_device_limit + anomaly_asn_margin` (default 0), либо если суточный трафик пользователя ниже `anomaly_ip_smart_traffic_gb` (default 20 ГБ, см. «Умное определение» ниже) | [Игнор IP] [Сбросить] | `anomaly_ip_enabled` |
...
- **Exit-прокси** (`exit_proxy_switched`, `exit_proxy_no_healthy`, `exit_proxy_recovered`, `exit_proxy_self_test_failed`, `exit_proxy_self_test_recovered`) — шлёт не алертер, а `services/exit_proxy/alerts.py` из цикла exit-прокси тем же ботом и чатом; свой cooldown по (нода, вид), тумблер в настройках раздела; записи попадают в ту же `alert_history` (см. «Exit-прокси»).
...
**Сводка над списком** (`BillingSummary`) считается на фронте из уже загруженного списка, без отдельных запросов: расход в месяц, сумма балансов (без помесячных проектов — у них нет счёта) и плитка «Срок оплаты». Суммы группируются по валютам и печатаются через `·` — рубли с долларами в одно число не складываются. Плитка «Срок оплаты»: если есть истёкшие проекты — «Истёк», при нескольких — с числом («Истёк (2)»), рядом мелким текстом «ближайший через 3д 5ч» по первому неистёкшему; если истёкших нет — крупно остаток ближайшего проекта (цвет по `statusColor`: ≤3 дн. красный, ≤7 жёлтый, дальше зелёный). Подпись под значением — имя ближайшего проекта и дата истечения в часовом поясе панели (`formatDateTime` приходит пропом со страницы); когда неистёкших с известным сроком нет — «Ближайших истечений нет». Иконка красная при истёкших, жёлтая при ближайшем ≤7 дн., иначе нейтральная.
...
Окно потребления у Yandex Cloud и у истории баланса одинаковое — 3 дня (`CONSUMPTION_WINDOW_DAYS` и `HISTORY_WINDOW_DAYS`): оценка держится свежей и реагирует на изменение нагрузки за те же трое суток.

Срок Selectel панель не считает — его отдаёт сам Selectel (`/v2/billing/prediction`: на сколько дней хватит баланса); списания и статистика расходов не запрашиваются. Значение трактуется как **дни**, хотя документация провайдера называет его часами: на живом аккаунте `primary: 46` совпало с расчётом по фактическим списаниям (53 дня), а прочтение «46 часов» разошлось бы с фактом в 28 раз. Пустые группы (`null`) и нули пропускаются, берётся ближайшее исчерпание среди остальных. Прогноз приводится к дневному расходу (`balance / days_left`) и пишется в `cloud_daily_cost` — дальше порог остатка, срок оплаты и калькулятор пополнения работают для всех провайдеров по одной формуле `compute_days_left(balance, threshold, daily_cost)`; при нулевом пороге срок в панели равен прогнозу Selectel. Ошибка получения прогноза не фатальна: баланс уже сохранён, в `cloud_last_error` остаётся причина (предупреждение о долге важнее и вытесняет её), срок показывается как неизвестный. Ошибки провайдера — доменные `CloudBillingError`/`CloudAuthError`; запросы к Selectel повторяются 3 раза с экспоненциальной паузой на 5xx, на 4xx не повторяются.

**Расход по истории баланса (`uses_balance_history`, Timeweb):** публичного API истории списаний у Timeweb нет, поэтому `_apply_snapshot` при каждой синхронизации дописывает точку `[iso_ts, balance]` в `cloud_balance_history` (JSON на сервере биллинга) и считает фактический дневной расход по снижению баланса. Точки старше `HISTORY_WINDOW_DAYS` (3 дня, то же окно, что у потребления Yandex Cloud) отбрасываются; синхронизации чаще `HISTORY_MIN_GAP` (15 минут — ручные «Обновить») передвигают последнюю точку, а не плодят новые, так что история остаётся примерно почасовой при штатном интервале чекера. Интервалы, где баланс вырос (пополнение), выбрасываются целиком — из числителя и из знаменателя: сумма пополнения из API не видна, и расход внутри такого интервала восстановить нельзя. Пока покрытых списаниями интервалов меньше `HISTORY_MIN_SPAN` (6 часов), оценка считается шумом и используется тариф из снапшота (`hourly_fee × 24`) — тариф отражает только текущий набор ресурсов, история же ловит и плавающие услуги (трафик, S3).
...
**Одноразовый (персональный) ключ (`dedicated_cert`):**

Чекбокс над блоком доп. компонентов. Без него в команду установки уходит общий ключ парка (`build_installer_token`) — сертификат с `clientAuth`, которым обладатель ноды технически может представиться панелью перед любой другой нодой. С чекбоксом `_build_deploy_params` вызывает `build_dedicated_installer_token(pki, panel_ip)` (`services/pki.py`): выпускается собственный сертификат сервера с CN `node-<12 hex>`, EKU только `serverAuth`, срок `DEDICATED_DEPLOY_VALIDITY_DAYS` (1095 дней — как у shared cert: это рабочий серверный сертификат ноды, не короткоживущий токен). Сертификат собирается на лету и нигде не сохраняется — ни в БД, ни в списке ключей установки.

Флаг едет и в `POST /api/servers/deploy`, и в `POST /api/servers/deploy/command`, поэтому работает одинаково при установке по SSH, в полуавтоматическом режиме и для каждой extra-карточки формы (у каждой своя копия параметров). В полуавтомате каждая пересборка команды (дебаунс 400 мс) выпускает новый сертификат — все они валидны, нода примет ту версию, которую оператор реально запустил: nginx ноды сверяет только подпись CA, CN не проверяется.

Созданный сервер получает `uses_shared_cert=False` и `dedicated_cert=True`. Второй флаг существует ради механизма миграции: `classify_server` относит такую ноду к классу `dedicated`, она не попадает в `needs_migration` и исключается из целей `POST /servers/migrate-all` — иначе клик по баннеру миграции залил бы на ноду общий cert с `clientAuth` и уничтожил изоляцию. В карточке сервера вместо предупреждения «Старый ключ» показывается нейтральный значок с подсказкой о персональном сертификате.

Отзыва сертификатов нет (как и у ключей установки под один сервер): ограничение — только срок действия. По его истечении нода перестанет проходить рукопожатие с панелью, перевыпуск делается переустановкой ноды с новым ключом.
...
- `NODE_READY_TIMEOUT = 180` / `NODE_READY_POLL_INTERVAL = 3` — окно ожидания ноды, которую панель поставила сама: агент уже здоров, ждём лишь старта nginx на порту API (см. `_wait_node_ready`)

**Постустановочная защита SSH и привязка профилей:**

Всё выполняется внутри фоновой задачи (не в HTTP-контексте):
1. Ожидание ответа ноды панели по mTLS (`_wait_node_ready`) — все шаги ниже идут к ней по сети
...
- `components/servers/NetworkAddressesCard.tsx` — карточка «Сеть / IP-адреса» на странице сервера (между лимитом полосы и терминалом): `GET /proxy/{id}/network/state`, `POST …/preview|apply|rollback`; см. «Сеть / IP-адреса» в разделе «Система». Ключи `server_details.network_*`, FAQ `SERVER_DETAILS_NETWORK`.
...
Секция `# === UPSTREAMS START/END ===` в http-контексте — keepalive-пул на каждое XHTTP-правило (`upstream xhttp_{имя} { server 127.0.0.1:{порт}; keepalive 64;  # auto: node ; keepalive_requests 100000; }`, в plain-ветке `proxy_pass http://xhttp_{имя}` + `proxy_set_header Connection ""` + `proxy_socket_keepalive on`): `packet-up` шлёт отдельный POST на каждый чанк, без пула каждый — новый TCP-коннект к loopback с TIME_WAIT на стороне nginx.

В той же секции — `split_clients "$request_id" $xray_source` (`LOOPBACK_SOURCES`: `127.0.0.1`–`127.0.0.16`, по 6.25% — `split_clients` принимает два знака после точки), а обе ветки XHTTP и gRPC-правила подключаются к инбаунду с этого адреса (`grpc_bind $xray_source` / `proxy_bind $xray_source`). Потолок соединений считается на пару «адрес источника → адрес:порт цели», и с одного `127.0.0.1` к одному инбаунду быстро открывается лишь половина эфемерного диапазона (около 32 тыс.): nginx не мультиплексирует потоки к апстриму, так что каждый одновременный поток клиента — своё соединение к loopback. Весь `127.0.0.0/8` локален на `lo`, поэтому каждый адрес источника даёт свой запас — 16 адресов ≈ 515 тыс. одновременных потоков на инбаунд, с запасом на ноды в 10–20 Гбит (лишний адрес ничего не стоит: это строка в `split_clients`) — а `listen` инбаунда остаётся `127.0.0.1`. nginx ставит на такой сокет `IP_BIND_ADDRESS_NO_PORT` — порт выбирается в `connect()`, у каждого адреса независимо (на стенде с nginx 1.28 и Xray 26 все четыре режима — stream-one, stream-up, packet-up, gRPC — рабочие, соединения ровно по всем 16 адресам, порты из быстрой половины). Ключ `$request_id` случаен на каждый запрос — адреса нагружаются ровно. Переменная объявлена внутри секции UPSTREAMS, поэтому правила ссылаются на неё, только когда секция есть; в конфиге без неё — подключение с `127.0.0.1`, как раньше, и `$xray_source` не упоминается. У профиля без правил Xray секция пустая. h2c-ветке (`grpc_pass`) keepalive-пул не даётся осознанно: nginx возвращает h2c-соединение в кэш, только если тело запроса дослано к моменту конца ответа, а у прокси-потока первым заканчивается ответ — на стенде пул не переиспользовал ни одного соединения. Размер пула `keepalive` помечен `# auto: node` — нода считает его от `worker_connections` (`clamp(worker_connections/64, 64, 512)`), потому что пул из 64 не покрывает packet-by-packet аплоад крупной ноды; `keepalive_requests` в пуле от размера хоста не зависит и маркером не помечен. `splice_rules` обновляет обе секции; в конфиге без UPSTREAMS-маркеров (собран до их появления или импортирован) XHTTP-правило проксирует напрямую на `127.0.0.1:{порт}` без пула — пул появится после «Вставить шаблон». `large_client_header_buffers 8 32k` в http-контексте — `packet-up` умеет нести данные в заголовке запроса (`uplinkDataPlacement: header`), и на больших `uplinkChunkSize` дефолтных буферов не хватает; директива существует только на уровне http/server, локацией её не закрыть, поэтому в импортированном конфиге без шаблона её нужно добавить руками.
...
Google банит выходы Cloudflare WARP целым диапазоном (капча «подозрительный трафик» в поиске и Gemini) и со временем относит IP серверов к RU по трафику российских клиентов — Gemini для такого адреса недоступен. Раздел держит Google-трафик на IP, который Google ещё не считает заблокированным: на включённых нодах агент поднимает локальный SOCKS5 (`127.0.0.1:<port>`, по умолчанию 7590) с пулом исходящих адресов, а в Remnawave этот socks указывается outbound'ом для Google. Вся логика выбора — на ноде (см. [node/DOCUMENTATION.md](../node/DOCUMENTATION.md#exit-прокси-exit_proxy)): панель лишь присылает конфиг, забирает статус, ведёт журнал и уведомляет. Панель упала — прокси на нодах продолжают работать и переключаться.

**Модель (`app/models.py`, три таблицы через `create_all`):**
- `ExitProxySettings` (singleton) — `enabled` (общий выключатель цикла), `port`, `check_interval_minutes`, `blocked_countries` (JSON, по умолчанию `["RU"]`), `builtin_checks` (JSON: `google_country`, `google_captcha`, `gemini`), `custom_checks` (JSON; сток — Claude и ChatGPT через их API, `api.anthropic.com/v1/models` и `api.openai.com/compliance/cookie_requirements`, блок по тексту отказа `request not allowed`/`unsupported_country`: сайты claude.ai и chatgpt.com за bot-защитой Cloudflare отвечают хостинговым IP 403-челленджем независимо от региона, а Reddit режет хостинговые диапазоны в любой стране; `upgrade_stored_stock_checks()` при `start_exit_proxy()` переводит нетронутые пользователем стоковые записи старого образца (`LEGACY_STOCK_CHECKS`) на новые — Reddit выключается, правленые не трогаются, ноды получают конфиг по расхождению хэша), `telegram_enabled`, `alert_cooldown_seconds`, `last_cycle_at/error`.
- `ExitProxyNode` (PK `server_id`) — нода, включённая в раздел: `enabled`, `select_mode` (`auto`/`manual`), `pinned_candidate`, `candidates_order`/`candidates_disabled` (JSON), последний статус ноды целиком (`node_status`, JSON), `current_candidate`, `self_test_ok`, `last_event_at` (водораздел событий ноды), `config_hash`, `sync_status` (`pending`/`synced`/`failed`/`denied`/`unsupported`), `sync_error`. Строка остаётся после выключения — порядок кандидатов и pin не теряются.
- `ExitProxyEvent` — журнал панели: `kind` (`switched`, `deferred`, `manual_switch`, `no_healthy`, `recovered`, `self_test_failed`, `self_test_recovered`, `check_failed`, `started`, `stopped`), `from_value`, `to_value`, `reason`.

**Цикл (`services/exit_proxy/service.py`, `ExitProxyService`, тик 60 с, стартовая пауза 45 с, `trigger()` будит досрочно после правок настроек):** для каждой включённой ноды (и выключенной с долгом `pending` — ей ещё надо довезти `enabled=false`), онлайн по `is_server_online`: гейт версии (`MIN_NODE_VERSION_EXIT_PROXY = 10.29.0`) и прав (`system:write`) → `unsupported`/`denied`; `build_node_config()` (`render.py`: глобальные настройки + правила ноды → схема `ExitProxyConfig` агента) → `config_hash()`; хэш отличается или статус не `synced` → `PUT /api/system/exit-proxy/config` → `synced`; затем `GET /status` → `absorb_status()`: статус сохраняется целиком, события ноды новее `last_event_at` переносятся в `ExitProxyEvent`, переход `self_test.ok` даёт событие `self_test_failed`/`self_test_recovered`, после чего уходят Telegram-уведомления (`alerts.py`; первый сбор статуса переигрывает хвост старых событий ноды только в журнал, без уведомлений; `switched` с пустым `from` — первый выбор — тоже без уведомления). Офлайн и сорвавшийся запрос → долг `KIND_EXIT_PROXY`. `check_now()` — `POST /check` на ноде и поллинг статуса каждые 3 с до снятия `check_in_progress` (≤180 с); `switch()` — `POST /switch`. Таймаут запросов к ноде 25 с — у путей `/api/system/exit-proxy/*` нет своего `location` в nginx ноды, действует общий 30 с (тест-инвариант `test_exit_proxy.py`).

**Порт** резервируется от эфемерной выдачи вместе с остальными: `reserved_ports_sync.merged_entries()` получает `service_entries` из `exit_proxy.settings.service_ports()` (порт только при включённом разделе), смена порта или общего выключателя запускает `apply_reserved_ports()`.

**WARP на ноду** ставится кнопкой из раздела: `deploy_service.build_warp_install_command()` (`MON_INSTALL_WARP=1 … install.sh --unattended`) через общий `HostInstallJobManager` (`services/remnawave_node_install.py`, тот же, что ставит ноду Remnawave: параметры — стартовая строка лога и колбэк успеха; для WARP колбэк — `check_now()`), лог — NDJSON-стрим `GET /api/exit-proxy/warp-install/{job_id}/stream` (в обходе GZip-middleware).

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET/PUT | /api/exit-proxy/settings | `{enabled, check_interval_min, port, blocked_countries[], notify_enabled, min_node_version, last_cycle_at, last_cycle_error}`; порт 1024–65535 и не из портов агента/Xray-теста/WARP/nginx (`RESERVED_SERVICE_PORTS`); смена порта/интервала/стран → все ноды `pending` |
| GET | /api/exit-proxy/status | Состояние воркера: `running`, `last_tick_at`, `last_error`, `enabled`, `port`, `min_node_version` |
| GET | /api/exit-proxy/nodes | `{nodes: [...]}` по всем активным серверам: `install_status` (`off`/`pending`/`active`/`failed`/`denied`/`unsupported`), `current_exit`, `pending_switch`, `candidates[]` с результатами проверок (`healthy`, `country`, `captcha`, `gemini`, `checks{}`), `self_test`, `warp.present`, `stats`, `check_in_progress` — см. `views.node_view()` |
| PUT | /api/exit-proxy/nodes/{id} | `{enabled?, select_mode?, pinned_candidate?, candidates_order?, candidates_disabled?}`; включение — 409 при старом агенте или закрытом домене `system`; после сохранения конфиг доставляется сразу (`sync_one`) |
| POST | /api/exit-proxy/nodes/{id}/check-now | Прогон проверок на ноде с ожиданием результата |
| POST | /api/exit-proxy/nodes/{id}/switch | `{tag}` — ручное переключение (в `auto` держится, пока здоров; в `manual` закрепляется) |
| POST | /api/exit-proxy/nodes/{id}/install-warp | → `{job_id}`; `GET /api/exit-proxy/warp-install/jobs`, `GET /api/exit-proxy/warp-install/{job_id}/stream` |
| GET | /api/exit-proxy/checks | `{builtin: [{key, enabled}], custom: [...]}`; `PUT /checks/builtin/{key}`, `POST /checks/custom`, `PUT`/`DELETE /checks/custom/{id}` — любое изменение переводит ноды в `pending` |
| GET | /api/exit-proxy/snippet | `{outbound_json, rules_json, text}` — outbound `socks 127.0.0.1:<port>` с тегом `exit-proxy` и правила: обход в `DIRECT` для `geosite:youtube`, `geosite:googlefcm`, `geosite:google-play` (первым — `geosite:google` включает их, а видео, push Android и загрузки Play через релей не нужны), блок `udp/443` в `BLOCK` (QUIC мимо socks не пройдёт) и остальной Google одним правилом (`geosite:google`, `geosite:google-gemini`, `googleapis.com`, `gstatic.com`, `googleusercontent.com`) |
| GET | /api/exit-proxy/log | Журнал панели, `limit` ≤ 500 |

**Frontend (`pages/ExitProxy.tsx`, `components/exitproxy/`, `stores/exitProxyStore.ts`):** вкладки «Ноды» (серверы папками, тумблер включения, бейдж состояния, текущий выход со страной и здоровьем, сквозная проверка; раскрытие — режим auto/manual, кандидаты с перетаскиванием приоритета (`@dnd-kit`), тумблером участия и кнопкой «Переключить», результаты проверок чипами, блок WARP с установкой и логом, «Проверить сейчас», «Отправить конфиг заново»; внизу — сворачиваемый блок «Конфиг для Remnawave» с копированием), «Проверки» (встроенные тумблерами, свои — inline-форма имя/URL/коды/шаблоны), «Настройки» (общий выключатель, интервал, порт с предупреждением, страны чипами, уведомления), «Журнал». Поллинг через `useSmartRefresh`. Общий `components/ui/Toggle.tsx` (переехал из Anti-DDoS), `utils/folders.ts` (`orderFolders`, переехал из `ServerAddDropdown`), `utils/format.ts` (`getFlag`, переехал из Remnawave). FAQ `PAGE_EXIT_PROXY`.

**Файлы:**
- `panel/backend/app/models.py` — `ExitProxySettings`, `ExitProxyNode`, `ExitProxyEvent`
- `panel/backend/app/services/exit_proxy/` — `settings.py`, `render.py`, `node_client.py`, `service.py`, `views.py`, `alerts.py`, `warp_install.py`
