...
| `selectel` | Статический API-ключ (заголовок `X-Token`) | `GET /v3/balances` — сумма `final_sum` по биллингам, делённая на 100 (API отдаёт копейки) | `GET /v2/billing/transactions` за 30 дней → сумма списаний ÷ 30; запасной вариант — `GET /v2/billing/prediction` |

Окно потребления Selectel — 30 дней, а не неделя: разовые месячные списания (выделенные серверы) попадают в него ровно один раз и не множатся на 30 при усреднении. Пагинация — по 500 записей, не более 20 страниц.
