...
- `ExitProxyEvent` — журнал панели: `kind` (`switched`, `manual_switch`, `no_healthy`, `recovered`, `self_test_failed`, `self_test_recovered`, `check_failed`, `started`, `stopped`), `from_value`, `to_value`, `reason`.
...
| GET | /api/exit-proxy/nodes | `{nodes: [...]}` по всем активным серверам: `install_status` (`off`/`pending`/`active`/`failed`/`denied`/`unsupported`), `current_exit`, `candidates[]` с результатами проверок (`healthy`, `country`, `captcha`, `gemini`, `checks{}`), `self_test`, `warp.present`, `stats`, `check_in_progress` — см. `views.node_view()` |
