...
- **Системные оптимизации** — устанавливается через `MON_INSTALL_OPTIMIZATIONS=1`; NIC-режим (multiqueue/hybrid/rps) определяется автоматически функцией `auto_detect_nic_mode()` на целевом сервере
...
**Поле `install_optimizations` в запросе `POST /api/servers/deploy`:**

`DeployParams` и `deploy_service.py` принимают поле `install_optimizations: bool`. При `true` в env-переменные запуска `install.sh` добавляется `MON_INSTALL_OPTIMIZATIONS=1`; NIC-режим определяется автоматически через `auto_detect_nic_mode()` на целевом сервере.
...
- Чекбоксы компонентов: **Системные оптимизации** (NIC-режим определяется автоматически), WARP, Remnawave, HTTP-прокси
...
| POST | /firewall-profiles/{id}/rules | Добавить правило |
| PUT | /firewall-profiles/{id}/rules/{index} | Обновить правило по индексу |
...
3. `POST /{id}/sync` раскатывает конфиг на все привязанные серверы параллельно
...
| PUT | /haproxy-profiles/{id} | Обновить профиль |
...
- **Per-server кнопка "Запустить HAProxy"** (иконка Play, зелёная) — отображается в строке сервера только когда `haproxy_running === false` (HAProxy явно остановлен). Расположена слева от кнопки "Sync to server". Вызывает `POST /api/proxy/{server_id}/haproxy/start` через `proxyApi.startHAProxy()`. Обработчик: `handleStartHaproxy`, состояние загрузки: `startingServerId`.
- **Bulk-кнопка "Запустить остановленные"** (иконка Play, зелёная) — отображается в тулбаре раздела серверов, только когда хотя бы у одного привязанного сервера `haproxy_running === false`. Параллельно запускает HAProxy на всех остановленных серверах через `Promise.allSettled`. Обработчик: `handleStartAllStopped`, состояние загрузки: `startingAll`.

Статус `haproxy_running` приходит из `HAProxyServerStatus` (поле из ноды через `/api/proxy/{id}/haproxy/status`). Три возможных значения: `true` (работает), `false` (остановлен), `null/undefined` (статус неизвестен). Кнопки показываются только при строгом `=== false`.
...
**Файлы:**
- `panel/backend/app/routers/haproxy_profiles.py` — API роутер
- `panel/frontend/src/pages/HAProxyConfigs.tsx` — страница управления
- `panel/frontend/src/api/client.ts` — `haproxyProfilesApi`, `proxyApi.startHAProxy()`, интерфейсы
