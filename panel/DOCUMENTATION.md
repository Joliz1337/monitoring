...
**Принцип работы:**
1. На нодах `XrayLogCollector` читает логи через `docker exec remnanode tail -f`
2. Логи агрегируются в памяти ноды (destination + email → count)
3. Панель каждые 60 сек вызывает `/api/remnawave/stats/collect` на каждой ноде
4. Нода отдаёт данные и очищает память
5. Панель инкрементирует счётчики в xray_visit_stats и добавляет запись в xray_hourly_stats
6. Раз в час обновляется кэш пользователей через Remnawave API
