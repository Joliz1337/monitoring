...
- **Системные оптимизации** — устанавливается через `MON_INSTALL_OPTIMIZATIONS=1`; NIC-режим (multiqueue/hybrid/rps) определяется автоматически функцией `auto_detect_nic_mode()` на целевом сервере; при включении появляется переключатель профиля (см. ниже)
...
**Профиль sysctl-оптимизаций (`opt_profile`):**

Отображается под чекбоксом «Системные оптимизации» в виде двух кнопок-переключателей. Передаётся в `POST /api/servers/deploy` и далее через `MON_OPT_PROFILE` в `install.sh`.
