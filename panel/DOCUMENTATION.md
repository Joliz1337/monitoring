...
- **Анти-DDoS** — многослойная защита от DDoS-атак на нодах: дежурный режим без лимитов, аварийный режим (iptables-цепочка `ANTIDDOS`: SYNPROXY + connlimit + hashlimit) включаемый вручную или автоматически локальным watchdog по сигналам из `/proc`, whitelist на ipset с ежечасной автосинхронизацией; страница с независимыми контролами автодетекта и аварийного режима по нодам, редактированием ручного whitelist (CDN-подсети); установка watchdog на ноды полностью автоматическая (zero-touch)
