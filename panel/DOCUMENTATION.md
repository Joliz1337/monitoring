...
| `hwid_exceeds_limit` | HWID-устройств > `hwid_device_limit`; триггерит авто-очистку через API; очистка подавляется, если суточный трафик пользователя ниже `anomaly_hwid_smart_traffic_gb` (default 20 ГБ, см. «Умное определение» ниже) | нет уведомления | `anomaly_hwid_enabled` |
...
| `invalid_device_data` | На HWID-устройстве пусто платформа/модель, либо версия ОС без цифр (`VERSION_PATTERN`) | [Игнор HWID] | `anomaly_devdata_enabled` |
...
- `anomaly_hwid_smart_enabled` (Boolean, default TRUE) / `anomaly_hwid_smart_traffic_gb` (Float, default 20.0) — то же «умное определение» для HWID-аномалии: перед авто-очисткой устройств панель сверяет суточный расход трафика пользователя (тот же `_user_daily_traffic`) и пропускает очистку, если он ниже порога — у семьи много устройств, но трафик как у одного человека. Проверка идёт после cooldown-проверки, и при подавлении cooldown не фиксируется — при росте трафика очистка сработает в ближайшем цикле. Данные о трафике недоступны — очистка выполняется (fail-open, как у IP-варианта).
