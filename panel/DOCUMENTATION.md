...
- **Управление** — список нод со статусом (дежурный/аварийный, источник, причина, watchdog), точечные и массовые (все ноды) переключатели аварийного режима и watchdog, кнопка «Установить watchdog на все» — теперь опциональная (раскатка и так идёт zero-touch фоновым опросом), используется чтобы обновить/переустановить немедленно. Версия watchdog-скрипта нодой отдаётся (`GET /api/antiddos/status` → `version`), но панель использует её только внутри `_maybe_auto_install` для сравнения — в UI не выводится.
