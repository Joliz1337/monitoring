...
**История сводки.** Плитка — кнопка: клик раскрывает под рядом общий блок с графиком (`MultiLineChart`, полоса пиков, настройки «Графики» через `useChartDisplay`) и `PeriodSelector` на 1h/24h/7d/30d/365d. Клик по другой плитке переключает метрику без запроса — ответ несёт все три сразу; повторный клик по активной сворачивает блок. Обе плитки скорости открывают один сетевой график с линиями ↓ и ↑. Пока блок свёрнут, история не запрашивается вообще; открытая перечитывается `useAutoRefresh` не чаще появления новой точки (1ч — шаг автообновления дашборда, 24ч — 60 с, дальше — 300 с). У `motion.div` блока постоянный `key`: без него `AnimatePresence` теряет уходящего ребёнка при ререндере от поллинга и оставляет свёрнутый график в DOM.
