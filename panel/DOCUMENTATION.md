...
**Проверка живости (`BalancerOptions.health_check_type`):**

| Значение | Директивы | Что доказывает |
|---|---|---|
| `tcp-check` | `option tcp-check` | Порт принял соединение |
| `httpchk` | `option httpchk <method> <uri>` + `http-check expect` | Приложение отвечает нужным кодом |
| `tls-check` | `tcp-check connect default ssl sni <sni>` + `tcp-check send`/`expect` | Порт открыт, ядро приняло соединение и сумело сходить на dest |

`tls-check` опирается на запасной ход REALITY: «неправильному» клиенту живой сервер отдаёт настоящий сайт-маскировку, поэтому TLS-рукопожатие с его SNI обязано пройти даже без валидного ключа. HAProxy делает рукопожатие, шлёт `GET /` и ждёт ответ, подходящий под `TLS_CHECK_EXPECT` (`^HTTP/1\..\ [234][0-9][0-9]` — 2xx/3xx/4xx годятся, 5xx нет). Так отсеивается упавший инбаунд за живым портом, которого `tcp-check` не видит.

**Границы применимости.** Зелёный статус означает «ядро живо и отвечает с этой ноды», не «ключи работают». Проверка не видит: блокировку на участке, по которому сама не проходит (фильтр у оператора клиента при чистом пути нода → сервер); неверный SNI (REALITY уводит в fallback любого неузнанного клиента, и он отвечает на любое имя, а не только на то, что в `serverNames`); соседние инбаунды на том же хосте (проверяется ровно адрес и порт из правила); удушение, начинающееся после рукопожатия — проверка укладывается в первые килобайты. Реальный вердикт «трафик идёт» даёт только e2e-прогон через ядро из нужной локации — раздел «Xray тест».

Детали генерации (`haproxy_config.py`): `connect default` — чтобы проверка унаследовала параметры строки `server`, прежде всего PROXY protocol, и переопределила только TLS и SNI; `verify none` на строке `server` обязателен, иначе при `ssl-server-verify required` (умолчание) HAProxy не стартует без CA-файла; у одиночного правила шаг проверки `TLS_CHECK_INTER = 30s` вместо 5 с, потому что каждая проверка — полное рукопожатие плюс реальный исходящий запрос ядра на сайт-маскировку. Доступно только для TCP-правил (у HTTPS панель терминирует TLS сама). SNI проверяется по `CHECK_SNI_PATTERN` — метки домена и точки, без пробелов и переводов строки, иначе в конфиг можно было бы дописать произвольную директиву. У одиночных правил настройка живёт в том же `balancer_options`, из которого генератор берёт только блок проверки.
