...
class Settings(BaseSettings):
    ...
    # API
    api_host: str = "127.0.0.1"  # слушаем только loopback, nginx проксирует
    api_port: int = 7500

    # mTLS: сертификаты распаковываются из NODE_SECRET в deploy.sh
    ssl_cert_path: str = "/etc/nginx/ssl/cert.pem"
    ssl_key_path: str = "/etc/nginx/ssl/key.pem"
    ca_cert_path: str = "/etc/nginx/ssl/ca.pem"
