...
"""
    ...
    def update_rule(self, name: str, updates: dict) -> tuple[bool, str]:
        ...
        # Check if we need to recreate the entire rule (type change)
        ...
        # If type, cert, or target_ssl changes - recreate the entire rule
        if type_changed or cert_changed or ssl_changed:
            # Validate new type requirements
            if new_type == "https" and not new_cert_domain:
                return False, "Certificate domain required for HTTPS rules"
            
            # Build new rule with all updated fields
