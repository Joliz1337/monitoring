...
def _rule_to_response(r: HAProxyRule) -> HAProxyRuleResponse:
    servers = [
        BackendServerModel(
            name=s.name, address=s.address, port=s.port, weight=s.weight,
    ...
    bo = None
    if r.balancer_options:
        o = r.balancer_options
        bo = BalancerOptionsModel(
            algorithm=o.algorithm, algorithm_param=o.algorithm_param,
            hash_type=o.hash_type, health_check_type=o.health_check_type,
            httpchk_method=o.httpchk_method, httpchk_uri=o.httpchk_uri,
            httpchk_expect=o.httpchk_expect,
            sticky_type=o.sticky_type, cookie_name=o.cookie_name,
            cookie_options=o.cookie_options,
            stick_table_type=o.stick_table_type, stick_table_size=o.stick_table_size,
            stick_table_expire=o.stick_table_expire,
            retries=o.retries, redispatch=o.redispatch, allbackups=o.allbackups,
            fullconn=o.fullconn, timeout_queue=o.timeout_queue,
        )

    return HAProxyRuleResponse(
        name=r.name, rule_type=r.rule_type, listen_port=r.listen_port,
        target_ip=r.target_ip, target_port=r.target_port,
        cert_domain=r.cert_domain, target_ssl=r.target_ssl,
        send_proxy=r.send_proxy, accept_proxy=r.accept_proxy,
        is_balancer=r.is_balancer,
        servers=servers, balancer_options=bo,
    )
...
async def list_rules():
    ...
    manager = get_haproxy_manager()
    rules = await asyncio.to_thread(manager.parse_rules)
...
async def create_rule(rule_data: HAProxyRuleCreate):
    ...
    rule = HAProxyRule(
        ...
        accept_proxy=rule_data.accept_proxy,
    ...
    if success:
        logger.info(
            f"HAProxy rule created: {rule.name} ({rule.rule_type}) port={rule.listen_port} "
            f"send_proxy={rule.send_proxy} accept_proxy={rule.accept_proxy}"
        )
...
async def validate_config():
    ...
    manager = get_haproxy_manager()
    is_valid, message = await asyncio.to_thread(manager.check_config)
...
async def get_config():
    ...
    manager = get_haproxy_manager()
    content = await asyncio.to_thread(manager.get_config)
...
async def get_available_certificates():
    ...
    manager = get_haproxy_manager()
    certs = await asyncio.to_thread(manager.get_available_certs)
...
async def get_all_certificates():
    ...
    manager = get_haproxy_manager()
    certs = await asyncio.to_thread(manager.get_all_certs_info)
...
async def get_certificate_info(domain: str):
    ...
    manager = get_haproxy_manager()
    info = await asyncio.to_thread(manager.get_cert_info, domain)
...
async def get_cron_status():
    ...
    manager = get_haproxy_manager()
    status = await asyncio.to_thread(manager.get_cron_status)
...
async def enable_cron():
    ...
    manager = get_haproxy_manager()
    success, message = await asyncio.to_thread(manager.setup_cert_renewal_cron)
...
async def disable_cron():
    ...
    manager = get_haproxy_manager()
    success, message = await asyncio.to_thread(manager.remove_cert_renewal_cron)
