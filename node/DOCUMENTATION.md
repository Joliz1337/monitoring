...
- **Эфемерные порты** — занятость исходящих портов в составе метрик: сколько номеров держит каждый исходящий адрес и каждая пара «адрес → адрес:порт цели», с остатком до потолка. Потолок считается на четвёрку, а не на хост, поэтому релей упирается в него на одном популярном направлении задолго до нехватки портов на сервере в целом
...
│       ├── ephemeral_ports.py      # Занятость эфемерных портов: разбор /proc/net/tcp{,6}, потолок на «адрес → адрес:порт цели»
...
- `system.ephemeral_ports` — занятость эфемерных портов по исходящим адресам, см. «Учёт эфемерных портов» ниже; `null`, если снять не удалось.
...
**Транзакция (`apply`).** `flock` на `/opt/monitoring/network/lock` → отказ, если предыдущая `pending|applying` → валидация плана (TX_ID, интерфейс в `/sys/class/net`, адреса, пути только под `/etc/netplan/`, `/etc/systemd/network/`, `/etc/network/`) → снимок `BEFORE` (все global-адреса интерфейса, отдельно не-`dynamic`) и наличие default-маршрутов v4/v6 → бэкап в `backups/<tx>/` (`manifest` путь→копия|ABSENT, `files/`, `managed.list.bak`, `routes.list.bak`, `meta.env`) → `TX_STATUS=applying` → запись файлов (tmp + `mv -f`) → бэкенд → явные `ip addr add/del` (idempotent) → `routes.list.next` из плана и `routes_sync` по нему → проверка до 20×0.5 с: все ADD стоят (IPv6 не `tentative`, `dadfailed` — провал сразу), все `BEFORE \ REMOVE` на месте, REMOVE сняты, default-маршруты, что были, остались, правила и маршруты шлюзов на месте (пропавшие — networkd сносит чужие при переконфигурации линка — ставятся заново на каждой попытке); с 6-й попытки пропавшие статические адреса возвращаются вручную. Провал → откат из бэкапа, статус `failed`, exit 4, без таймера. Успех → `managed.list` ← план, `routes.list` ← `routes.list.next`, `pending` + `TX_DEADLINE_AT`, таймер: `systemd-run --unit=mon-extra-ips-rollback-<tx> --on-active=N --timer-property=AccuracySec=1s` (без `AccuracySec` откат опаздывал бы до минуты); без `systemd-run` — `nohup setsid sh -c 'sleep N; …'` с закрытым дескриптором замка (иначе спящий процесс держал бы `flock`, и confirm/rollback получали бы «занято» до срабатывания таймера; pid в `TX_TIMER`; такой таймер живёт в cgroup контейнера — просроченную pending-транзакцию добивает `start()` агента). `confirm` останавливает таймер (гонку с ним решает `flock`: кто первый взял замок, второй видит не-`pending` и выходит), пишет `confirmed`, чистит бэкапы до 5. `boot-guard` (`DefaultDependencies=no`, `Before=network-pre.target`, `Wants=network-pre.target` — target пассивный, без `Wants` `Before=` не упорядочивает; `ConditionPathExists=…/transaction.env`): статус `pending|applying` и `TX_BOOT_ID` ≠ текущему → восстановление файлов (для netplan ещё `netplan generate` — генератор отработал раньше guard'а) со статусом `rolled_back`. `ExtraIpManager.start()` в lifespan: транзакция в `applying` (контейнер убили посреди apply) или `pending` с истёкшим дедлайном → `rollback-unconfirmed`.

**Свой шлюз адреса.** Хостер выдал адрес из другой сети (часто `/32`) со своим шлюзом — ответы с него должны уходить через этот шлюз, а не через шлюз основного адреса. Это policy routing, а не конфиг бэкенда: у пяти бэкендов разный синтаксис маршрутов, а ошибка в маршруте netplan стоит сети после ребута; правило в ядре после ребута просто исчезает. На каждую пару (интерфейс, шлюз) — таблица из диапазона `1001–1100` (`GATEWAY_TABLE_MIN/MAX`, одинаковы в Python и скрипте; пул исходящих адресов занимает `101–130`) с `default via <шлюз> dev <iface> onlink` (`onlink` — шлюз вне подсети адреса); на каждый адрес — `ip rule from <адрес> lookup <таблица> priority 1000`. Адреса с общим шлюзом делят таблицу (`plan_routes` переиспользует её, новую берёт `free_gateway_table`; кончились — `400`). Перед ними одно правило на семейство `priority 999 lookup main suppress_prefixlength 0`: любой не-default маршрут основной таблицы (своя подсеть, docker-мосты, туннели) по-прежнему побеждает, по источнику выбирается только default. Трафик без привязки к адресу идёт через основной шлюз, как раньше. Шлюз, совпадающий со шлюзом default-маршрута, приравнивается к «без шлюза» (`without_default_gateways`) — своя таблица ему не нужна. Желаемое состояние — `routes.list`; `routes_sync` в скрипте приводит ядро к нему декларативно: ставит недостающее, снимает свои правила (приоритет 1000, таблицы диапазона) и таблицы, которых в списке нет, suppress-правило — когда у семейства не осталось адресов; в устоявшемся состоянии ничего не пишет. Вызывается из `apply`, отката (live), `restore-runtime` (`mon-extra-ips.service` включён, пока `managed.list` или `routes.list` не пусты; адреса, уже поставленные бэкендом, пропускаются) и `sync-routes` — цикл агента (`ROUTES_SYNC_INTERVAL_SEC = 30`, только когда `routes.list` не пуст; `flock -n` — идущая транзакция синхронизирует сама; вывод `ROUTES_CHANGED=n`, ненулевой — warning в логе). Шлюз меняется только удалением и повторным добавлением адреса. Известное ограничение: если оператор сам держит правила с приоритетом 1000 и таблицами 1001–1100 или такое же suppress-правило с приоритетом 999, нода считает их своими.

**Файлы состояния `/opt/monitoring/network/`** (каталог ro-mount в контейнере — Python читает напрямую, пишет только скрипт; строчные форматы без jq/python): `managed.list` (`<iface> <addr>/<prefix>`), `routes.list` (`<iface> <addr> <gateway> <table>`), `transaction.env` (`TX_ID, TX_STATUS(applying|pending|confirmed|rolled_back|failed), TX_IFACE, TX_BACKEND, TX_ADD, TX_REMOVE, TX_STARTED_AT/TX_DEADLINE_AT/TX_FINISHED_AT` (epoch, в API — ISO UTC), `TX_TIMER`, `TX_BOOT_ID`, `TX_MESSAGE`, `TX_WARNINGS`), `history.log` (TSV, скрипт обрезает до 200, API отдаёт последние 20 новыми сверху), `backups/<tx>/`.

**Guard'ы (`check_request`)** до любого касания хоста: интерфейс — из списка способных нести адрес (`list_address_interfaces`, `net_interfaces.py`) и up; remove — только из `managed.list`, не `protected`, не primary; add уже стоящего чужого адреса — отказ, стоящего нашего — молча пропускается (идемпотентность), но с другим шлюзом — отказ с подсказкой удалить и добавить заново; адрес на другом интерфейсе — отказ; шлюз — адрес этого хоста — отказ; ≤256 адресов за транзакцию, `add ∩ remove = ∅`, шлюз того же семейства, что адрес, не сам адрес и не другой добавляемый адрес, link-local разрешён (модель). Роутер `routers/network.py` (prefix `/api/system/network` → домен `system` без правки карт capabilities): guard'ы → 400, занято → 409. nginx: `location /api/system/network/` с `proxy_read/send_timeout 180s` — `netplan apply` на медленном хосте до ~30 с плюс проверка DAD; таймаут не меньше `APPLY_TIMEOUT_SECONDS` панели (тест-инвариант с обеих сторон).

**Файлы:** `node/app/services/host_extra_ips.sh`, `node/app/services/extra_ips.py`, `node/app/services/net_interfaces.py`, `node/app/models/network.py`, `node/app/routers/network.py`, `node/app/main.py` (роутер + `start()`), `node/nginx/templates/api.conf.template`. Тесты — `node/tests/test_extra_ips.py`: разбор `ip -j addr`/маршрутов (со шлюзом) и primary, резолв netplan-определений (имя/set-name/MAC/обёртка `network:`), выбор бэкенда, рендеры (netplan по разделам, networkd drop-in, ifupdown-стансы, `splice` — вставка/замена/удаление без изменения байт вне блока), guard'ы (в т.ч. шлюзов), раскладка шлюзов по таблицам, round-trip файлов состояния и history, план и разбор вывода скрипта по кодам выхода, `bash -n` и наличие verbs/`AccuracySec=1s`, поля юнитов, инвариант nginx-таймаута, `ExtraIpManager` на FakeExecutor. `node/tests/test_extra_ips_netns.py` — скрипт на настоящем ядре в `unshare --user --net --mount` (без root, без импорта `app`): синхронизация и её идемпотентность, маршрутизация по источнику (свой шлюз, своя подсеть напрямую, основной шлюз для остального), самолечение, уборка лишнего, apply → rollback/confirm, удаление адреса вместе с маршрутом, `restore-runtime`, провал проверки на упавшем линке; второй тест — живой трафик через два роутера-namespace (основной шлюз не знает клиента, клиент стоит за шлюзом доп. адреса): без своего шлюза ответов нет, со шлюзом есть, основной адрес не задет, самолечение возвращает ответы; без user namespace (AppArmor Ubuntu 24.04) пропускается.

### Доступы хостера (`hoster_access.py`)

Разведка и вырезание средств доступа хостера внутрь арендованной ВМ: qemu-guest-agent (гипервизор через virtio-порт выполняет root-команды и сбрасывает пароль), cloud-init (на перезагрузке заново пишет пароль/ключи/юзеров из seed или метаданных), мониторинг-агенты, чужие SSH-ключи, лишние sudo-юзеры, apt-зеркала хостера и `trusted=yes`, ослабленный sshd. Закрывает доступ **изнутри гостя**; уровень гипервизора (снапшот, rescue) не трогается.

**Разведка — read-only, один заход.** `SCAN_SCRIPT` (`shell="bash"`, только чтение) за один `HostExecutor.execute` выдаёт дамп по секционным маркерам `@@`; `parse_scan_output()` разбирает его в `HostFacts` (чистая функция), `detect(facts)` строит находки. Данные передаются base64 (ключи, содержимое sudoers/apt/dropin) — без проблем с экранированием. Отпечатки SSH-ключей считает `ssh-keygen -lf` в самом скрипте, к каждому ключу прилагается base64 исходной строки для точечного удаления.

**Категории** (`DetectedItem.category`): `qemu_guest_agent`, `cloud_init`, `monitoring_agent` (allow-list `MONITORING_AGENTS` по пакету **или** systemd-юниту — google-guest-agent Яндекса виден только юнитом; наши сервисы под allow-list не попадают физически), `foreign_ssh_key` (хостерские по маркерам `HOSTER_KEY_MARKERS` — `default_selected`; чужой ключ без маркеров показывается, но по умолчанию не выбран — защита своего ключа), `foreign_user` (uid≥1000 с логином; облачные по имени/группе `google-sudoers` выбраны по умолчанию), `hoster_apt_repo`, `sshd_weakening` (парольный вход/root по паролю — `default_selected=false`, чтобы не отрезать себя). `access_critical` помечает то, чьё удаление может лишить аварийного доступа.

**Вырезание — необратимо, требует `confirm`.** `purge(finding_ids)` **заново сканирует** и сопоставляет id с реально найденным: клиент не может удалить произвольный путь, только обнаруженное разведкой прямо сейчас. Диспатч по `kind`: `shell` (`apt-get purge` с `DPkg::Lock::Timeout=60`, `systemctl disable --now`+`mask`, `userdel -r`; qemu-ga ещё `apt-mark unhold`, cloud-init — флаг `/etc/cloud/cloud-init.disabled`, google-агент — снятие `/etc/sudoers.d/google_sudoers`), `ssh_key` (переписывание `authorized_keys` без строки ключа, чтение/запись байт-в-байт через `host_files`), `apt_source` (`neutralize_apt_source` — снятие `trusted=yes`, подмена зеркала хостера на `archive.ubuntu.com`, выброс отдельных 3rd-party репозиториев; отдельный репозиторий определяется по пути не `/ubuntu`). Одна упавшая находка не роняет остальные (граница операции), результат — список `{id, ok, message}`.

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | /api/system/hoster-access/scan | `supported`, `hoster_hint`, `generated_at`, `findings[]` (`id, category, title, detail, severity, access_critical, default_selected, remove_hint`) |
| POST | /api/system/hoster-access/purge | `{finding_ids[], confirm}` (без `confirm`/пустой список → 400) → `{results[], reboot_recommended}` |

Префикс `/api/system/` → домен `system` без правки карт capabilities (scan — `ro`, purge — `rw`). nginx: `location /api/system/hoster-access/` с `proxy_read/send_timeout 300s` — несколько `apt purge` под локом dpkg могут занять минуты. **Файлы:** `node/app/services/hoster_access.py`, `node/app/models/hoster_access.py`, `node/app/routers/hoster_access.py`, `node/nginx/templates/api.conf.template`. Тесты — `node/tests/test_hoster_access.py`: разбор скана, детекция по профилям хостеров (VK/Selectel/Yandex), google-агент по юниту, наши сервисы не помечаются, свой ключ не выбран по умолчанию, `neutralize_apt_source` (trusted=yes, 3rd-party, стоковый источник не трогается), purge на FakeExecutor (неизвестный id, shell-команды, переписывание ключа).
...
**Кандидаты-выходы.** IPv4 default-интерфейса из `ExtraIpManager.state()` (`scope=global`, с флагами `primary`/`managed`) и WARP (TCP-проба `127.0.0.1:9091`). `merge_candidates()` сохраняет порядок пользователя из конфига (`candidates_order`); WARP, которого пользователь не расставлял, ставит первым (при равном счёте он предпочтительнее — через него Google не запоминает трафик за собственными адресами ноды), новые адреса — в конец, исчезнувшие убирает, `candidates_disabled` снимает участие. Порядок = приоритет.

**Проверки (`host_check.sh` → `/opt/monitoring/scripts/exit-proxy-check.sh`, ставится по sha256 как `extra-ips.sh`).** Все запросы — curl с хоста через nsenter: для IP `curl -4 --interface <ip>`, для WARP `curl --socks5-hostname 127.0.0.1:9091`. `probe <kind> <address> <timeout> <base64 payload>` печатает одну строку JSON: трасса `cloudflare.com/cdn-cgi/trace` (`ip`, `warp`; без ответа повторяется один раз — загруженный выход может не ответить с первого раза; нет ответа и во второй — `ok:false`, дальше не идёт), страна по YouTube (`"GL":"XX"` в конфиге страницы, cookie `SOCS=CAI` обходит экран согласия) с подтверждением по подвалу поиска (`country_confirm`, на вердикт не влияет), капча поиска (429 или редирект на `/sorry/`), Gemini (`blocked` по тексту «isn't supported/available in your country/region» либо 403/429, `ok` при 200, иначе `error`), пользовательские проверки (`-L`, вердикт по `block_status`, `block_url_regex` на `url_effective`, `block_regex` на теле, `expect_status`; нет ответа — `status:null`; 403/429/503 с челленджем Cloudflare (`cf_chl_opt`, `_cf_chl`, «just a moment», …) — `status:null` и `detail` «cloudflare challenge, not tested»: хостинговым IP край отдаёт челлендж вместо сайта, это «не знаем», а не блок). `TRACE_URL` переопределяется переменной `EXIT_PROXY_TRACE_URL` — только для тестов. Payload — строки `BUILTIN`/`CHECK` с полями через `\x1f` (табуляция в bash схлопывает пустые поля). Кандидаты проверяются параллельно (семафор 4), таймаут исполнителя `check_timeout×8+15`. `selftest <port> <timeout>` — трасса через собственный socks.

**Счёт и выбор (`selection.py`).** `tally()` считает по кандидату включённые проверки — `CheckTally(passed, failed, unknown, google_failed, reachable)`: провал — страна из `blocked_countries`, капча, Gemini `blocked` (это ещё и `google_failed`), сработавшая пользовательская проверка; «не знаем» — трасса не прошла (`NO_RESULT`, `reachable=False`), страна не определилась, Gemini `error`, проверка без ответа или челлендж Cloudflare. `verdict` (статус, чипы, события): `False` при любом провале, `None` при любом «не знаем», иначе `True`; `urgent` — не отвечает или режет Google. `choose_exit()`: `manual` с валидным `pinned_candidate` → он; иначе ранг `(−passed, google_failed, failed)` по включённым, лучший при равном ранге — первый по приоритету (WARP по умолчанию). Лучший = текущий → остаёмся. Текущий `urgent`, а лучший строго лучше по рангу → смена сразу. Равный ранг среди нездоровых → остаёмся. Иначе (текущий проиграл по своим проверкам, либо равный ему здоровый выше по приоритету) — смена только если `pending` из прошлого прогона совпадает с лучшим, иначе `Decision(current, pending=best)`: одна флаки-проверка или разовый таймаут выход не дёргают. Никто не прошёл всё — всё равно берётся лучший по счёту с причиной `no_healthy` (событие один раз, пока держится).

**SOCKS5 (`socks_server.py`).** RFC 1928 без аутентификации, только `CONNECT` (UDP/BIND → `0x07`; QUIC клиент глушит правилом в Remnawave), ATYP IPv4/домен/IPv6. Выход подставляет менеджер через `route()` на каждое новое соединение: IP → сокет с `IP_BIND_ADDRESS_NO_PORT` (Linux ≥ 4.2, значение 24: порт источника ядро выбирает в `connect()` по четвёрке, а не резервирует в `bind()` «на любое назначение», иначе на один выходной IP 64 512 соединений на все адреса вместе с минутой TIME_WAIT) + `bind((ip, 0))` + `sock_connect` (домен резолвится только в записи семейства адреса), WARP → мини-клиент SOCKS5 к 9091 с передачей домена (DNS внутри туннеля). Relay по 64 КиБ с полузакрытием (EOF с одной стороны не рвёт ответ другой); лимит 25 000 соединений (`MAX_CONNECTIONS`, по два дескриптора на соединение при `nofile` 65 536 контейнера), backlog 4096 (`LISTEN_BACKLOG`: при смене выхода все клиенты переподключаются разом). Каждое соединение помнит свой выход: `drop_connections(except_exit)` рвёт все чужие — при автоматическом и ручном переключении трафик, включая уже открытые сессии, сразу уходит на новый IP.

**Менеджер (`manager.py`, `ExitProxyManager`, singleton, старт в lifespan).** Состояние — `exit_proxy.json` рядом с БД трафика (том агента; конфиг, кандидаты, результаты, текущий выход, self-test, хвост событий ≤200), атомарная запись. `apply_config()`: сохраняет, при первом включении сразу находит адреса и выбирает первого по приоритету — WARP, если он есть (проверки уточнят позже), поднимает/гасит/переносит socks по `enabled`/`port`, пересчитывает выбор по имеющимся результатам (страны, порядок, pin), при смене набора проверок будит цикл. Цикл: первая проверка через 20 с после старта, далее по `interval_minutes`; `run_checks()` под замком — обнаружение → пробы → `health` → `choose_exit` → при смене `current` сброс соединений и событие `switched` → self-test (ожидаемый IP — тот, что Google видел через этот выход, за NAT адрес интерфейса и внешний различаются; для WARP — `warp=on`). События: `switched`, `deferred` (кандидат лучше, смена ждёт подтверждения следующим прогоном; `pending_switch` в статусе и состоянии, ручное переключение сбрасывает), `manual_switch`, `no_healthy` (один раз, пока держится), `recovered`, `started`, `stopped`, `check_failed`. Ошибка пробы одного кандидата — `error` в его результате, цикл не падает; порт занят — `listen_error` в статусе.

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | /api/system/exit-proxy/status | `enabled`, `listening`, `listen_error`, `port`, `current`, `select_mode`, `pinned_candidate`, `pending_switch`, `candidates[]` (+`healthy`, `last_check`), `warp_present`, `check_in_progress`, `last_check_at/error`, `self_test`, `stats` (активные/всего/неудачные соединения), `events` (последние 20), `script_installed` |
...
**Файлы:** `node/app/services/exit_proxy/{models,selection,socks_server,manager}.py`, `host_check.sh`, `node/app/routers/exit_proxy.py`, `node/app/main.py`. Тесты: `test_exit_proxy_selection.py` (слияние с WARP первым, счёт и вердикт, «больше пройденных — выигрывает», приоритет при равном здоровом счёте только после подтверждения, срочная смена при недоступности/бане Google, отложенная при своих проверках, таймаут против блока, manual, «ни одного здорового»), `test_exit_proxy_socks.py` (рукопожатие, CONNECT по IPv4/домену, отказ UDP/auth, сброс соединений только чужого выхода, цепочка через upstream-socks), `test_exit_proxy_manager.py` (FakeExecutor: прогон и выбор, переключение со сбросом живого соединения, `no_healthy` один раз, персистентность, смена порта/выключение, ручное переключение и pin, WARP-кандидат, отложенная смена с событием `deferred` и её сохранение, правка настроек не подтверждает, payload проверок, установка скрипта; `HostScriptTest` — `bash -n` и прогон `probe` настоящим bash против локального HTTP-сервера через `EXIT_PROXY_TRACE_URL`: челлендж Cloudflare → `status:null`, честный 403 → блок, 200 → ok, трасса после 503 повторяется).

### Пул исходящих адресов (`source_pool.py`)

Пространство исходящих TCP-соединений ограничено четвёркой (src ip, src port, dst ip, dst port): на пару «адрес ноды → адрес:порт цели» приходится только диапазон эфемерных портов — 64 512 штук, из которых быстро выдаётся половина (см. «Учёт эфемерных портов»). Нода, через которую десятки тысяч клиентов идут на один популярный адрес, упирается в этот потолок: каждый новый `connect()` заставляет ядро перебирать занятую половину диапазона под спинлоком (`__inet_check_established` в профиле), процессор уходит в system time, а трафик не растёт. Потолок кратен числу исходящих адресов и двигается только их добавлением — SNAT не помогает, потому что переписывает адрес уже после подбора порта.

**Как устроено.** Xray в Remnawave помечает исходящие соединения фиксированным набором меток `MARK_BASE..MARK_BASE+MARK_COUNT-1` (101–130; 30 outbound'ов `freedom` с `sockopt.mark` и балансировщик `roundRobin` — сниппет отдаёт панель). Конфиг один на весь парк, потому что метки везде одинаковы; какой метке какой адрес — решает нода: `build_bindings()` раскладывает метки по адресам **по кругу** (`mark i → адрес i % N`), так 30 меток делятся ровно на любое число адресов, а нода с одним адресом правил не ставит вовсе (любая метка и так уйдёт по основной таблице с него). На каждый адрес — таблица `TABLE_BASE + j` с `default via <gw> dev <iface> src <addr>` (без `via` на point-to-point линках; у адреса со своим шлюзом из «Сетевых адресов» — через него с `onlink`), на каждую метку — `ip rule fwmark M lookup T priority P`. Выбор источника происходит при `connect()` до подбора порта, поэтому порт ищется уже в пустом пространстве другого адреса. Только IPv4 и только TCP: на UDP метки Xray не действуют, а потолок портов касается только TCP.
...
### Учёт эфемерных портов (`ephemeral_ports.py`)

Потолок исходящих TCP-соединений — не на хост, а на четвёрку (src ip, src port, dst ip, dst port): на пару «наш адрес → адрес:порт цели» ядро выдаст не больше портов, чем в `ip_local_port_range` за вычетом `ip_local_reserved_ports`. Релей, гонящий всех клиентов на один backend, и Xray, у которого весь Google идёт на горстку адресов, упираются в этот потолок задолго до нехватки портов на сервере в целом — и упираются молча: `connect()` начинает перебирать занятые порты под спинлоком, процессор уходит в system time, а трафик не растёт. Раздел отвечает на вопрос «сколько осталось до предела» до того, как предел наступит; лечится он теми же двумя способами — добавить исходящий адрес (см. «Пул исходящих адресов») или развести трафик по разным портам цели.

**Откуда данные.** Из тех же `/proc/net/tcp` и `/proc/net/tcp6`, что уже читает `_read_host_connections()` для счётчиков состояний — файлы читаются один раз на оба применения, отдельного прохода по сокетам нет. Считается это в том же потоке, что и остальной системный срез (`asyncio.to_thread`), и попадает под общий 5-секундный кэш `get_system_info()`.

- **Входящие соединения отсеиваются по локальному порту.** У них это порт слушающего сокета, а не выданный ядром эфемерный, и без отсева входящий трафик релея выглядел бы как расход портов. Слушающие порты собираются предпроходом по обоим дампам (`scan_listening_ports`, предфильтр по подстроке `" 0A "`) — знать их нужно до разбора сокетов. Отсев точен, а не эвристичен: занятый слушателем порт ядро эфемерным никому не выдаст.
- **Адреса держатся в hex-виде до самой выдачи.** Разбирать текст каждого сокета на ноде с сотней тысяч соединений дороже, чем прочитать файл; в текст (`decode_address`) переводятся только те ≤32 адреса и ≤10 направлений на адрес, что уходят в ответ. `/proc` печатает каждое 32-битное слово в порядке байтов хоста, поэтому на little-endian байты идут задом наперёд (127.0.0.1 → `0100007F`), а IPv4-mapped IPv6 (`::ffff:a.b.c.d`) сворачивается к тому же адресу, что и в IPv4-дампе, — иначе один адрес ноды считался бы дважды.
- **Остаток считается до конца быстрой половины, а не всего диапазона.** `connect()` (`__inet_hash_connect`) первым проходом перебирает только порты чётности `low` — со случайного смещения шагом 2 — и к другой чётности переходит, лишь провалив весь первый проход; вторая половина по умолчанию достаётся `bind()`. Пока на направлении есть свободный порт первого прохода, поиск укладывается в несколько шагов; когда он кончился, каждое новое соединение перебирает всю половину, и процессор уходит в system time при свободной второй половине. Поэтому сокет относится к быстрой половине по чётности своего порта (`KernelPortSettings.is_fast_port`), `fast_capacity` — её размер минус зарезервированные порты той же чётности (`count_reserved_fast`), а `free` направления — остаток до её конца. Ядро округляет длину диапазона вниз до чётной, поэтому при нечётной длине верхний порт в первый проход не входит (`fast_pass_high`). Проверено на живом ядре: 400 соединений к одному адресу:порту получили 400 портов чётности `low`, на реальной ноде `ss` показал то же.
- **`tcp_tw_reuse` учитывается.** При значении `1` ядро отдаёт порты из TIME_WAIT новым исходящим сразу, и занятыми они не считаются; при `2` (дефолт ядра) — только для loopback; при `0` порт занят до истечения таймера. Поэтому `held`, `fast_held` и `free` считает нода, а не панель: иначе интерфейс вычитал бы TIME_WAIT там, где ядро его уже переиспользует.
- **Направления ранжируются по занятому в быстрой половине, а не по числу сокетов.** При `tcp_tw_reuse=1` направление с горой TIME_WAIT потолок не подпирает, а порты второй половины не приближают полный перебор — первым в списке должно стоять то, которому до него реально ближе всех; интерфейс берёт его как показатель адреса.
- **Остатка у самого адреса нет** — он есть только у направления: адрес с тремя направлениями суммарно держит втрое больше портов, чем потолок, и «свободно» на уровне адреса было бы числом, которое ничего не значит.

**Что отдаётся** (`system.ephemeral_ports` в `/api/metrics`): `range_low`/`range_high` и `reserved` (сколько зарезервированных портов попало в диапазон), `capacity` — весь диапазон за вычетом резерва, `fast_capacity` — быстрая половина, `tw_reuse`, и `sources[]` — до 32 исходящих адресов по убыванию занятости, у каждого `used`/`time_wait`/`destinations_total` и до 10 направлений (`ip`, `port`, `used`, `time_wait`, `held` — занято во всём диапазоне, `fast_held` — в быстрой половине, `free` — остаток быстрой половины). Ошибка разбора гасится с записью в лог и `null` в поле: `/api/metrics` — единственный признак живости ноды, и учёт портов не стоит того, чтобы из-за него сервер уходил в offline (та же причина, что у счётчиков портов и температур).

**Цена.** На синтетическом дампе в 120 000 сокетов прирост к текущему проходу — около 90 мс на сбор (предпроход по LISTEN плюс агрегация); при кэше в 5 с и опросе раз в 10 с это меньше процента одного ядра. Отдельного фонового семплера, как у счётчиков портов, не нужно: системных вызовов, ждущих чужую блокировку, здесь нет — только чтение `/proc`.

**Файлы:** `node/app/services/ephemeral_ports.py`, вызов — в `_read_host_connections()`/`get_system_info()` (`metrics_collector.py`), модели `EphemeralPorts`/`EphemeralSource`/`EphemeralDestination` (`models/metrics.py`), тесты — `node/tests/test_ephemeral_ports.py` (37 тестов: разбор адресов в порядке байтов хоста и IPv6, чтение диапазона и подсчёт резерва внутри него и в быстрой половине, быстрая половина исчерпана при наполовину свободном диапазоне, порты второй половины её не расходуют, нечётная длина диапазона, чётность от нижней границы, фолбэк на дефолты ядра, отсев входящих на слушателе, портов вне диапазона и неподключённых сокетов, сворачивание IPv4-mapped к тому же источнику, три режима `tcp_tw_reuse`, ранжирование направлений по занятому в быстрой половине, кламп остатка и потолки выдачи).
### Анти-DDoS
