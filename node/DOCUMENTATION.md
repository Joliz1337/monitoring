...
**Ограничения:** только IPv4; локально сгенерированный трафик самой ноды (OUTPUT) не пробрасывается — только транзитный через PREROUTING; у `both` строки tcp и udp несут одну метку и в счётчиках суммируются. Анти-DDoS (цепочка `ANTIDDOS` в INPUT, SYNPROXY, hashlimit) проброшенного трафика не касается — он идёт PREROUTING → FORWARD; `--notrack` watchdog ставит только на порты слушающих сокетов, DNAT-порты под него не попадают, если только не совпадают с портом локального сервиса.
