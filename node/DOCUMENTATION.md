...
`_compute_maxconn()` (`haproxy_manager.py`): `MAXCONN_PER_RAM_MB = 10` соединений на МБ RAM — худший случай 2×16 КБ буфера (клиент+сервер) на соединение даёт HAProxy занять не больше ~40% памяти хоста. Дополнительно ограничен `fs.nr_open`: `(nr_open − 1024) // 3` (~2 fd на соединение с запасом) — без этого ограничения HAProxy может не стартовать под systemd strict-limits, если `maxconn` выше доступного файлового лимита. Итог зажат в `[MAXCONN_MIN = 10000, MAXCONN_MAX = 500000]`.
