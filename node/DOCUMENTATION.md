...
- **`tcp_tw_reuse` учитывается.** При значении `1` ядро отдаёт порты из TIME_WAIT новым исходящим сразу, и занятыми они не считаются; при `2` (дефолт ядра) — только для loopback; при `0` порт занят до истечения таймера. Поэтому `held` и `free` считает нода, а не панель: иначе интерфейс вычитал бы TIME_WAIT там, где ядро его уже переиспользует.
- **Направления ранжируются по занятому, а не по числу сокетов.** При `tcp_tw_reuse=1` направление с горой TIME_WAIT потолок не подпирает, и первым в списке должно стоять то, которому до потолка реально ближе всех — интерфейс берёт его как показатель адреса.
...
**Что отдаётся** (`system.ephemeral_ports` в `/api/metrics`): `range_low`/`range_high` и `reserved` (сколько зарезервированных портов попало в диапазон), `capacity` — потолок на направление, `tw_reuse`, и `sources[]` — до 32 исходящих адресов по убыванию занятости, у каждого `used`/`time_wait`/`destinations_total` и до 10 направлений (`ip`, `port`, `used`, `time_wait`, `held`, `free`). Ошибка разбора гасится с записью в лог и `null` в поле: `/api/metrics` — единственный признак живости ноды, и учёт портов не стоит того, чтобы из-за него сервер уходил в offline (та же причина, что у счётчиков портов и температур).
...
**Файлы:** `node/app/services/ephemeral_ports.py`, вызов — в `_read_host_connections()`/`get_system_info()` (`metrics_collector.py`), модели `EphemeralPorts`/`EphemeralSource`/`EphemeralDestination` (`models/metrics.py`), тесты — `node/tests/test_ephemeral_ports.py` (30 тестов: разбор адресов в порядке байтов хоста и IPv6, чтение диапазона и подсчёт резерва внутри него, фолбэк на дефолты ядра, отсев входящих на слушателе, портов вне диапазона и неподключённых сокетов, сворачивание IPv4-mapped к тому же источнику, три режима `tcp_tw_reuse`, ранжирование направлений по занятому, кламп остатка и потолки выдачи).
### Анти-DDoS
