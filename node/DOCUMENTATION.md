...
- Всего тестов ноды — 539 (`python -m unittest discover -s node/tests`)
...
**Счёт и выбор (`selection.py`).** `tally()` считает по кандидату включённые проверки — `CheckTally(passed, failed, unknown)`: провал — страна из `blocked_countries`, капча, Gemini `blocked`, сработавшая пользовательская проверка; «не знаем» — трасса не прошла (`NO_RESULT`), страна не определилась, Gemini `error`, проверка без ответа или челлендж Cloudflare. `CheckTally.verdict` (статус, чипы, события): `False` при любом провале, `None` при любом «не знаем», иначе `True`. `choose_exit()`: `manual` с валидным `pinned_candidate` → он; иначе ранг `(−passed, failed)` по включённым — выигрывает выход с наибольшим числом пройденных проверок (3 из 3 бьёт 2 из 3), при равном числе пройденных таймаут лучше подтверждённого блока; текущий остаётся, пока никто не набрал ранг строго лучше (одиночный таймаут текущего переключает только на выход, который прошёл больше), среди равных новых — первый по приоритету. Никто не прошёл всё — всё равно берётся лучший по счёту с причиной `no_healthy` (событие один раз, пока держится); отката на первый по приоритету нет.
