...
- **HAProxy**: `apply(content, cpu_count)` вызывается из `apply_config()` следом за `_ensure_global_maxconn` и подставляет `nbthread`/`cpu-map` под комментарий-маркер `# cpu-affinity (auto)` в секции `global` обоих шаблонов (`_generate_base_config()` на ноде, `generate_base_config()` в `panel/backend/app/services/haproxy_config.py`). Ранее сгенерированный блок (между `BLOCK_START`/`BLOCK_END`) вырезается на каждом применении **до** проверки `is_enabled()` — поэтому выключение настройки чистит конфиг от старой привязки при ближайшем применении, а не оставляет её навсегда.
...
- `resolve_app_cpus(value, cpu_count)` (переименована из `resolve_haproxy_cpus` — стала общей для обеих точек применения) — ядра для приложения (все, кроме сетевых) по значению `auto`/явный список (`0,1` или `0-2`)/`off`. Ничего не применяется, если: настройка выключена; маркера нет в конфиге (для HAProxy); значение `off`; ядра не определились; хост меньше `MIN_CPUS_FOR_SPLIT = 4` ядер; приложению осталось бы меньше половины ядер (сетевых ядер неправдоподобно много — это автоматически отключает механизм при полноценном аппаратном multiqueue, где прерывания разложены на все ядра, либо кривой парсинг); либо работает `irqbalance` (перемещает векторы, и статичная привязка со временем разъезжается) — ручной список ядер применяется даже при работающем `irqbalance`, в отличие от `auto`.
...
- Всего тестов ноды — 217 (`python -m unittest discover -s node/tests`)
...
**Версионирование watchdog-скрипта:** константа `WATCHDOG_VERSION` в шапке `ddos-watchdog.sh` (сейчас `"2.2.0"`) — команда `status` возвращает её полем `version`, отдельная команда `version` печатает только её. Значение растёт при изменении логики скрипта; панель сверяет его с версией, установленной на ноде (см. «Установка» ниже).
...
### CPU: замер по дельте счётчиков ядра, без опоры на настенные часы

Коллектор метрик — синглтон, создаётся в `lifespan` до приёма запросов. Per-CPU занятость считается вручную по дельте `psutil.cpu_times(percpu=True)` между двумя снимками, а не через `psutil.cpu_percent()` — временной интервал берётся из самих счётчиков ядра, а не из `time.monotonic()`, который поток мог прочитать до простоя (тред-пул, конкурентный запрос метрик на том же локе) и получить заниженную реальную дельту.

- `_per_cpu_from(before, after)` — classmethod, доля busy-времени каждого ядра между двумя снимками `cpu_times`. Если суммарная дельта тиков любого ядра меньше `CPU_MIN_TICKS_SECONDS = 0.2`с — возвращает `None` целиком, а не по ядрам: тик ядра — 10мс, и на дельте в единицы тиков доля busy вырождается в точные 0/33.3/50/66.7/100 («одно ядро 100%, остальные 0») — этому нельзя доверять ни для одного ядра, если хоть одно отстало. Результат ограничен `[0, 100]` и округлён до одного знака.
- `prime_cpu_baseline()` — стартовый блокирующий замер при запуске ноды: два снимка `cpu_times` с паузой `CPU_PRIME_SECONDS = 0.3`с между ними, вызывается из `lifespan` (`node/app/main.py`) через `asyncio.to_thread` до приёма запросов — первый же запрос метрик получает реальные значения, а не нули пустого baseline.
- `_sample_per_cpu()` — под `threading.Lock` берёт текущий `cpu_times` и считает дельту от `_prev_cpu_times` через `_per_cpu_from`. Если дельта отбракована (`None`) — baseline **не сдвигается**, отдаётся последний валидный замер; следующий запрос посчитает дельту от того же старого baseline, уже накопив больше тиков — самовосстановление без отдельного порога по времени. Смена числа ядер (ресайз VPS) сбрасывает baseline на текущий снимок с нулевыми процентами — старые счётчики несопоставимы с новым набором ядер.

**Тесты:** `node/tests/test_cpu_sampling.py` — 7 тестов: нормальный интервал даёт реальные проценты, короткая дельта (единицы тиков) отбраковывается целиком, отбраковка срабатывает и при одном отстающем ядре из нескольких, занятость ограничена 100%, отбракованный замер не сдвигает baseline и отдаёт последнее валидное значение, следующий запрос после отбраковки меряет от того же baseline на уже длинной дельте, смена числа ядер сбрасывает baseline.
