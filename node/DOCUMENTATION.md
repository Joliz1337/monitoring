...
- **Remnawave Nginx** — обнаружение установки Remnawave на хосте (`/opt/remnawave` по умолчанию), приём и атомарное применение nginx.conf от панели (backup → in-place запись → `nginx -t` → reload, откат при ошибке), автоподстановка host-специфичных лимитов (`worker_rlimit_nofile`/`worker_connections`/`ssl_session_cache`/`keepalive` XHTTP-пула) под MemTotal/nofile самой ноды, проверка существования сертификатов на хосте с автомонтированием недостающих каталогов в контейнер, валидация конфига через живой или одноразовый контейнер, reload/restart сервиса
- **Синхронизация времени** — установка IANA timezone через `timedatectl` и синхронизация через тот демон времени, что стоит на хосте (`chrony`, `systemd-timesyncd`, `ntpsec`, `ntp`); нет ни одного — ставится `chrony`
...
**Splice-пайп (`tune.pipesize`) — по RAM хоста, тем же механизмом:** шаблон включает безусловный zero-copy (`option splice-request`/`splice-response` — весь трафик релея это длинные TCP-потоки, эвристике `splice-auto` тут нечего угадывать), а размер пайпа вставляет `_ensure_global_pipesize()` в `apply_config()`, если `tune.pipesize` не задан в профиле явно. `_compute_pipesize()`: RAM ≥ 4 ГБ → 262144, ≥ 2 ГБ → 131072, меньше — системный дефолт 64 КБ (`PIPESIZE_RAM_TIERS_MB`) — пайп больше снижает число syscall на гигабитных потоках, но его страницы это невытесняемая память ядра на каждый активный сплайс.
...
| GET | /api/system/network/state | Интерфейсы с адресами (пометки `managed`/`primary`), бэкенд сетевого конфига, текущая транзакция доп. IP и история |
| POST | /api/system/network/apply | Добавить/убрать доп. адреса транзакцией с таймером отката (`{interface, add, remove, protected, rollback_timeout_sec}`) |
| POST | /api/system/network/confirm | Панель снова достучалась до ноды — снять таймер отката (идемпотентно) |
| POST | /api/system/network/rollback | Ручной откат транзакции, ждущей подтверждения |

**`POST /api/system/time-sync`** — значение `timezone` идёт в host-скрипт `app/services/host_time_sync.sh` (переменная `TZ_NAME` первой строкой команды, `build_time_sync_command()` в `app/services/time_sync.py`), исполняемый на хосте от root через `nsenter`, поэтому проверяется трижды: формат ограничен паттерном `^[A-Za-z0-9_+/-]+$` (`TIMEZONE_PATTERN`, набор символов IANA-имён без точки — «..» из каталога зон не выйти), сама подстановка идёт через `shlex.quote`, и до выполнения `timezone_exists_on_host()` сверяет имя с реальным файлом в `/usr/share/zoneinfo` на хосте (`test -f`), а не только с форматом. Скрипт находит установленный демон времени (`chrony` → `systemd-timesyncd` → `ntpsec` → `ntp`, приоритет у активного), без демона ставит `chrony` через `apt-get`, в контейнере (LXC) демон не трогает — часы там принадлежат хосту; затем `set-timezone` → `set-ntp true` → `systemctl restart` демона → до 15 с ждёт `NTPSynchronized=yes`. Таймаут выполнения `TIME_SYNC_TIMEOUT` = 240 с (установка пакета). Ответ (`report_from_result()` → `TimeSyncReport`): `success` — код выхода 0; `ntp_service` — имя демона без `.service` (пусто в контейнере); `ntp_installed` — демон поставлен этим вызовом; `ntp_managed_by_host`; `ntp_enabled`/`ntp_synchronized`/`timezone`/`current_time` — из `timedatectl show`; `errors` — строки stderr, только при ненулевом коде выхода. Подробно о скрипте — в [panel/DOCUMENTATION.md](../panel/DOCUMENTATION.md#синхронизация-времени). Тесты — `tests/test_time_sync.py` (включая `bash -n` скрипта).
...
- Офлайн-образы для нод под жёсткой блокировкой: если pull провалился, но все образы из `docker compose config --images` уже есть на диске (`compose_images_present` в `apply-update.sh` и `deploy.sh`), сборка пропускается — идёт сразу рестарт на локальный образ. Позволяет обновлять/ставить ноду там, где GHCR недоступен: образ переносится вручную (`docker save` на достижимой машине → `docker load` на ноде), тег должен совпадать с `${MON_IMAGE_TAG:-latest}` из `.env`.
...
**Окно за интервал опроса — `GET /api/metrics?window=N`.** Панель передаёт `N` — секунды с прошлого удачного опроса этой ноды (1..`MAX_WINDOW_SEC` = 330); в ответ добавляется блок `window`, а поля выше остаются за последнюю секунду (их читают карточки дашборда и `/metrics/live` панели — без параметра блок равен `null`, ответ не меняется):

- `window_sec` — реально покрытое время (сумма окон вошедших замеров, по monotonic-часам), `samples` — сколько замеров вошло; сразу после старта покрытие может быть короче запрошенного.
- `cpu_avg`, `cpu_max` — среднее и максимум по замерам от секундного среднего по ядрам: `cpu_max` — пик хоста в какую-то секунду, а не самое горячее ядро. `per_cpu_avg` — среднее каждого ядра за окно.
- `net_rx_avg`/`net_tx_avg`/`net_rx_max`/`net_tx_max` — по сумме физических интерфейсов в каждом замере (максимум суммы, не сумма максимумов); `disk_read_avg`/`disk_write_avg` — по `disk_total`.
- Средние взвешены по `window_sec` каждого замера: задержавшийся тик покрыл больше времени и весит больше. Замер, у которого проценты CPU унаследованы от предыдущего (слишком короткое окно тиков), в CPU-часть окна не входит; замеры с другим числом ядер (ресайз VPS внутри окна) — тоже.
...
**Автоподстановка host-специфичных лимитов (`# auto: node`) — прямой аналог `_ensure_global_maxconn` у HAProxy.** Профиль от панели один на много нод с разной RAM/nofile, поэтому строки шаблона, помеченные маркером `AUTO_MARKER = "# auto: node"` (`worker_rlimit_nofile`, `worker_connections`, `ssl_session_cache` в главном/http-контексте и `keepalive` в каждом `upstream xhttp_*`), нода пересчитывает под свой хост при каждом применении. Если оператор вручную убрал маркер — строка считается заданной явно и не трогается. Регэксп подстановки требует пробел после имени директивы, поэтому `keepalive_requests`/`keepalive_timeout` (оканчиваются на `_…`) под `keepalive` не подпадают.

- `compute_host_limits(compose_nofile)` — `worker_rlimit_nofile` = `DOCKER_NOFILE` (иначе `NOFILE_LIMIT`) из `/opt/monitoring/configs/tuning-facts.env`; при недоступности facts — `ulimits.nofile.soft` из docker-compose.yml установки, иначе 65536. `worker_connections` = nofile/4 (2 файловых дескриптора на проксируемое соединение с запасом), clamp `[1024, 65536]`. `ssl_session_cache` = MemTotal_MB/100 МБ, clamp `[10, 100]`. `keepalive` XHTTP-пула = `worker_connections/64`, clamp `[64, 512]` — при packet-by-packet аплоаде пул из 64 не покрывает крупную ноду, и запросы начинают открывать новые коннекты к `127.0.0.1` с TIME_WAIT на loopback.
...
- `nginx_service_block(compose)` — текст только сервиса `remnawave-nginx`: от строки монтирования `./nginx.conf` вверх до ключа сервиса (первая строка с отступом меньше, чем у `volumes:`), вниз до следующего ключа того же уровня; без якоря — весь файл. Нужен потому, что `install.sh` отдаёт том `/etc/letsencrypt` сервису `remnanode`, а не nginx, и поиск volume по всему файлу считал сертификаты примонтированными — nginx после пересоздания падал с `cannot load certificate`, а панель получала «received 409».
- `cert_mount_dir(cert_file)` / `is_covered_by_mounts(path, targets)` / `compose_mount_targets(compose)` / `missing_cert_mounts(compose, cert_files)` — вычисляют, какие каталоги сертификатов (включая кастомные пути из опций профиля) не покрыты ни одним volume сервиса `remnawave-nginx` (только его — через `nginx_service_block`). Для путей под `/etc/letsencrypt` монтируется весь корень (`LETSENCRYPT_ROOT`), а не подкаталог `live/{domain}` — файлы там являются симлинками в `../../archive`, и узкий маунт дал бы контейнеру битые ссылки.
...
- `_apply_with_recreate(...)` — сама транзакция: валидация кандидата одноразовым контейнером **до** любых правок (`validate_content(..., force_one_off=True)` — живому контейнеру может не хватать тех самых маунтов, поэтому проверка идёт через `docker run --rm` с добавленными каталогами) → backup+запись пропатченного `docker-compose.yml` (если патчился) → backup+запись `nginx.conf` → `docker compose up -d --force-recreate remnawave-nginx` (смена точки монтирования и новые volume не подхватываются `reload`) → `_container_survived_start()`. Любая неудача на любом шаге — откат обоих файлов и обратное пересоздание контейнера.
- `_container_survived_start()` — проверка после пересоздания. `nginx -t` кандидата порты не биндит, поэтому занятый порт (или другая ошибка рантайма) валит nginx уже в работающем контейнере; `restart: always` тут же поднимает его снова, и `docker exec nginx -t` в этот момент получал бы `unable to upgrade to tcp, received 409` без единой строки причины. Вместо exec нода `_STARTUP_GRACE_SEC` (3 с, опрос раз в `_STARTUP_POLL_SEC` 0.5 с) смотрит `docker inspect` — `{{.State.Status}}|{{.RestartCount}}|{{.State.ExitCode}}`: контейнер должен оставаться `running` с нулём рестартов всё это время. Иначе — до отката, пока контейнер ещё существует, забираются `docker logs --tail 30`, и в сообщение панели уходит статус, код выхода, число рестартов и сам лог (через `explain_validation_error`, чтобы подсказка про сертификаты тоже сработала). Тесты — `node/tests/test_remnawave_nginx_recreate.py`.
...
- `node/nginx/templates/api.conf.template` — `location /api/remnawave/` с таймаутом 120с (apply может включать `docker compose up`)
...
- Всего тестов ноды — 475 (`python -m unittest discover -s node/tests`)
...
**Длительность apply и таймаут nginx.** Правила ставятся по одному `ufw allow ...` через nsenter — около 0.5 с каждое, плюс `reset` и `enable` по ~2 с: профиль на 20 правил применяется ~14 с, и запросы сериализуются `_apply_lock`. Для `/api/firewall/` в `nginx/templates/api.conf.template` выделен свой `location` с `proxy_read_timeout 120s` (равен `APPLY_TIMEOUT_SECONDS` панели) — под общим `location /` (30 с) второй apply, вставший за локом, получал бы 504, хотя нода доводила его до конца: панель писала `failed`, а правила на ноде уже стояли. Тест-инвариант — `panel/backend/tests/test_firewall_apply_timeout.py`.
...
Хостер выдал ноде ещё один адрес или блок адресов — панель вешает их на физический интерфейс так, чтобы они пережили ребут, не заходя по SSH. Формат сетевого конфига у хостеров разный, а ошибка в нём — потеря сервера, поэтому изменение идёт транзакцией: бэкап → запись конфига → живое применение → проверка → таймер отката на хосте. Подтверждение — панель заново достучалась до ноды (`POST confirm`); нет подтверждения к дедлайну — хост сам восстанавливает бэкап. Основной адрес (prefsrc default-маршрута, иначе первый global-адрес семейства не из наших), адреса хостера и адрес, по которому панель ходит на ноду (`protected`), — только чтение: удалить можно лишь то, что добавила панель (`managed.list`).

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | /api/system/network/state | `supported`, `backend`/`backend_detail` (для интерфейса default-маршрута), `default_interface`, `interfaces[]` (способные нести адрес: физические карты без `master`, bond, VLAN, bridge — кроме docker/veth/туннелей; слейвы bond'а и порты bridge'а не показываются, на дедике с bond0 адреса живут на бонде; поле `kind`; default первым; адреса с `family`, `scope`, `managed`, `primary`, `dynamic`; loopback и link-local не показываются), `managed[]`, `transaction`, `history` (≤20), `rollback_timeout_sec` |
| POST | /api/system/network/apply | `{interface, add: [{address, prefix}], remove: [...], protected: [ip], rollback_timeout_sec: 120 (30..600)}` → `{success, transaction_id, status, message, rolled_back, error_log, backend, deadline_at, warnings}`; pending/applying уже есть → `409`; ошибка guard'ов → `400`; провал самого применения — `200` с `success=false` (`rolled_back` — откатились ли) |
| POST | /api/system/network/confirm | `{transaction_id}` → `{success, status, message}`; на уже завершённой транзакции — `200` с её статусом (цикл панели может повторить confirm после потерянного ответа) |
| POST | /api/system/network/rollback | `{transaction_id}` — откат pending-транзакции по кнопке в панели |

**Разделение труда.** Всё, что обязано работать без контейнера агента, — в bash-скрипте на хосте `host_extra_ips.sh` (в образе рядом с `time_sync`, на хост пишется в `/opt/monitoring/scripts/extra-ips.sh` при расхождении sha256 — `ensure_installed()`, лениво при первом apply, вместе с юнитами `mon-extra-ips-guard.service` и `mon-extra-ips.service`): бэкап, запись файлов, применение бэкенда, проверка, таймер, откат, boot-guard, восстановление на fallback. Скрипт конфиги не рендерит — получает от Python план (`KEY=value`, файлы в base64, `printf … | base64 -d | extra-ips.sh apply`) и умеет только верби `detect <iface>`, `apply`, `confirm <tx>`, `rollback <tx>`, `rollback-unconfirmed <tx>` (цель таймера), `boot-guard`, `restore-runtime`, `self-test`. Коды выхода: 0 ok, 2 плохой план/статус, 3 занято, 4 применение упало и откатилось, 5 откат тоже упал. Python (`extra_ips.py`) — чтение адресов хоста (`ip -j addr show`; default-маршруты читаются отдельной командой и не влияют на код выхода — на хосте с `ipv6.disable=1` `ip -6 route` падает; недоступность `ip -j` отдаётся в `state` как `supported: false` с причиной, а не пустым списком), валидация (`check_request`), детект бэкенда (`choose_backend`, кэш 60 с — панель поллит состояние каждые 3 с во время транзакции), рендер конфигов, разбор `ip -j addr`/`ip -j route` и файлов состояния, `ExtraIpManager` под `asyncio.Lock`.

**Бэкенды** (порядок детекта — первый, кто владеет интерфейсом): netplan первым — если он описывает интерфейс, нижние слои он же и перегенерирует. Определение в netplan резолвится через merged-вид `netplan get network` по `set-name` → имени → `match.macaddress` (cloud-init пишет `id0:` + `match` + `set-name`; файл с ключом `eth0:` создал бы второе определение того же устройства, и networkd применил бы только одно из них). PyYAML в образе нет — `parse_netplan_definitions()` разбирает только нужные уровни по отступам.

| Бэкенд | Персистенция | Живое применение | Откат |
|---|---|---|---|
| netplan | `/etc/netplan/60-monitoring-extra-ips.yaml` (600; netplan предупреждает о world-readable), раздел тот же, что у определения (`ethernets/bonds/vlans/bridges`), только наши адреса — netplan конкатенирует списки `addresses` из разных файлов, cloud-init перезаписывает лишь свой | `netplan generate` (валидация) → `netplan apply` → явные `ip addr add/del` | restore файла → `generate && apply` + `ip` |
| networkd | drop-in `<file>.network.d/monitoring-extra-ips.conf` (`[Network] Address=`) | `networkctl reload` (не `reconfigure` — тот сбрасывает DHCP) → `ip addr add/del`; runtime-адреса, которые reload сбросил, возвращаются в `readd_lost_static` с предупреждением в `warnings` | rm drop-in → `reload` + `ip` |
| NetworkManager | `nmcli connection modify <con> ±ipv4.addresses/±ipv6.addresses`; бэкап keyfile из `/etc/NetworkManager/system-connections/` и прежних значений свойств; соединение без keyfile в `/etc` (in-memory) не годится — boot-guard не смог бы восстановить | `nmcli device reapply` → `ip`; при `ipv6.method` disabled/ignore добавление IPv6 отклоняется до касания | restore keyfile → `nmcli connection reload` + `reapply`, запасной путь — `connection modify` старыми значениями |
| ifupdown | стансы `iface X inet/inet6 static` + `address a/p` в `/etc/network/interfaces.d/monitoring-extra-ips` (если есть `source`) или огороженный маркерами блок в конце `/etc/network/interfaces` (`splice_ifupdown_block` не трогает байты вне блока); свой файл при детекте исключается | `ifquery --list --allow=auto` (валидация) → `ip`; `ifup/ifdown` не вызываются | restore + `ip` |
| fallback | `managed.list` + `mon-extra-ips.service` (oneshot после network-online, `restore-runtime`); `enable` только здесь, `disable` когда список опустел | `ip addr add/del` | `ip` |

**Транзакция (`apply`).** `flock` на `/opt/monitoring/network/lock` → отказ, если предыдущая `pending|applying` → валидация плана (TX_ID, интерфейс в `/sys/class/net`, адреса, пути только под `/etc/netplan/`, `/etc/systemd/network/`, `/etc/network/`) → снимок `BEFORE` (все global-адреса интерфейса, отдельно не-`dynamic`) и наличие default-маршрутов v4/v6 → бэкап в `backups/<tx>/` (`manifest` путь→копия|ABSENT, `files/`, `managed.list.bak`, `meta.env`) → `TX_STATUS=applying` → запись файлов (tmp + `mv -f`) → бэкенд → явные `ip addr add/del` (idempotent) → проверка до 20×0.5 с: все ADD стоят (IPv6 не `tentative`, `dadfailed` — провал сразу), все `BEFORE \ REMOVE` на месте, REMOVE сняты, default-маршруты, что были, остались; с 6-й попытки пропавшие статические адреса возвращаются вручную. Провал → откат из бэкапа, статус `failed`, exit 4, без таймера. Успех → `managed.list` ← план, `pending` + `TX_DEADLINE_AT`, таймер: `systemd-run --unit=mon-extra-ips-rollback-<tx> --on-active=N --timer-property=AccuracySec=1s` (без `AccuracySec` откат опаздывал бы до минуты); без `systemd-run` — `nohup setsid sh -c 'sleep N; …'` (pid в `TX_TIMER`; такой таймер живёт в cgroup контейнера — просроченную pending-транзакцию добивает `start()` агента). `confirm` останавливает таймер (гонку с ним решает `flock`: кто первый взял замок, второй видит не-`pending` и выходит), пишет `confirmed`, чистит бэкапы до 5. `boot-guard` (`DefaultDependencies=no`, `Before=network-pre.target`, `Wants=network-pre.target` — target пассивный, без `Wants` `Before=` не упорядочивает; `ConditionPathExists=…/transaction.env`): статус `pending|applying` и `TX_BOOT_ID` ≠ текущему → восстановление файлов (для netplan ещё `netplan generate` — генератор отработал раньше guard'а) со статусом `rolled_back`. `ExtraIpManager.start()` в lifespan: транзакция в `applying` (контейнер убили посреди apply) или `pending` с истёкшим дедлайном → `rollback-unconfirmed`.

**Файлы состояния `/opt/monitoring/network/`** (каталог ro-mount в контейнере — Python читает напрямую, пишет только скрипт; строчные форматы без jq/python): `managed.list` (`<iface> <addr>/<prefix>`), `transaction.env` (`TX_ID, TX_STATUS(applying|pending|confirmed|rolled_back|failed), TX_IFACE, TX_BACKEND, TX_ADD, TX_REMOVE, TX_STARTED_AT/TX_DEADLINE_AT/TX_FINISHED_AT` (epoch, в API — ISO UTC), `TX_TIMER`, `TX_BOOT_ID`, `TX_MESSAGE`, `TX_WARNINGS`), `history.log` (TSV, скрипт обрезает до 200, API отдаёт последние 20 новыми сверху), `backups/<tx>/`.

**Guard'ы (`check_request`)** до любого касания хоста: интерфейс — из списка способных нести адрес (`list_address_interfaces`, `net_interfaces.py`) и up; remove — только из `managed.list`, не `protected`, не primary; add уже стоящего чужого адреса — отказ, стоящего нашего — молча пропускается (идемпотентность), адрес на другом интерфейсе — отказ; ≤256 адресов за транзакцию, `add ∩ remove = ∅` (модель). Роутер `routers/network.py` (prefix `/api/system/network` → домен `system` без правки карт capabilities): guard'ы → 400, занято → 409. nginx: `location /api/system/network/` с `proxy_read/send_timeout 180s` — `netplan apply` на медленном хосте до ~30 с плюс проверка DAD; таймаут не меньше `APPLY_TIMEOUT_SECONDS` панели (тест-инвариант с обеих сторон).
...
- Кольцевой буфер `deque(maxlen=MAX_WINDOW_SEC)` хранит последние замеры (~330 с; на 64 ядрах — около 1.5 МБ). `window(seconds)` идёт от новейшего замера к старым, накапливая `window_sec`, пока покрытие меньше запрошенного, и отдаёт кортеж в том же порядке — от новейшего к старому (`samples[0]` — последний замер, по нему `summarize_window()` берёт число ядер); `None` — когда `snapshot()` тоже `None`. Вызывается, как и `advance()`, только из потока event loop — лок на deque не нужен. Сводку за окно считает чистая функция `summarize_window()` (взвешенные средние, максимумы, отбор замеров с измеренным CPU и совпадающим числом ядер).

Зависимости (`read_counters`, `is_whole_disk`, `clock`) инжектируются в конструктор — тесты идут без psutil и без патчей.

**Тесты:** `node/tests/test_rate_sampler.py` — 38 тестов: проценты CPU (реальные значения, отбраковка коротких дельт целиком и при одном отстающем ядре, потолок 100%), рейты сети/диска по дельте и окну, счётчик «назад» → 0, появившийся и исчезнувший интерфейс, слишком близкое чтение не двигает baseline, смена числа ядер, сумма по целым дискам и фолбэк, `sampled_at`, протухание; буфер ограничен `MAX_WINDOW_SEC`, `window()` отбирает по покрытию (тик в 2 с — меньше сэмплов), короткий буфер отдаёт всё, протухший семплер → `None`, флаг `cpu_measured` при измерении/наследовании/смене ядер, `weighted_mean`, `summarize_window` (взвешивание, `cpu_max` — пик среднего по ядрам, исключение унаследованных сэмплов и сэмплов с другим числом ядер, сеть по сумме физических с пиком по сумме, диск по `disk_total`). `node/tests/test_metrics_live_rates.py` — 15 тестов: коллектор копирует рейты семплера в интерфейсы/total (только физические), диски/`io_total`, CPU; без сэмпла — нули и `live_rates: null`; блок `window` переживает `AllMetrics.model_validate(...).model_dump()`, без параметра равен `None`, набор ключей совпадает с `WindowRates`, `physical_interfaces()` исключает виртуальные и bond-слейвы.
...
Сам ключ считает и применяет рендерер (`@@RESERVED_PORTS@@` в `common.base.conf`): база — `7500` (внутренний uvicorn), `7501-7564` (локальные socks-порты проверки прокси-конфигураций из панели: ядро поднимается одно на пачку, порт — на каждую проверку в ней, поэтому размер диапазона и есть потолок одновременных проверок на ноде, см. `configs/xray-test-runner.sh`), `NODE_API_PORT` из `/opt/monitoring-node/.env` (по умолчанию 9100, у рендерера — `detect_node_api_port()`, переопределяется `MON_FACT_NODE_API_PORT` в тестах) и `2222` (SSH ноды Remnawave) — плюс дополнительные порты из `/opt/monitoring/configs/reserved-ports.conf` (порты и диапазоны `a-b`, построчно или через запятую, `#`-комментарии). Итог канонизируется в формат ядра (сортировка, слияние пересечений и смежных, диапазоны `a-b`) — иначе `verify` сравнивал бы нашу строку с ядровой и всегда видел бы расхождение; инвариант рендерера отвергает файл, резервирующий суммарно больше 4096 портов. Значение попадает в `computed` в `tuning-facts.json` и проверяется верификацией наравне с остальными.
