...
| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | /api/system/network/state | `supported`, `backend`/`backend_detail` (для интерфейса default-маршрута), `default_interface`, `interfaces[]` (физические, default первым; адреса с `family`, `scope`, `managed`, `primary`, `dynamic`; loopback и link-local не показываются), `managed[]`, `transaction`, `history` (≤20), `rollback_timeout_sec` |
