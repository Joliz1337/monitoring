...
# One-time: the node-local certbot cron is gone. It ran `systemctl stop haproxy`
# unconditionally every night at 03:00 — dropping every established tunnel on
# every port, even though certbot only renews certs expiring within 30 days.
# Renewal is operator-driven now (HAProxy page in the panel).
# This must run on the HOST: the container has no /etc/cron.d mount, so the
# Python API could never remove the cron file itself. Do not "optimise" this
# back into Python. Exact filenames only — the Debian certbot package ships
# /etc/cron.d/certbot and certbot.timer, which are none of our business.
# Guarded: inside the container /etc/cron.d is not mounted but
# /opt/monitoring-node is, so an unguarded run would delete the script and leave
# the host cron firing at a missing file every night.
if [ $IN_CONTAINER -eq 0 ]; then
