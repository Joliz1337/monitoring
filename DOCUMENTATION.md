...
Раньше профили `configs/vpn/sysctl.conf` и `configs/panel/sysctl.conf` были наборами плоских констант, одинаковых для сервера на 4 ГБ и на 248 ГБ — на маленьких хостах это были OOM-векторы (`nf_conntrack_max=2097152` ≈ 670 МБ одной только таблицы), на больших — недокрут (`fs.nr_open=524288` ломал контейнеры с бо́льшим `ulimits`). Теперь единственный владелец всех производных чисел — `configs/tune-sysctl.sh`: он читает MemTotal/nproc/MTU/скорость линка самого хоста и пересчитывает conntrack, буферы, лимиты дескрипторов и HAProxy `maxconn` при каждом применении.

Верби: `facts` (посчитать и напечатать факты), `render` (посчитать → отрендерить все выходные файлы → `sysctl -e -p`), `verify` (сверить живые значения с записанными фактами), `rollback` (восстановить `.prev`), `self-test` (прогнать внутренние инварианты). Значения (включая `tcp_mem`/`udp_mem`/`vm.min_free_kbytes`) пишутся в файл, а не применяются транзиентно — раньше их сбрасывал к дефолтам ядра любой сторонний `sysctl --system`. Рендерер отказывается писать при неразрешённом токене, дублирующемся ключе в базовых файлах или нарушении числовой цепочки (например `fs.nr_open` ниже лимита HAProxy) — предыдущий конфиг остаётся в силе.
...
Опорные значения для 2 CPU/4 ГБ → 8 CPU/16 ГБ → 64 CPU/248 ГБ: `nf_conntrack_max` 524288/2097152/4194304, `fs.nr_open` 131072/524288/2097152, `rmem_max` 32/64/64 МБ, HAProxy `maxconn` 39000/159000/500000. Суммарный потолок сокетной памяти (`tcp_mem`+`udp_mem`) — 25% RAM (было ~50%).
...
`configs/vpn/`, `configs/panel/`, `configs/sysctl.conf`, `configs/limits.conf`, `configs/systemd-limits.conf` удалены — рендерер и `configs/profiles/` заменили их полностью.
...
2. **Аварийный режим («под атакой»)** — набор iptables-правил в отдельной цепочке `ANTIDDOS` (не в raw INPUT — переживает `ufw --force reset` от Firewall Profiles), порядок фиксирован документированной схемой netfilter: blocklist ACCEPT/DROP → whitelist ACCEPT → established ACCEPT → SSH (автоопределяется)/nginx mTLS API(9100)/внутренний uvicorn-API ноды(7500) ACCEPT (никогда не дропаются) → **SYNPROXY** на автоопределённые клиентские порты (проверка TCP-рукопожатия до создания conntrack-записи — гасит SYN-флуд со спуфнутых IP; `--wscale`/`--mss` вычисляются рендерером из реальных `rmem_max` и MTU, не захардкожены) → drop INVALID → не-SYN пакеты в состоянии NEW → hashlimit (лимит новых соединений/сек с одного IP, общая таблица `ad_emg`, маска `HASHLIMIT_SRCMASK=32`). Raw-правила `--notrack` ставятся **последними** и только если SYNPROXY принят в каждой группе портов — при частичном отказе снимаются. `iptables -m multiport` принимает не более 15 портов на правило, поэтому клиентские порты разбиваются на группы по ≤15. Джамп из INPUT в `ANTIDDOS` ставится только пока режим активен.
   ...
   До версии 2.0.0 проверка INVALID стояла **до** SYNPROXY — снятый в raw-таблице с трекинга SYN не давал завершающему ACK клиента найти запись conntrack, тот считался INVALID и терялся до SYNPROXY, то есть аварийный режим был полным блэкхолом новых соединений на всё время атаки. `connlimit` убран полностью: он обходит conntrack-бакет источника на каждом NEW-пакете (дорожает ровно во время атаки) и карает CGNAT-адреса операторов; скорость ограничивает hashlimit, стоячее количество — conntrack-таймауты.
