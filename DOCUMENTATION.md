...
**Мастер-переключатель «Защита включена» (`AntiDdosSettings.enabled`)** — реальный рубильник всего парка, а не просто флаг для фоновых циклов панели: при выключении панель в фоне снимает аварийный режим и выключает автодетект на всех активных нодах, весь парк возвращается в дежурный режим; при включении — включает автодетект обратно на всех нодах. Тонкая настройка остаётся на уровне отдельных нод: тумблеры «Автодетект»/«Аварийный режим», ручная кнопка «Я под атакой», глобальные «Включить/выключить аварийный на всех».
...
- [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md) — веб-панель: API, БД, конфигурация, безопасность (v10.15.2)
