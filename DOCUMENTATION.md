...
│   ├── nginx/
│   └── docker-compose.yml
├── node/              # API-агент на каждой ноде (FastAPI, без своей БД)
│   ├── DOCUMENTATION.md
│   ├── app/
│   ├── nginx/
│   └── docker-compose.yml
...
├── .github/
│   └── screenshots/   # PNG-скриншоты 8 разделов панели + логотип для README/README.en (сняты с dev-сборки фронтенда на вымышленных данных; README.md в папке — чек-лист кадров и рекомендации по съёмке)
├── install.sh         # Главный установщик
├── README.md          # Публичное описание проекта (русский)
├── README.en.md       # Публичное описание проекта (английское зеркало README.md)
├── CHANGES.md         # Публичная история изменений простым языком (не путать с CHANGELOG.md ниже)
├── CHANGELOG.md       # Внутренний технический changelog (в .gitignore, не публикуется)
...
Вкладка панели **«Xray тест»** (иконка колбы в меню) — проверка прокси-конфигураций: ссылки любых протоколов, готовые JSON-конфиги и подписки прогоняются через настоящее ядро (Xray или sing-box) с панели или с любой ноды, с мульти-SNI и экспортом рабочих ключей. Подробности — в [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md#xray-тест).

### CLI-обёртка `mon`

`install_cli()` устанавливает `/usr/local/bin/mon` (`BIN_PATH`) — тонкую обёртку, которая при каждом запуске скачивает `install.sh` с GitHub во временный файл (`mktemp /tmp/mon-installer.XXXXXX`, удаляется по `trap ... EXIT`) и запускает `bash "$INSTALLER" "$@"`; при недоступности GitHub — фолбэк на локальную копию `/opt/monitoring-panel/install.sh` или `/opt/monitoring-node/install.sh`. Установщик передаётся файлом, а не текстом через `bash -c`, потому что его размер превышает `MAX_ARG_STRLEN` — лимит ядра Linux на длину одного аргумента (128 КБ). URL обёртки строится из `MON_BRANCH` (плейсхолдер `@@BRANCH@@` в heredoc-шаблоне подставляется после генерации, чтобы не раскрывался как переменная самой обёртки) — на сервере, установленном с dev-канала, `mon` тянет установщик из `dev`, а не всегда из `main`.

Содержимое обёртки в `install_cli()` задано heredoc-блоком; файл перезаписывается только если отличается от эталонного текста, поэтому повторные запуски установщика на неизменённом сервере не производят лишних действий. `main()` вызывает `install_cli()` при каждом запуске установщика (не только при отсутствии файла) — устаревшая или повреждённая обёртка `mon` на уже установленном сервере автоматически приводится к актуальному виду. `scripts/install-mon-cli.sh [main|dev]` — отдельный скрипт, генерирующий тот же текст `mon` для установки CLI без полного `install.sh`; ветка берётся из аргумента, иначе из `MON_BRANCH`, иначе `main`.

Тот же приём — скачивание установщика во временный файл вместо `bash -c` — использует firstboot-скрипт Hetzner Rescue (см. ниже).
...
Входные файлы — `configs/profiles/common.base.conf` (общие ключи) + `vpn.base.conf`/`panel.base.conf` (только поведенческие отличия: короткие таймауты conntrack на VPN-ноде против долгоживущих исходящих сессий панели; пол эфемерного диапазона — 1024 у vpn против 9101 у panel) + `limits.tmpl`/`systemd-limits.tmpl`. Сервисные порты рендерер исключает из эфемерной выдачи через `net.ipv4.ip_local_reserved_ports`: база — 7500, 7501-7532 (проверка прокси-конфигураций из панели), `NODE_API_PORT` из `.env` ноды, 2222 (Remnawave) — плюс доп. порты из `/opt/monitoring/configs/reserved-ports.conf`, которыми управляет панель (общий список парка и списки отдельных нод); подробности — в [node/DOCUMENTATION.md](node/DOCUMENTATION.md#резервация-портов-от-эфемерной-выдачи). Вычисляемые значения помечены токенами `@@TOKEN@@` и подставляются рендерером внутрь базового файла, а не дописываются отдельным блоком — `sysctl` берёт последнее вхождение ключа, и дубль молча менял бы смысл в зависимости от порядка строк. Оператор может переопределить любое значение через `/opt/monitoring/configs/local-overrides.conf` — рендерер его не трогает и подключает последним.
...
Опорные значения для 2 CPU/4 ГБ → 8 CPU/16 ГБ → 64 CPU/248 ГБ: `nf_conntrack_max` 524288/2097152/4194304, `fs.nr_open` 131072/524288/2097152, `rmem_max` 32/64/64 МБ, HAProxy `maxconn` 39000/159000/500000. Суммарный потолок сокетной памяти (`tcp_mem`+`udp_mem`) — 25% RAM.
...
| `--api-port=N` | Экспортирует `NODE_API_PORT` — кастомный порт mTLS-API ноды (по умолчанию 9100) |
...
`apply_system_optimizations()` принимает опциональные аргументы `(profile, nic_mode)` — при их наличии интерактивное меню пропускается, без аргументов (пункт 7 меню) показывается интерактивное подменю. `cleanup_temp` выполняется в самом конце установки, поэтому временные конфиги из репозитория доступны на шаге оптимизаций.

**Функции `install.sh`:**

- `install_warp()` — устанавливает Cloudflare WARP; маппит кодовое имя дистрибутива на поддерживаемый Cloudflare релиз (bullseye/bookworm/jammy/noble) — неподдерживаемые кодовые имена (например Ubuntu 25.04 "plucky") автоматически подменяются на `noble`; после `apt-get install cloudflare-warp` проверяет успешность установки: если пакет не установился или `warp-cli` недоступен — выводит `[ERROR]`, удаляет нерабочий файл `/etc/apt/sources.list.d/cloudflare-client.list` и возвращает код 1; сразу после установки пакета пишет drop-in `/etc/systemd/system/warp-svc.service.d/memory-limit.conf` (`MemoryMax=1G`, `Restart=always`, `RestartSec=5`) и перезапускает `warp-svc` — у демона известна утечка памяти на долгом аптайме, по достижении потолка systemd перезапускает его сам, туннель переподключается автоматически (Always On); сообщения локализованы через ключи `warp_codename_fallback`, `warp_install_failed` и `warp_svc_limit` (MSG_EN и MSG_RU)
...
Многослойная защита от DDoS-атак на нодах (`WATCHDOG_VERSION` 2.4.0), независимая от системного sysctl-тюнинга выше — conntrack-хардening из общего профиля остаётся первой линией обороны. Три слоя:

1. **Дежурный режим** — никаких лимитов по IP в мирное время, нулевые накладные расходы и ложные срабатывания.
2. **Аварийный режим («под атакой»)** — набор iptables-правил в отдельной цепочке `ANTIDDOS` (не в raw INPUT — переживает `ufw --force reset` от Firewall Profiles), порядок фиксирован документированной схемой netfilter: blocklist ACCEPT/DROP → whitelist ACCEPT → established ACCEPT → SSH (автоопределяется)/nginx mTLS API(9100)/внутренний uvicorn-API ноды(7500) ACCEPT (никогда не дропаются) → **SYNPROXY** на автоопределённые клиентские порты (проверка TCP-рукопожатия до создания conntrack-записи — гасит SYN-флуд со спуфнутых IP; `--wscale`/`--mss` вычисляются рендерером из реальных `rmem_max` и MTU, не захардкожены) → drop INVALID → не-SYN пакеты в состоянии NEW → hashlimit (лимит новых соединений/сек с одного IP, общая таблица `ad_emg`, маска `HASHLIMIT_SRCMASK=32`). Raw-правила `--notrack` ставятся **последними** и только если SYNPROXY принят в каждой группе портов — при частичном отказе снимаются. `iptables -m multiport` принимает не более 15 портов на правило, поэтому клиентские порты разбиваются на группы по ≤15. Джамп из INPUT в `ANTIDDOS` ставится только пока режим активен.

   Порядок «SYNPROXY раньше drop INVALID» принципиален: у снятого в raw-таблице с трекинга SYN завершающий ACK клиента не находит записи conntrack и считается INVALID — стой проверка INVALID раньше SYNPROXY, этот ACK терялся бы и аварийный режим был бы полным блэкхолом новых соединений. `connlimit` не используется: он обходил бы conntrack-бакет источника на каждом NEW-пакете (дорожал бы ровно во время атаки) и карал бы CGNAT-адреса операторов; скорость ограничивает hashlimit, стоячее количество — conntrack-таймауты.

**Автоопределение SSH-порта** — never-drop покрывает SSH независимо от того, сменён ли порт с дефолтного 22: `detect_ssh_ports()` в `ddos-watchdog.sh` вычисляет его из директивы `Port` в `sshd_config`/`sshd_config.d/*.conf`, `ListenStream=` в systemd socket-активации (`ssh.socket`, дефолт Ubuntu 24) и живых sshd-листенеров (`ss`), откат на 22 если ничего не найдено. Never-drop = статические management-порты (`9100`, `7500`) + кастомный порт API из `.env` ноды (`NODE_API_PORT`, если задан) + автоопределённые SSH-порты.
3. **Автодетект (watchdog)** — локальный сторож на ноде читает сигналы из `/proc` (реальные дропы conntrack `insert_failed`, заполнение conntrack как слабый намёк, всплеск SyncookiesSent, pps при мелком среднем пакете — считается только по пакетам к самой ноде: транзит FORWARD (`ForwDatagrams` из `/proc/net/snmp`) вычитается, иначе DNAT-релей с сотнями тысяч мелких ACK/SYN клиентов стабильно выглядел бы как флуд, хотя аварийный режим на INPUT ему ничем не помог бы, — softirq%, дропы `/proc/net/softnet_stat` и `ListenOverflows`) с консервативными порогами: сильные сигналы (syncookies, conntrack `insert_failed`) включают режим сразу, слабые — после удержания ~45 с; авто-выключение после ~15 мин тишины. Дропы `/proc/net/softnet_stat` — тоже сильный сигнал, но с двумя оговорками: из дельты дропов вычитается дельта собственного ограничителя ядра `flow_limit_count` (он включается вместе с RPS и режет поток, занявший больше половины истории очереди — на одноочередной сетевой карте один быстрый клиент даёт сотни таких «дропов» за цикл при полностью здоровой ноде), и остаток должен продержаться `SOFTNET_HOLD_CYCLES` циклов подряд (по умолчанию 2, ~20 с), прежде чем включить аварийный режим — одиночный микровсплеск защиту не поднимает. Пороги — не абсолютные константы: `tune-sysctl.sh` пишет их в `/opt/monitoring/antiddos/config.auto` по факту CPU/RAM хоста, оператор может переопределить в `/opt/monitoring/antiddos/config`, который подключается последним; оба файла перечитываются функцией `load_config()` в начале **каждого** цикла, а не один раз при старте процесса — пересчитанные `tune-sysctl.sh` пороги подхватываются в течение одного цикла, без рестарта сервиса. `SOFTIRQ_PCT` считается от нормализованного по CPU агрегата `/proc/stat`, поэтому порог на малых хостах (`CPUS≤4`) выше (70%), чем на больших (50%) — иначе одно занятое ядро на 2-ядерной ноде и половина 64-ядерной означали бы одно и то же. Ручной пин (`source=manual`) автоматика не снимает — только админ. Self-heal: watchdog переставляет джамп в INPUT после стороннего `ufw reset`. Пока аварийный режим активен, цикл каждые ~10 с проверяет доступность ноды (`127.0.0.1:7500/health` + живая SSH-сессия) — 3 подряд неудачи снимают цепочку, чтобы баг в правилах не отрезал доступ к серверу.

   Отладка: верби `dry-run` (печатает точные команды `iptables`, ничего не выполняя) и `self-test` (структурно доказывает, что SYNPROXY стоит раньше INVALID DROP и что raw `--notrack` есть тогда и только тогда, когда есть SYNPROXY).
...
**Whitelist** — отдельный ipset-набор `antiddos_allow` на диске ноды (переживает ребут и недоступность панели). До первого опроса панели засеян IP панели и `127.0.0.0/8`, чтобы атака в этом окне не зарейтлимитила саму панель. Панель наполняет набор ежечасно из трёх частей: авто (IP всех активных нод + IP панели), ручная (CIDR-подсети CDN и т.п., настраиваются в панели) и авто-источники по URL (произвольные списки IP/CIDR, например Cloudflare/Yandex Cloud — панель парсит IPv4/CIDR из ответа независимо от формата). ACCEPT по нему работает только в аварийном режиме.

**Файлы:**
- `configs/ddos-watchdog.sh` — самодостаточный host-скрипт (не зависит от Docker/панели). CLI: `loop` (systemd-сервис — детект + self-heal), `enable-manual`/`disable-manual`, `watchdog-on`/`watchdog-off`, `apply`/`clear`, `selfheal`, `whitelist-sync` (IP через stdin), `detect-ports`, `dry-run`, `self-test`, `version`, `status`. Состояние — `/opt/monitoring/antiddos/state.json` (mode/source/since/reason/watchdog).
- `configs/ddos-watchdog.service` — systemd-unit (`Type=simple`, `Restart=always`, `ExecStart=... loop`).

**Установка — по умолчанию.** Свежая нода получает watchdog напрямую при установке: `install_antiddos_watchdog()` в `install.sh` вызывается сразу после применения системных оптимизаций, ставится в состоянии покоя (`watchdog=on`, аварийный режим выключен). Существующие ноды получает панель: в фоновом опросе статуса (раз в ~60 сек) она сверяет версию установленного скрипта с `WATCHDOG_VERSION` в `ddos-watchdog.sh` на GitHub и сама ставит/обновляет его через `POST /api/antiddos/install`. Через `apply-update.sh` это не идёт — rsync обновления агента исключает `configs/`, так что репозиторных конфигов рядом с нодой нет. Backend-эндпоинты ручной установки (`POST /antiddos/install-all`, `POST /proxy/{id}/antiddos/install`) остаются в API, кнопки в интерфейсе панели нет — установка не требует участия оператора.

**Требования к ядру:** SYNPROXY использует `xt_SYNPROXY`/`nf_synproxy_core` (best-effort — если модули недоступны, SYNPROXY пропускается, hashlimit и drop-invalid остаются), плюс `hashlimit`. На Ubuntu 24 iptables = iptables-nft (тот же стек, что UFW/Docker/ipset_manager).
...
- [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md) — веб-панель: API, БД, конфигурация, безопасность (v10.72.0)
...
Панель опрашивает ноды коллектором метрик и ходит к ним по требованию через proxy-роутер (`/api/proxy/{id}/...`). Собственной базы истории у ноды нет: она отдаёт мгновенный срез хоста, кумулятивные счётчики и скорости за последнюю секунду (CPU, байт/с по интерфейсам и дискам — их считает фоновый посекундный семплер ноды, а не панель по своему интервалу опроса), а по запросу коллектора — ещё и среднее с максимумом за окно с прошлого опроса, чтобы точка истории описывала весь интервал, а не одну секунду. Всю историю (метрики, трафик, интервалы недоступности) считает и хранит панель в PostgreSQL. На диске ноды остаются только конфигурационные файлы — списки блокировок, whitelist анти-DDoS, перечень отслеживаемых портов, набор DNAT-правил.
...
Счётчики снимаются на ноде, арифметика и хранение — на панели:

1. Нода ведёт кумулятивные счётчики: по интерфейсам из `/proc/net/dev` и по портам — в собственных цепочках iptables `TRAFFIC_ACCOUNTING_IN`/`_OUT`, которые фоновая задача опрашивает раз в 30 секунд. Отдаются они одним ответом `GET /api/metrics` вместе с `system.boot_id` и версией агента.
2. Панель считает разность соседних наблюдений, раскладывает её по часовым бакетам пропорционально времени и пишет в таблицу `server_traffic` (часовые и суточные бакеты в разрезах total / интерфейс / порт). Предыдущее значение счётчика лежит в PostgreSQL, поэтому перезапуск панели не теряет базу для расчёта.
3. Периоды, когда нода не отвечала, панель отмечает в `server_downtime` — на графиках там рисуется разрыв, а не ноль.

Единственный источник правды по истории трафика — база панели. Суммарный трафик и разбивку по интерфейсам панель считает для ноды любой версии — кумулятивные счётчики приходят в общем запросе метрик. Разбивка по портам требует агента `10.13.0`: панель определяет это по версии и показывает на странице трафика просьбу обновить ноду вместо таблицы портов. Накопленную такими нодами историю панель переносит к себе один раз и после подтверждения удаляет с ноды. Подробности — в [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md#traffic) и [node/DOCUMENTATION.md](node/DOCUMENTATION.md#traffic).
