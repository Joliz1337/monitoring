...
Потолок исходящих TCP-соединений считается не на сервер, а на четвёрку (наш адрес, наш порт, адрес цели, порт цели): на пару «адрес ноды → адрес:порт цели» ядро выдаст не больше портов, чем в эфемерном диапазоне. Релей, гонящий всех клиентов на один backend, и Xray, у которого весь Google идёт на горстку адресов, упираются в этот потолок при полностью свободном сервере — и упираются молча, отдавая процессор перебору портов вместо трафика. Нода считает занятость по `/proc/net/tcp` в составе метрик, панель показывает её на странице трафика: полоса и остаток на каждый исходящий адрес, внутри — сами направления. Два способа поднять потолок — добавить исходящий адрес (раздел «Исходящие адреса», раскладка через policy routing) или развести трафик по разным портам цели. Подробности — в [node/DOCUMENTATION.md](node/DOCUMENTATION.md#учёт-эфемерных-портов-ephemeral_portspy) и [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md#traffic).
