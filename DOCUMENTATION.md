...
Входные файлы — `configs/profiles/common.base.conf` (общие ключи) + `vpn.base.conf`/`panel.base.conf` (только поведенческие отличия: короткие таймауты conntrack на VPN-ноде против долгоживущих исходящих сессий панели; пол эфемерного диапазона — 1024 у vpn против 9101 у panel) + `limits.tmpl`/`systemd-limits.tmpl`. Сервисные порты рендерер исключает из эфемерной выдачи через `net.ipv4.ip_local_reserved_ports`: база — 7500, 7501-7532 (проверка прокси-конфигураций из панели), `NODE_API_PORT` из `.env` ноды, 2222 (Remnawave) — плюс доп. порты из `/opt/monitoring/configs/reserved-ports.conf`, которыми управляет панель (общий список парка, списки отдельных нод и порт локального SOCKS5 exit-прокси, пока раздел включён); подробности — в [node/DOCUMENTATION.md](node/DOCUMENTATION.md#резервация-портов-от-эфемерной-выдачи). Вычисляемые значения помечены токенами `@@TOKEN@@` и подставляются рендерером внутрь базового файла, а не дописываются отдельным блоком — `sysctl` берёт последнее вхождение ключа, и дубль молча менял бы смысл в зависимости от порядка строк. Оператор может переопределить любое значение через `/opt/monitoring/configs/local-overrides.conf` — рендерер его не трогает и подключает последним.
...
- [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md) — веб-панель: API, БД, конфигурация, безопасность (v10.73.0)
...
Панель опрашивает ноды коллектором метрик и ходит к ним по требованию через proxy-роутер (`/api/proxy/{id}/...`). Собственной базы истории у ноды нет: она отдаёт мгновенный срез хоста, кумулятивные счётчики и скорости за последнюю секунду (CPU, байт/с по интерфейсам и дискам — их считает фоновый посекундный семплер ноды, а не панель по своему интервалу опроса), а по запросу коллектора — ещё и среднее с максимумом за окно с прошлого опроса, чтобы точка истории описывала весь интервал, а не одну секунду. Всю историю (метрики, трафик, интервалы недоступности) считает и хранит панель в PostgreSQL. На диске ноды остаются только конфигурационные файлы — списки блокировок, whitelist анти-DDoS, перечень отслеживаемых портов, набор DNAT-правил, список добавленных панелью IP-адресов. Изменения, способные оборвать связь (доп. IP-адреса на интерфейсе), нода применяет транзакцией с таймером отката на хосте, а панель подтверждает их, заново подключившись к ноде; без подтверждения нода сама возвращает бэкап.
...
### Запас исходящих портов

Потолок исходящих TCP-соединений считается не на сервер, а на четвёрку (наш адрес, наш порт, адрес цели, порт цели): на пару «адрес ноды → адрес:порт цели» ядро выдаст не больше портов, чем в эфемерном диапазоне, а быстро — только половину: `connect()` первым проходом перебирает порты одной чётности, и когда они кончаются, каждое соединение перебирает их целиком. Релей, гонящий всех клиентов на один backend, Xray, у которого весь Google идёт на горстку адресов, и nginx перед Xray на `127.0.0.1` упираются в этот потолок при полностью свободном сервере — и упираются молча, отдавая процессор перебору портов вместо трафика. Нода считает занятость быстрой половины по `/proc/net/tcp` в составе метрик, панель показывает её на странице трафика: полоса и остаток на каждый исходящий адрес, внутри — сами направления. Способы поднять потолок — добавить исходящий адрес (раздел «Исходящие адреса», раскладка через policy routing), развести трафик по разным портам цели; для nginx → Xray шаблон профиля Remnawave nginx сам раскладывает соединения по адресам источника `127.0.0.1–127.0.0.16`. Подробности — в [node/DOCUMENTATION.md](node/DOCUMENTATION.md#учёт-эфемерных-портов-ephemeral_portspy) и [panel/DOCUMENTATION.md](panel/DOCUMENTATION.md#traffic).
