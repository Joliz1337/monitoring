...
# The node used to install /etc/cron.d/certbot-renew, which ran
# `systemctl stop haproxy` unconditionally every night at 03:00 — dropping every
# established tunnel on every port, even though certbot only renews certificates
# expiring within 30 days (i.e. it did nothing 29 nights out of 30). Certificate
# renewal is operator-driven now, from the HAProxy page in the panel.
# Exact filenames only: the Debian certbot package ships /etc/cron.d/certbot and
# certbot.timer — those belong to apt and must not be touched.
cleanup_legacy_cert_cron() {
    if [ -f /etc/cron.d/certbot-renew ] || [ -f /opt/monitoring-node/renew-certs.sh ]; then
        rm -f /etc/cron.d/certbot-renew /opt/monitoring-node/renew-certs.sh 2>/dev/null || true
        rm -f /var/log/certbot-renew.log 2>/dev/null || true
        log_success "Removed legacy certbot renewal cron (was stopping HAProxy nightly)"
    fi
}
...
# Возвращает максимум hw-очередей для интерфейса.
# Поддерживает драйверы с Combined и драйверы с раздельными RX/TX (mlx4_en, часть igb/ixgbe).
# Fallback — подсчёт rx-очередей в sysfs (на случай драйверов без channels API).
get_max_hw_queues() {
    local iface=$1
    ...
    if command -v ethtool &>/dev/null; then
        ...
        if [[ "$cb" =~ ^[0-9]+$ ]] && [ "$cb" -gt 0 ]; then
            echo "$cb"; return 0
        fi
        local eff=0
        [[ "$rx" =~ ^[0-9]+$ ]] && [ "$rx" -gt "$eff" ] && eff=$rx
        [[ "$tx" =~ ^[0-9]+$ ]] && [ "$tx" -gt "$eff" ] && eff=$tx
        if [ "$eff" -gt 0 ]; then
            echo "$eff"; return 0
        fi
    fi
...
detect_multiqueue_support() {
    ...
    for dev_path in /sys/class/net/*; do
        ...
        local max_q
        max_q=$(get_max_hw_queues "$iface")

        found_any=true
        if [ "$max_q" -gt 1 ] 2>/dev/null; then
            echo -e "  ${GREEN}✓${NC} $iface: $(msg nic_hw_supported) ($(msg nic_hw_max_queues): $max_q)"
        else
            echo -e "  ${YELLOW}✗${NC} $iface: $(msg nic_hw_not_supported) ($(msg nic_hw_max_queues): ${max_q:-n/a})"
        fi
    done
...
apply_system_optimizations() {
    ...
    # The INPUTS now live on the host under /opt/monitoring, and tune-sysctl.sh
    # renders /etc/sysctl.d/99-vless-tuning.conf from them with every
    # size-dependent value computed from this host's MemTotal/nproc. Keeping the
    # inputs is what lets the boot-time re-render work offline (and pick up a
    # VPS resize) — the old flow copied straight into /etc/sysctl.d and kept no
    # source.
    mkdir -p "$OPT_DIR/configs/profiles" "$OPT_DIR/scripts" 2>/dev/null || true

    local RENDER_INPUTS="profiles/common.base.conf profiles/$opt_profile.base.conf \
profiles/limits.tmpl profiles/systemd-limits.tmpl"
    ...
    if [ -n "$CONFIG_SRC" ] && [ -f "$CONFIG_SRC/tune-sysctl.sh" ]; then
        ...
        for f in $RENDER_INPUTS; do
            if [ ! -f "$CONFIG_SRC/$f" ]; then
                log_error "Missing optimization input: $CONFIG_SRC/$f"
                return 1
            fi
            cp "$CONFIG_SRC/$f" "$OPT_DIR/configs/$f" 2>/dev/null || true
            chmod 644 "$OPT_DIR/configs/$f" 2>/dev/null || true
        done
        cp "$CONFIG_SRC/tune-sysctl.sh" "$OPT_DIR/scripts/tune-sysctl.sh" 2>/dev/null || true
        chmod +x "$OPT_DIR/scripts/tune-sysctl.sh" 2>/dev/null || true
        [ -f "$CONFIG_SRC/VERSION" ] && cp "$CONFIG_SRC/VERSION" "$OPT_DIR/configs/VERSION" 2>/dev/null || true
        log_success "Optimization inputs installed"
    else
        ...
        # A truncated download now lands in /opt/monitoring/configs, where the
        # renderer's token guard catches it, instead of going straight into
        # /etc/sysctl.d as a half-written file.
        local f
        for f in $RENDER_INPUTS; do
            if ! download_config "$f" "$OPT_DIR/configs/$f"; then
                log_error "Failed to download $f"
                return 1
            fi
            chmod 644 "$OPT_DIR/configs/$f" 2>/dev/null || true
        done
        if ! download_config "tune-sysctl.sh" "$OPT_DIR/scripts/tune-sysctl.sh"; then
            log_error "Failed to download tune-sysctl.sh"
            return 1
        fi
        chmod +x "$OPT_DIR/scripts/tune-sysctl.sh" 2>/dev/null || true
        download_config "VERSION" "$OPT_DIR/configs/VERSION" >/dev/null 2>&1 || true
        log_success "Optimization inputs downloaded"
    fi

    # Marker must exist before the renderer runs: the boot-time ExecStartPre has
    # no argument and reads the profile from here.
    ...
    # The renderer owns the whole apply sequence, in this order: modprobe
    # nf_conntrack (net.netfilter.* silently no-op without it) -> render ->
    # raise fs.nr_open BEFORE writing limits.conf (a limits value above nr_open
    # makes PAM fail and can break login) -> sysctl -e -p -> daemon-reload.
    log_info "Rendering and applying kernel tuning..."
    if ! "$OPT_DIR/scripts/tune-sysctl.sh" render "$opt_profile"; then
        log_error "Kernel tuning render failed — previous config left in place"
        return 1
    fi
    log_success "Kernel tuning applied (see $OPT_DIR/configs/tuning-facts.json)"
    ...
    # The NIC tune scripts set the per-queue RPS/XPS/IRQ values that the
    # renderer cannot express in a .conf file, so verify after they have run.
    #
    # Single implementation: the renderer compares live kernel values against
    # the facts file it just wrote, so expectations are per-host and cannot go
...
# Where the shipped configs/ directory lives for this run: the freshly cloned
# copy if we downloaded one, else next to the installer. Empty means "not
# available locally" and every caller falls back to a GitHub raw download.
detect_config_src() {
    if [ -d "$TMP_DIR/configs" ]; then
        echo "$TMP_DIR/configs"
    elif [ -n "$0" ] && [ -d "$(dirname "$0")/configs" ]; then
        echo "$(dirname "$0")/configs"
    fi
}

# The anti-DDoS watchdog used to be installed only on demand from the panel, so
# a freshly deployed node had no protection at all until an operator clicked a
# button. It is installed and enabled here instead, in its resting state
# (watchdog=on, mode=off): the emergency chain only appears when an attack is
# actually detected, and disappears after the hysteresis window.
install_antiddos_watchdog() {
    ...
    if [ -n "$CONFIG_SRC" ] && [ -f "$CONFIG_SRC/ddos-watchdog.sh" ]; then
        cp "$CONFIG_SRC/ddos-watchdog.sh" "$OPT_DIR/scripts/ddos-watchdog.sh" 2>/dev/null || true
        cp "$CONFIG_SRC/ddos-watchdog.service" /etc/systemd/system/ddos-watchdog.service 2>/dev/null || true
    else
        timeout "$TIMEOUT_CURL" curl -fsSL --connect-timeout 30 --max-time "$TIMEOUT_CURL" \
            "$GITHUB_RAW/ddos-watchdog.sh" -o "$OPT_DIR/scripts/ddos-watchdog.sh" 2>/dev/null || {
            log_warn "Could not download ddos-watchdog.sh — anti-DDoS not installed"
            return 0
        }
        timeout "$TIMEOUT_CURL" curl -fsSL --connect-timeout 30 --max-time "$TIMEOUT_CURL" \
            "$GITHUB_RAW/ddos-watchdog.service" -o /etc/systemd/system/ddos-watchdog.service 2>/dev/null || {
            log_warn "Could not download ddos-watchdog.service — anti-DDoS not installed"
            return 0
        }
    fi
    ...
    timeout "$TIMEOUT_SYSTEMCTL" systemctl daemon-reload >/dev/null 2>&1 || true
    timeout "$TIMEOUT_SYSTEMCTL" systemctl enable --now ddos-watchdog.service >/dev/null 2>&1 \
        && log_success "Anti-DDoS watchdog installed and running (detection on, rules off)" \
        || log_warn "Anti-DDoS watchdog installed but could not be started"
}
...
install_remnawave() {
    ...
    # Container FD limits must be <= fs.nr_open, otherwise setrlimit returns
    # EPERM and the container refuses to start. The old hardcoded 1048576 broke
    # exactly that way on any host whose profile set a lower nr_open. Derive it
    # from the same single number the rest of the chain uses.
    local DOCKER_NOFILE=65536
    if [ -r /opt/monitoring/configs/tuning-facts.env ]; then
        # shellcheck source=/dev/null
        DOCKER_NOFILE=$(. /opt/monitoring/configs/tuning-facts.env 2>/dev/null; echo "${DOCKER_NOFILE:-65536}")
    fi
    case "$DOCKER_NOFILE" in
        ''|*[!0-9]*) DOCKER_NOFILE=65536 ;;
    esac

    # Unquoted heredoc so $DOCKER_NOFILE expands; the block has no other $ or
    # backticks, so nothing else changes meaning.
...
install_warp() {
    ...
    local warp_repo_list="/etc/apt/sources.list.d/cloudflare-client.list"

    # Репозиторий Cloudflare
    curl -fsSL https://pkg.cloudflareclient.com/pubkey.gpg | \
        gpg --yes --dearmor --output /usr/share/keyrings/cloudflare-warp-archive-keyring.gpg 2>/dev/null

    # Cloudflare публикует пакеты не для всех релизов — неподдерживаемые маппим на noble
    local codename
    codename=$(lsb_release -cs 2>/dev/null)
    if [[ ! " bullseye bookworm jammy noble " =~ " ${codename} " ]]; then
        log_warn "$(msg warp_codename_fallback): ${codename} → noble"
        codename="noble"
    fi

    echo "deb [signed-by=/usr/share/keyrings/cloudflare-warp-archive-keyring.gpg] \
https://pkg.cloudflareclient.com/ ${codename} main" | \
        tee "$warp_repo_list" >/dev/null

    apt-get update -qq >/dev/null 2>&1

    if ! apt-get install -y cloudflare-warp >/dev/null 2>&1 || ! command -v warp-cli &>/dev/null; then
        log_error "$(msg warp_install_failed)"
        # Удаляем нерабочий репозиторий, иначе он ломает apt update в дальнейшем
        rm -f "$warp_repo_list"
        apt-get update -qq >/dev/null 2>&1
        return 1
    fi
